...
const WanLinkPanel: React.FC = () => {
  ...
  const [testedFormSignature, setTestedFormSignature] = useState('');
  ...
  const load = async (signal?: AbortSignal, silent = false) => {
    ...
    try {
      ...
      if (roleFilter) query.set('link_role', roleFilter);
      if (groupFilter) query.set('group_id', groupFilter);
      if (keyword.trim()) query.set('keyword', keyword.trim());
      const [linkResp, optionResp, groupResp] = await Promise.all([
        ...
        fetch('/api/monitoring/wan-link-groups', { headers: tokenHeaders(), signal }),
      ]);
      if (!linkResp.ok) throw new Error(linkResp.status === 401 ? (zh ? '登录已失效，请重新登录' : 'Login expired') : linkResp.status === 403 ? (zh ? '权限不足' : 'Permission denied') : linkResp.status >= 500 ? (zh ? '服务器暂时不可用，请重试' : 'Server unavailable; retry') : (zh ? '出口链路加载失败' : 'Unable to load WAN links'));
      ...
      if (groupResp.ok) {
        const groupPayload = await groupResp.json();
        setGroupOptions((groupPayload.items || []).map((item: { id: string; group_name: string }) => ({ id: item.id, group_name: item.group_name })));
      }
      setSelectedId((current) => focusedLinkRef.current || (payload.items?.some((item: WanLink) => item.id === current) ? current : payload.items?.[0]?.id || ''));
    } catch (error) {
  ...
  const loadHistory = async (id: string, minutes = historyMinutes, signal?: AbortSignal) => {
    if (!id) { setHistory(null); setHistoryError(''); return; }
    setHistoryError('');
    try {
      ...
      if (!response.ok) throw new Error(response.status === 401 ? (zh ? '登录已失效，请重新登录' : 'Login expired') : response.status === 403 ? (zh ? '权限不足' : 'Permission denied') : response.status >= 500 ? (zh ? '趋势服务暂时不可用，请重试' : 'History service unavailable; retry') : (zh ? '趋势加载失败' : 'Unable to load history'));
      if (!signal?.aborted) setHistory(await response.json());
    } catch (error) {
      if ((error as Error)?.name !== 'AbortError') { setHistoryError(error instanceof Error ? error.message : String(error)); showToast(error instanceof Error ? error.message : String(error), 'error'); }
    }
  };

  useEffect(() => { const controller = new AbortController(); void load(controller.signal); return () => controller.abort(); }, [page, siteFilter, providerFilter, healthFilter, roleFilter, groupFilter, keyword]);
  ...
  }, [page, siteFilter, providerFilter, healthFilter, roleFilter, groupFilter, keyword, selectedId, historyMinutes]);
  useEffect(() => {
    if (!formOpen) return;
  ...
  useEffect(() => {
    const focus = (event: Event) => {
      const detail = (event as CustomEvent<{ linkId?: string; startedAt?: string }>).detail || {};
      if (!detail.linkId) return;
      focusedLinkRef.current = detail.linkId;
      setSelectedId(detail.linkId);
      const started = detail.startedAt ? new Date(detail.startedAt).getTime() : Number.NaN;
      if (Number.isFinite(started)) {
        const minutes = Math.max(5, Math.min(43200, Math.ceil((Date.now() - started) / 60000) + 60));
        setHistoryMinutes(minutes);
      }
    };
    window.addEventListener('wan:focus-link', focus);
    return () => window.removeEventListener('wan:focus-link', focus);
  }, []);

  const selected = links.find((item) => item.id === selectedId) || history?.link || null;
  const summary = useMemo(() => ({
    total: totalLinks,
    healthy: Number.isFinite(Number(serverSummary.healthy_links)) ? Number(serverSummary.healthy_links) : links.filter((item) => item.health_status === 'healthy').length,
    // The list endpoint calculates these over the filtered result set, not
    // just the current page.  Falling back to page values keeps older
    // backends usable while preventing a paginated view from understating
    // the overall出口汇总.
    download: serverSummary.download_bps != null && Number.isFinite(Number(serverSummary.download_bps)) ? Number(serverSummary.download_bps) : links.some((item) => item.download_bps != null) ? links.reduce((sum, item) => sum + Number(item.download_bps || 0), 0) : null,
    upload: serverSummary.upload_bps != null && Number.isFinite(Number(serverSummary.upload_bps)) ? Number(serverSummary.upload_bps) : links.some((item) => item.upload_bps != null) ? links.reduce((sum, item) => sum + Number(item.upload_bps || 0), 0) : null,
    maxUtil: serverSummary.max_util_pct != null && Number.isFinite(Number(serverSummary.max_util_pct)) ? Number(serverSummary.max_util_pct) : (() => { const values = links.flatMap((item) => [item.download_util_pct, item.upload_util_pct].filter((value): value is number => value != null).map(Number)); return values.length ? Math.max(...values) : null; })(),
    alerts: Number.isFinite(Number(serverSummary.active_alerts)) ? Number(serverSummary.active_alerts) : links.reduce((sum, item) => sum + Number(item.active_alert_count || 0), 0),
  ...
  const chartData = (history?.history || []).map((item) => ({
    ...
    errorRate: item.in_error_rate == null && item.out_error_rate == null ? null : Number(item.in_error_rate || 0) + Number(item.out_error_rate || 0),
    discardRate: item.in_discard_rate == null && item.out_discard_rate == null ? null : Number(item.in_discard_rate || 0) + Number(item.out_discard_rate || 0),
  }));
  const hasValidChartData = chartData.some((item) => item.download != null || item.upload != null || item.downloadUtil != null || item.uploadUtil != null || item.errors != null || item.discards != null);
  ...
  const formSignature = `${form.site_id || ''}|${form.device_id || ''}|${form.interface_id || ''}|${form.if_index || ''}`;
  const update = (key: string, value: any) => {
    setForm((current) => ({ ...current, [key]: value }));
    if (['site_id', 'device_id', 'interface_id', 'if_index'].includes(key)) setTestedFormSignature('');
  };
  const testForm = async () => {
    setTestingForm(true);
    try {
      const payload = { ...form, if_index: form.if_index ? Number(form.if_index) : undefined, contracted_download_mbps: Number(form.contracted_download_mbps), contracted_upload_mbps: Number(form.contracted_upload_mbps), collection_interval_sec: Number(form.collection_interval_sec) };
      const response = await fetch('/api/monitoring/wan-links/test-config', { method: 'POST', headers: { ...tokenHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok || body.status !== 'success') throw new Error(body?.error_message || body?.error_code || (zh ? '采集测试未通过' : 'Collection test did not pass'));
      setTestedFormSignature(formSignature);
      showToast(zh ? '采集测试通过，可以保存链路' : 'Collection test passed; the link can be saved', 'success');
    } catch (error) { setTestedFormSignature(''); showToast(error instanceof Error ? error.message : String(error), 'error'); } finally { setTestingForm(false); }
  };
  const save = async () => {
    if (testedFormSignature !== formSignature) {
      showToast(zh ? '请先完成设备/接口采集测试' : 'Run the device/interface collection test before saving', 'error');
      return;
    }
    setSaving(true);
    try {
  ...
  const remove = async (id: string) => {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body?.detail?.message || body?.detail || (zh ? '出口链路删除失败' : 'Unable to delete WAN link'));
      showToast(zh ? '出口链路已删除' : 'WAN link deleted', 'success');
      await load();
    } catch (error) {
  ...
  return <section className="mt-5 rounded-2xl border border-[var(--card-border)] bg-[var(--card-bg)] p-4 shadow-sm md:p-5">
    {historyError && <div className="mb-3 flex items-center justify-between rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs text-rose-700"><span>{historyError}</span><button type="button" onClick={() => void loadHistory(selectedId, historyMinutes)} className="font-bold underline">{zh ? '重试趋势' : 'Retry history'}</button></div>}
