...
const AppMainContent: React.FC = () => {
  ...
  const mainClassName = `relative flex-1 min-w-0 flex flex-col min-h-0 ${isStandaloneScrollTab ? 'overflow-hidden' : 'overflow-y-auto'}`;
  ...
  return (
    <main className={mainClassName}>
      ...
      {activeTab === 'access' && (
        <Suspense fallback={lazyPanelFallback}>
          <AccessCenterTab devices={devices} language={language} showToast={showToast} />
        </Suspense>
      )}
      ...
      {activeTab === 'cmdb' && (
        <Suspense fallback={lazyPanelFallback}>
          {cmdbPage === 'racks' ? (
            ...
            <CmdbManagementTab language={language} cmdbPage={cmdbPage} />
          )}
        </Suspense>
      )}
          ...
          <TopologyPage
            ...
            onTopologyProtocolFilterChange={setTopologyProtocolFilter}
             ...
             onOpenWorkspace={(device) => {
               ...
               if (!workspaceWindow) {
                 showToast(language === 'zh' ? '浏览器阻止了新窗口，请允许本站弹窗后重试' : 'The browser blocked the new tab. Allow pop-ups and try again.', 'error');
               }
             }}
      ...
      {activeTab === 'credentials' && (
        <Suspense fallback={lazyPanelFallback}>
          <CredentialManagementTab language={language} />
        </Suspense>
      )}
