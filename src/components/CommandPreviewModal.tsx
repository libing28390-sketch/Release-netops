...
const CommandPreviewModal: React.FC<CommandPreviewModalProps> = ({
  ...
  useEscapeClose(open && Boolean(preview), onClose);
      ...
      >
        <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            <ActionButton icon={Copy} variant="accent" size="sm" onClick={onCopyAll}>
              {language === 'zh' ? '复制全部' : 'Copy All'}
            </ActionButton>
            <ActionIconButton icon={X} label={language === 'zh' ? '关闭' : 'Close'} onClick={onClose} className="text-white/60 hover:text-white" />
          </div>
        </div>
