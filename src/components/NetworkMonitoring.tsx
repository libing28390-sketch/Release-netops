...
import {
  ...
  ChevronLeft, ChevronRight, Network, FileSpreadsheet,
} from 'lucide-react';
...
interface NetworkMonitoringProps {
  ...
  onOpenSnmpWalk?: (device: Device) => void;
}
...
type MetricProvider = 'native' | 'shadow' | 'victoriametrics';
...
const NetworkMonitoring: React.FC<NetworkMonitoringProps> = ({
  ...
  const handleExportInterfacesExcel = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/monitoring/v1/export/interfaces?format=xlsx', { headers, credentials: 'same-origin' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const blob = await res.blob();
      const disposition = res.headers.get('Content-Disposition') || '';
      let filename = `nexora_interfaces_${new Date().toISOString().slice(0, 10)}.xlsx`;
      ...
      showToast(zh ? `已成功导出报表：${filename}` : `Exported: ${filename}`, 'success');
    } catch (e: any) {
      showToast(zh ? `导出失败: ${e?.message || '网络异常'}` : `Export error: ${e?.message}`, 'error');
    } finally {
  ...
  const [syncingHostname, setSyncingHostname] = useState(false);
  const [batchSyncing, setBatchSyncing] = useState(false);
  const [rangeHours, setRangeHours] = useState<number>(24);
  const [autoRefresh, setAutoRefresh] = useState(true);
  const [metricProvider, setMetricProvider] = useState<MetricProvider>('native');
  const [providerStatus, setProviderStatus] = useState<'loading' | 'ready' | 'unavailable'>('loading');

  // Docker advertises the active provider through the backend environment;
  // resolve it once so the page does not silently keep using Native after a
  ...
  useEffect(() => {
    ...
    fetch('/api/monitoring/provider', { headers: authHeaders(), signal: controller.signal })
      .then(async (response) => {
        const payload = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(payload?.detail || 'provider discovery failed');
        return payload;
      })
      .then((payload) => {
        const provider = String(payload?.provider || '').toLowerCase() as MetricProvider;
        if (['native', 'shadow', 'victoriametrics'].includes(provider)) setMetricProvider(provider);
        setProviderStatus('ready');
      })
      .catch((error) => {
        if (!(error instanceof DOMException && error.name === 'AbortError')) setProviderStatus('unavailable');
      });
  ...
  const canRunSnmpWalk = ['administrator', 'operator'].includes(String(currentUserRole).trim().toLowerCase());
  ...
  const fetchRealtime = useCallback(async (deviceId: string, signal?: AbortSignal) => {
    const params = new URLSearchParams({
      window_minutes: String(REALTIME_WINDOW_MINUTES),
      limit: '1000',
    });
    if (metricProvider) {
      params.set('provider', metricProvider);
    }
    const resp = await fetch(`/api/monitoring/device/${deviceId}/realtime?${params.toString()}`, {
      headers: authHeaders(),
      signal,
    });
  ...
  const fetchTrend = useCallback(async (deviceId: string, hours: number, interfaceName?: string, signal?: AbortSignal) => {
    ...
    if (payload?.status === 'error') throw new Error(payload?.error || 'metric provider query failed');
  ...
  const handleSyncHostname = useCallback(async () => {
    if (!selectedId || syncingHostname) return;
    setSyncingHostname(true);
    try {
      const resp = await fetch(`/api/devices/${selectedId}/sync-hostname`, {
        method: 'POST',
        headers: authHeaders(),
      });
      const data = await resp.json().catch(() => ({}));
      if (!resp.ok) {
        throw new Error(data.detail || (zh ? '同步失败' : 'Failed to sync hostname'));
      }
      showToast(data.message || (zh ? '主机名同步成功' : 'Hostname synced successfully'), 'success');
      setDeviceRows((prev) =>
        prev.map((d) =>
          d.id === selectedId
            ? { ...d, hostname: data.new_hostname, sys_name: data.raw_sys_name }
            : d
        )
      );
    } catch (err: any) {
      showToast(err?.message || (zh ? '主机名同步失败' : 'Failed to sync hostname'), 'error');
    } finally {
      setSyncingHostname(false);
    }
  }, [selectedId, syncingHostname, showToast, zh]);
  ...
  const handleBatchSyncHostnames = useCallback(async () => {
    if (batchSyncing) return;
    setBatchSyncing(true);
    try {
      const resp = await fetch('/api/devices/batch-sync-hostname', {
        method: 'POST',
        headers: { ...authHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({
          device_ids: deviceRows.map((d) => d.id),
        }),
      });
      const data = await resp.json().catch(() => ({}));
      if (!resp.ok) {
        throw new Error(data.detail || (zh ? '批量同步失败' : 'Batch sync failed'));
      }
      showToast(
        data.message ||
          (zh
            ? `批量同步完成：更新 ${data.updated || 0} 台，未变更 ${data.unchanged || 0} 台`
            : `Batch sync complete: ${data.updated || 0} updated, ${data.unchanged || 0} unchanged`),
        data.failed > 0 ? 'warning' : 'success'
      );
      if (Array.isArray(data.details)) {
        const detailMap = new Map<string, { new_hostname?: string; raw_sys_name?: string }>();
        data.details.forEach((item: any) => {
          if (item?.device_id) {
            detailMap.set(String(item.device_id), item);
          }
        });
        setDeviceRows((prev) =>
          prev.map((d) => {
            const match = detailMap.get(d.id);
            if (match && match.new_hostname) {
              return { ...d, hostname: match.new_hostname, sys_name: match.raw_sys_name };
            }
            return d;
          })
        );
      ...
      showToast(err?.message || (zh ? '批量同步主机名失败' : 'Failed to batch sync hostnames'), 'error');
    } finally {
      setBatchSyncing(false);
    }
  }, [alertsPage, alertsPhase, alertsSeverity, batchSyncing, deviceRows, loadDeviceData, rangeHours, selectedId, showToast, trendInterface, zh]);
      ...
      <PageHero
        ...
        actions={
          <div className="flex items-center gap-2">
            <label className="flex items-center gap-1.5 rounded-xl border border-gray-200/70 bg-white px-2.5 py-1.5 text-xs font-semibold text-gray-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
              <Database size={12} className="text-cyan-600" />
              <span className="sr-only">{zh ? '指标后端' : 'Metric provider'}</span>
              <select
                value={metricProvider}
                onChange={(event) => setMetricProvider(event.target.value as MetricProvider)}
                className="bg-transparent text-xs font-semibold outline-none cursor-pointer"
                aria-label={zh ? '指标后端' : 'Metric provider'}
              >
                <option value="native">{zh ? '原生遥测' : 'Native'}</option>
                <option value="shadow">{zh ? '对比模式' : 'Shadow'}</option>
                <option value="victoriametrics">VictoriaMetrics</option>
              </select>
              <span className={`hidden text-[10px] font-bold sm:inline ${providerStatus === 'ready' ? 'text-emerald-600' : providerStatus === 'loading' ? 'text-amber-600' : 'text-rose-600'}`} title={zh ? '后端指标数据源状态' : 'Metric provider status'}>{providerStatus === 'loading' ? (zh ? '读取中' : 'Loading') : providerStatus === 'ready' ? (zh ? '已配置' : 'Configured') : (zh ? '不可用' : 'Unavailable')}</span>
            </label>
            ...
            <button
              ...
              onClick={handleExportInterfacesExcel}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border border-emerald-600/30 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 transition-all cursor-pointer"
              title={zh ? '导出包含多工作表（接口状态、流量统计、高负荷预警、错包排行）的专业 Excel 报表' : 'Export interfaces multi-sheet Excel'}
            >
              <FileSpreadsheet size={13} className={exportingExcel ? 'animate-spin' : 'text-emerald-600 dark:text-emerald-400'} />
              <span>{exportingExcel ? (zh ? '正在导出…' : 'Exporting…') : zh ? '导出 Excel' : 'Export Excel'}</span>
            </button>
            <a
              href="/monitor/grafana"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border border-cyan-500/20 bg-cyan-500/10 text-cyan-700 dark:text-cyan-300 hover:bg-cyan-500/20 transition-all cursor-pointer"
              title={zh ? '在 Grafana 中查看全网接口与设备时序大盘' : 'View Grafana dashboards'}
            >
              <Activity size={13} />
              <span>{zh ? 'Grafana 监控大盘' : 'Grafana Dashboard'}</span>
            </a>
          </div>
        }
      />
        ...
        <div className="w-[270px] shrink-0 border-r border-gray-200/70 dark:border-zinc-800/80 bg-gray-50/40 dark:bg-zinc-900/30 flex flex-col overflow-hidden">
          <div className="p-3 border-b border-gray-200/70 dark:border-zinc-800/80 space-y-2.5">
            ...
            <div className="flex items-center justify-between text-[11px] font-medium text-gray-400 px-0.5">
              ...
              <button
                type="button"
                onClick={handleBatchSyncHostnames}
                disabled={batchSyncing || deviceListLoading || deviceTotal === 0}
                title={zh ? '通过 RFC 1213 通用 SNMP sysName 一键批量校准在管设备主机名' : 'Batch sync hostnames via standard SNMP sysName'}
                className="flex items-center gap-1 text-[11px] font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 disabled:opacity-40 transition cursor-pointer"
              >
                <RefreshCw size={11} className={batchSyncing ? 'animate-spin' : ''} />
                <span>{batchSyncing ? (zh ? '同步中…' : 'Syncing…') : (zh ? '批量同步' : 'Batch Sync')}</span>
              </button>
            </div>
              ...
              <div className="bg-white dark:bg-zinc-900/90 border border-gray-200/70 dark:border-zinc-800/80 flex items-center justify-between rounded-2xl p-4 shadow-2xs">
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {selectedDevice.sys_name && selectedDevice.sys_name !== selectedDevice.hostname && (
                        <span className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-800/60" title={zh ? 'SNMP 报告的系统名称' : 'SNMP sysName'}>
                          sysName: {selectedDevice.sys_name}
                        </span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-3">
                  {canRunSnmpWalk && (
                    <button
                      type="button"
                      onClick={() => onOpenSnmpWalk?.(selectedDevice)}
                      title={zh ? '使用该设备已关联的 SNMP 凭据执行只读 Walk' : 'Run a read-only walk with this device’s linked SNMP credential'}
                      className="flex items-center gap-1.5 rounded-xl border border-cyan-200 bg-cyan-50/80 px-3 py-1.5 text-xs font-semibold text-cyan-800 transition hover:bg-cyan-100 dark:border-cyan-900/70 dark:bg-cyan-950/35 dark:text-cyan-200 dark:hover:bg-cyan-950/55"
                    >
                      <Database size={12} />
                      <span>SNMP Walk</span>
                    </button>
                  ...
                  <button
                    type="button"
                    onClick={handleSyncHostname}
                    disabled={syncingHostname}
                    title={zh ? '通过通用 SNMP sysName 校验并同步设备主机名' : 'Validate and sync hostname from device via universal SNMP sysName'}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border border-blue-200 bg-blue-50/80 text-blue-700 hover:bg-blue-100 transition cursor-pointer disabled:cursor-wait disabled:opacity-60"
                  >
                    <RefreshCw size={12} className={syncingHostname ? 'animate-spin text-blue-600' : ''} />
                    <span>{syncingHostname ? (zh ? '同步中…' : 'Syncing…') : (zh ? '同步主机名' : 'Sync Hostname')}</span>
                  </button>
