...
const normalizeMarkdownContent = (value: string): string => (
  value.replace(/\b(https?)\\:\/\//gi, '$1://')
);

const normalizeSafeHref = (value: string | undefined): string | null => {
  const href = String(value || '').trim()
    .replace(/^<|>$/g, '')
    .replace(/^https?\\:\/\//i, (match) => match.replace('\\', ''));
...
const CodeBlockHeader: React.FC<{ language: string; code: string }> = ({ language, code }) => {
  ...
  const pointerCopyTriggered = React.useRef(false);
  ...
  const handlePointerUp = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    pointerCopyTriggered.current = true;
    void handleCopy();
    window.setTimeout(() => {
      pointerCopyTriggered.current = false;
    }, 0);
  };

  const handleButtonClick = () => {
    if (pointerCopyTriggered.current) return;
    void handleCopy();
  };
  ...
  return (
    <div className="flex items-center justify-between px-3.5 py-1.5 bg-[#18181b] border-b border-gray-800 rounded-t-xl text-xs font-mono select-none">
      ...
      <ActionButton
        ...
        variant={copied ? 'success' : copyFailed ? 'danger' : 'default'}
        size="sm"
        aria-label={copied ? '已复制代码' : '复制代码'}
        onPointerUp={handlePointerUp}
        onClick={handleButtonClick}
        title={copyFailed ? '复制失败，请检查浏览器权限' : copied ? '已复制代码' : '复制代码'}
        className="!h-7 !border-transparent !bg-transparent !px-2 !text-[11px] !text-gray-400 hover:!bg-gray-800 hover:!text-white"
      >
...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content, className = '' }) => {
  return (
    <div
      className={`markdown-body select-text space-y-3 leading-relaxed text-sm ${className}`}
