...
export const AppModals: React.FC<any> = (props) => {
  ...
  useEscapeClose(Boolean(showLifecycleConfirm), () => { setShowLifecycleConfirm(null); setLifecycleConfirmChecked(false); });
  ...
  return (
    <>
      <ProfileModal
        ...
        onEmailPreferenceChange={(field, value) => setNotificationChannels((prev) => ({ ...prev, email: { ...prev.email, [field]: value } }))}
          ...
          try {
            const response = await fetch(`/api/users/${profileUserId}/notify-test`, {
              ...
              headers: {
                ...
                'Authorization': `Bearer ${localStorage.getItem('netops_token') || ''}`,
              },
              body: JSON.stringify({
                platform: channel,
      ...
      {showDetailsModal && viewingDevice && (
        <DeviceDetailModal
          ...
          onPlatformBindingSaved={(binding) => {
            if (typeof setViewingDevice === 'function') {
              setViewingDevice((current: any) => current ? { ...current, ...binding } : current);
            }
          }}
