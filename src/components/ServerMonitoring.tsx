...
const NETWORK_PLATFORM_CODES = new Set([
  ...
  'checkpoint_gaia', 'hillstone_stoneos', 'ruijie_os', 'zte_zxros',
]);
...
const ServerMonitoring: React.FC<ServerMonitoringProps> = ({ language, devices, showToast, isAuthenticated = true }) => {
  ...
  }, [serverDevices, search, siteFilter, statusFilter]);

  const selectedDevice = useMemo(
    () => serverDevices.find((d) => d.id === selectedId) || null,
    [serverDevices, selectedId],
  );
  ...
  useEffect(() => {
    if (!selectedId && filteredServers.length > 0) {
  ...
  useEffect(() => {
    if (!isAuthenticated || !selectedId) { setSeries([]); return; }
    let cancelled = false;
  ...
  }, [isAuthenticated, selectedId, rangeHours, fetchTrend]);
  ...
  }, [autoRefresh, selectedId, rangeHours, fetchTrend]);
  ...
  }, [selectedId, rangeHours, fetchTrend, showToast, zh]);
    ...
    return (
      <div className="flex flex-col h-full overflow-hidden">
        <PageHero
          ...
          subtitle={zh ? 'Linux 主机性能遥测 · 基于巡检指标集' : 'Linux host telemetry · Driven by inspection metrics'}
        />
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center max-w-md px-6">
            ...
            <p className="mt-1 text-xs text-[var(--muted-text)]">
              {zh
                ? '请在「资产管理」中添加平台为 Linux 或分类为服务器的设备。'
                : 'Add devices with platform Linux or category server in Asset Management.'}
            </p>
          </div>
        </div>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-[11px] font-mono text-[var(--muted-text)]">{srv.ip_address}</p>
                  </div>
                </button>
              );
            }}
