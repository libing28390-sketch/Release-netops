...
export const useConfigWorkspace = ({
  ...
  const configVendorScopedDevices = useMemo(() => {
    ...
    return devices.filter(
      (device) => getVendorFromPlatform(device.platform).toLowerCase() === vendor.toLowerCase(),
    );
  ...
  const handleValidateTemplateWorkspace = useCallback(() => {
  ...
  }, [configValidationIssues, language, showToast]);
  ...
  }, [selectedConfigTemplate, editorContent, currentUser, refreshConfigWorkspace, language, showToast]);
