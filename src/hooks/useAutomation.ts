...
import { isAutomationPlatformSupported, normalizeAutomationPlatform } from '../pages/AutomationExecute/helpers';
import { authHeaders } from '../api/http';

const QUICK_QUERY_TIMEOUT_MS = 45_000;
...
export const useAutomation = ({ 
  currentUser, isAuthenticated, language, showToast, navigate,
  setSelectedDevice,
  ...
  // Quick-query requests can involve slow or unreachable devices. Keep a
  // cancellation handle so closing the result window immediately releases the
  // UI instead of waiting for the fetch promise to settle.
  const quickQueryAbortRef = useRef<AbortController | null>(null);
  const quickQueryRunIdRef = useRef(0);
  ...
  const formatQuickQueryRawOutput = useCallback((payload: any) => {
    ...
    if (category?.source === 'snmp') {
      if (category?.error_code === 'UNSUPPORTED_VENDOR') {
        return language === 'zh'
          ? '该厂商或平台尚无经过核实的 SNMP 光模块定义，已跳过采集。'
          : 'No verified SNMP optical definition exists for this vendor or platform; collection was skipped.';
      }
      if (category?.error_code === 'SNMP_CREDENTIALS_MISSING') {
        return language === 'zh'
          ? '未配置可用的 SNMP 凭据，无法读取光模块数据。'
          : 'SNMP credentials are not configured, so optical data could not be read.';
      }
      if (!category?.success) {
        const detail = String(category?.error || '').trim();
        return language === 'zh'
          ? `SNMP 光模块采集失败${detail ? `：${detail}` : '。'}`
          : `SNMP optical collection failed${detail ? `: ${detail}` : '.'}`;
      }
      const count = Number(category?.count || category?.records?.length || 0);
      if (count > 0) {
        return language === 'zh'
          ? `SNMP 光模块采集完成，读取到 ${count} 条接口数据。`
          : `SNMP optical collection completed with ${count} interface records.`;
      }
      return language === 'zh'
        ? 'SNMP 查询已完成，但设备没有返回可用的光模块诊断数据。'
        : 'SNMP query completed, but the device returned no usable optical diagnostics.';
    }
  ...
  const handleResetQuickQuery = useCallback(() => {
    quickQueryRunIdRef.current += 1;
    quickQueryAbortRef.current?.abort();
    quickQueryAbortRef.current = null;
  ...
  const handleClearSelection = useCallback(() => {
    ...
    setBatchDeviceIds([]);
  }, [setSelectedDevice, setBatchDeviceIds]);
  ...
  const quickDetectedPlatforms = useMemo(() => {
    return Array.from(new Set(quickTargetDevices.map((d) => normalizeAutomationPlatform(d.platform)).filter(Boolean)));
  }, [quickTargetDevices]);
  ...
  const quickScenarioSupportsDetectedPlatform = useMemo(() => {
    return !!(
      ...
      isAutomationPlatformSupported(
        quickDetectedPlatform,
        quickPlaybookScenario.supported_platforms || [],
      )
    );
  ...
  const loadScenarios = useCallback(async () => {
    try {
      const [scenResp, platResp] = await Promise.all([
        fetch('/api/playbooks/scenarios', { headers: authHeaders() }),
        fetch('/api/playbooks/platforms', { headers: authHeaders() }),
      ]);
  ...
  const loadPlaybookHistory = useCallback(async (page = 1, statusFilter = 'all', scenarioFilter = '') => {
    try {
      ...
      const resp = await fetch(`/api/playbooks?${params}`, { headers: authHeaders() });
      if (resp.ok) {
  ...
  const loadJobs = useCallback(async () => {
    if (!isAuthenticated || !localStorage.getItem('netops_token')) return;
    try {
      const resp = await fetch('/api/playbooks?page=1&page_size=50&status=all', { headers: authHeaders() });
      if (!resp.ok) return;
      ...
      const mapped: Job[] = items.map((row) => ({
        ...
        // Status values from playbook engine: success | failed | partial | running | pending | awaiting_approval | approval_rejected.
  ...
  useEffect(() => {
    if (activeTab === 'automation') {
      loadScenarios();
      if (automationPage === 'execute' || automationPage === 'scenarios' || automationPage === 'playbooks' || automationPage === 'history') {
        loadPlaybookHistory();
      }
    }
  ...
  const runParsedCustomCommand = useCallback(async (label: string, command: string) => {
    ...
    quickQueryAbortRef.current?.abort();
    const requestId = ++quickQueryRunIdRef.current;
    const controller = new AbortController();
    quickQueryAbortRef.current = controller;
    const timeoutId = window.setTimeout(() => controller.abort(), QUICK_QUERY_TIMEOUT_MS);
    try {
      ...
      if (requestId !== quickQueryRunIdRef.current || controller.signal.aborted) return false;
      const data = await resp.json();
      if (!resp.ok) {
      ...
      if (requestId !== quickQueryRunIdRef.current) return false;
      if (controller.signal.aborted) {
        showToast(
          language === 'zh' ? '查询超时，已停止等待设备响应' : 'Query timed out while waiting for the device',
          'error',
        );
        setQuickQueryOutput(language === 'zh' ? '查询超时，设备未在规定时间内返回响应。' : 'Query timed out before the device responded.');
        return false;
      }
      ...
      if (requestId === quickQueryRunIdRef.current) {
        quickQueryAbortRef.current = null;
        setQuickQueryRunning(false);
      }
    }
  ...
  const exportQuickQueryTable = useCallback((commands: string[]) => {
    ...
    const metaRows = [
      ...
      { key: language === 'zh' ? '解析器' : 'Parser', value: table.category?.parser || 'platform-parser' },
  ...
  const runQuickQuery = useCallback(async (label: string, commands: string, operationalCategory?: string, authRole: string = 'normal') => {
    ...
    try {
      const categoryKey = operationalCategory || 'custom_command';
      const results = await Promise.all(
        targets.map(async (device) => {
          try {
            const isCustom = categoryKey === 'custom_command';
            const url = isCustom
              ? `/api/devices/${device.id}/parsed-command`
              : `/api/devices/${device.id}/operational-data`;
      ...
      // The user may have closed the overlay or started another query while
      // the previous requests were still settling. Do not let stale results
      // reopen the overlay or overwrite the newer query state.
      if (requestId !== quickQueryRunIdRef.current) return;
      if (controller.signal.aborted) {
        setQuickQueryOutput(language === 'zh' ? '查询超时，设备未在规定时间内返回响应。' : 'Query timed out before the device responded.');
        showToast(
          language === 'zh' ? '查询超时，已停止等待设备响应' : 'Query timed out while waiting for the device',
      ...
      let backendParseStatus = 'unmatched';
      let backendParser = '';
      let backendSource = '';
      let backendAdapter: any = null;
      const backendErrors: Array<{ code?: string; message: string }> = [];
      ...
      results.forEach((res) => {
        ...
        if (firstCat) {
          if (firstCat.parser) {
            backendParser = String(firstCat.parser);
          }
          if (firstCat.parse_status) {
            backendParseStatus = firstCat.parse_status;
          }
          if (firstCat.source) {
            backendSource = String(firstCat.source);
          }
          if (firstCat.adapter) {
            backendAdapter = firstCat.adapter;
          }
          if (firstCat.error || firstCat.error_code) {
            backendErrors.push({
              code: firstCat.error_code ? String(firstCat.error_code) : undefined,
              message: String(firstCat.error || ''),
            });
          }
          if (Array.isArray(firstCat.commands) && firstCat.commands.length > 0) {
            backendCmds = [...backendCmds, ...firstCat.commands];
          }
          if (Array.isArray(firstCat.records)) {
      ...
      if (backendCmds.length > 0) {
        setQuickQueryCommands([...new Set(backendCmds)]);
      }

      const hasStructuredRecords = combinedRecords.length > 0;
      const mockStructured = {
        device: {
          hostname: targets.length === 1 ? targets[0].hostname : (language === 'zh' ? '多选聚合' : 'Aggregate'),
          platform: targets.length === 1 ? targets[0].platform : 'mixed'
        },
        categories: [
          {
            name: categoryKey,
            parser: backendParser || (operationalCategory ? 'platform-registry' : 'platform-parser'),
            count: combinedRecords.length,
            records: combinedRecords,
            parse_status: backendParseStatus,
            source: backendSource || undefined,
            adapter: backendAdapter || undefined,
            errors: backendErrors,
            error_code: backendErrors[0]?.code,
            error: backendErrors[0]?.message || undefined,
          },
        ],
      };
  ...
  return {
    ...
    runQuickPlaybook, handleClearSelection,
