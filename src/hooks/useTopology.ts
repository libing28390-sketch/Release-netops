...
const toUnmanagedTopologyDevice = (node: any): Device => ({
  id: String(node?.id || '').trim(),
  hostname: String(node?.hostname || 'Unknown neighbor'),
  ...
  version: '',
  role: 'external',
  site: String(node?.site || node?.site_name || 'Unassigned'),
  site_id: String(node?.site_id || ''),
  site_name: String(node?.site_name || node?.site || 'Unassigned'),
  uptime: '',
  ...
  connected_device_ids: Array.isArray(node?.connected_device_ids) ? node.connected_device_ids : [],
} as Device);

const normalizeUnmanagedTopologyLink = (link: any) => ({
  ...link,
  id: String(link?.link_key || ''),
  link_key: String(link?.link_key || ''),
  relation_type: 'PHYSICAL',
  semantic_relation: 'PHYSICAL',
  discovery_source: String(link?.discovery_source || 'lldp'),
  status: 'unknown',
  is_unmanaged: true,
  is_inferred: false,
  inferred: false,
  evidence_count: Number(link?.evidence_count || 1),
...
export const useTopology = ({ devices, language, showToast, isAuthenticated, enabled, refreshDevices }: UseTopologyArgs) => {
  const [topologyLinks, setTopologyLinks] = useState<any[]>([]);
  const [topologyUnmanagedNodes, setTopologyUnmanagedNodes] = useState<Device[]>([]);
  ...
  const [hostnameSyncing, setHostnameSyncing] = useState(false);
  ...
  const authExpiredSignaledRef = React.useRef(false);
  // Keep the latest inventory available to the refresh callback without
  // making the callback/effect restart every time background polling updates
  // one device field.
  const devicesRef = React.useRef(devices);
  devicesRef.current = devices;

  const refreshTopologyData = useCallback(async () => {
    // App renders hooks before the login screen, so this hook must not issue
    // anonymous requests while authentication is still being resolved (or
    // after a cross-tab logout).  Otherwise the 401 response can cause a
    // request/remount loop in the shell.
    const headers = topologyAuthHeaders();
    if (!enabled || !isAuthenticated || !headers.Authorization) return;
    ...
    const [linksResponse, sitesResponse, graphResponse] = await Promise.all([
      fetch(`/api/topology/links?${params.toString()}`, { headers }),
      fetch('/api/topology/sites', { headers }),
      fetch(`/api/topology/graph?${graphParams.toString()}`, { headers }),
    ]);
    if ([linksResponse, sitesResponse, graphResponse].some((response) => response.status === 401)) {
      if (!authExpiredSignaledRef.current) {
        authExpiredSignaledRef.current = true;
        window.dispatchEvent(new Event('netops:auth-expired'));
      }
      return;
    }
    authExpiredSignaledRef.current = false;
    ...
    const unmanagedNodes = (Array.isArray(linksPayload?.unmanaged_nodes) ? linksPayload.unmanaged_nodes : [])
      .map(toUnmanagedTopologyDevice)
      .filter((node: Device) => Boolean(node.id));
    const unmanagedLinks = (Array.isArray(linksPayload?.unmanaged_links) ? linksPayload.unmanaged_links : [])
      .map(normalizeUnmanagedTopologyLink)
      .filter((link: any) => Boolean(link.source_device_id && link.target_device_id));
    setTopologyUnmanagedNodes(unmanagedNodes);
    setTopologyUnmanagedLinks(unmanagedLinks);
    const externalLinksForView = topologyGraphView === 'external'
      || topologyGraphView === 'physical'
      || topologyGraphView === 'all'
      ? unmanagedLinks
      : [];
    let nextLinks = [...legacyLinks, ...externalLinksForView];
    ...
    if (graphResponse.ok) {
      ...
      if (topologyGraphView === 'physical') {
        nextLinks = [...(foldedPhysicalLinks.length > 0 ? foldedPhysicalLinks : graphLinks), ...externalLinksForView];
        ...
        nextLinks = [...foldedPhysicalLinks, ...logicalLinks, ...externalLinksForView];
      } else if (topologyGraphView === 'external') {
        nextLinks = externalLinksForView;
      } else {
    ...
    if (refreshDevices) {
      const currentDeviceIds = new Set(devicesRef.current.map((device) => String(device.id)));
  ...
  }, [enabled, hideStaleLinks, isAuthenticated, refreshDevices, topologyGraphView, topologySiteFilter]);

  React.useEffect(() => {
    if (!enabled || !isAuthenticated || !topologyAuthHeaders().Authorization) {
      setTopologyLinks([]);
      setTopologyUnmanagedNodes([]);
      setTopologyUnmanagedLinks([]);
      setTopologySites([]);
      setTopologyNodeMetadata({});
      setTopologyGraphNodeDevices([]);
      setTopologyDataError('');
      return;
    }
  ...
  }, [enabled, isAuthenticated, refreshTopologyData]);
  ...
  const describeTopologyLink = useCallback(
    (link: any, sourceDevice?: Device, targetDevice?: Device): TopologyDecoratedLink => {
      ...
      const isUnmanaged = Boolean(link.is_unmanaged || sourceDevice?.is_unmanaged || targetDevice?.is_unmanaged);
      ...
      if (isUnmanaged) {
        ...
        operationalSummary = language === 'zh'
          ? '未纳管邻居：已发现但尚未匹配到 CMDB 设备。'
          : 'Unmanaged neighbor: discovered but not matched to a CMDB device.';
      } else if (inferred) {
        operationalState = 'unknown';
        operationalSummary = language === 'zh'
          ? '这是推断链路，需要等待真实邻居证据确认。'
  ...
  const graphInventory = useMemo(() => {
    ...
    topologyGraphNodeDevices.forEach((device) => existingIds.add(String(device.id)));
    return [
      ...devices,
      ...topologyGraphNodeDevices.filter((device) => !devices.some((item) => String(item.id) === String(device.id))),
      ...topologyUnmanagedNodes.filter((device) => !existingIds.has(String(device.id))),
    ];
  }, [devices, topologyGraphNodeDevices, topologyUnmanagedNodes]);
  ...
  const topologyStats = useMemo(() => ({
    ...
    managedLinkCount: topologyVisibleLinks.filter((link) => !link.is_unmanaged).length,
    externalLinkCount: topologyVisibleLinks.filter((link) => Boolean(link.is_unmanaged)).length,
  ...
  }), [topologyConnectedDeviceIds, topologyFilteredDevices, topologyVisibleLinks]);
  ...
  const handleTriggerDiscovery = useCallback(async () => {
    if (hostnameSyncing || topologyDiscoveryRunning) return;

    setHostnameSyncing(true);
    let hostnameSyncFailureCount = 0;
    let hostnameSyncUpdatedCount = 0;
    let hostnameSyncUnavailable = false;
    try {
      const syncResponse = await fetch('/api/devices/batch-sync-hostname', {
        method: 'POST',
        headers: {
          ...topologyAuthHeaders(),
      ...
      if (!syncResponse.ok) {
        throw new Error(topologyApiErrorMessage(syncPayload, 'topology_hostname_sync_failed'));
      }

      hostnameSyncFailureCount = Number(syncPayload?.failed || 0);
      hostnameSyncUpdatedCount = Number(syncPayload?.updated || 0);
    } catch {
      hostnameSyncUnavailable = true;
    } finally {
      setHostnameSyncing(false);
    }
    ...
    try {
      ...
      const syncStatusMessage = hostnameSyncUnavailable
        ? (language === 'zh' ? '主机名同步暂不可用，' : 'Hostname sync was unavailable; ')
        : hostnameSyncFailureCount > 0
          ? (language === 'zh'
            ? `${hostnameSyncFailureCount} 台设备主机名未能同步，`
            : `Hostname sync did not complete for ${hostnameSyncFailureCount} devices; `)
          : hostnameSyncUpdatedCount > 0
            ? (language === 'zh' ? `已同步 ${hostnameSyncUpdatedCount} 台设备名称，` : `Synced ${hostnameSyncUpdatedCount} device names; `)
            : '';
      showToast(
        language === 'zh'
          ? `${syncStatusMessage}拓扑发现任务已启动，可实时查看进度。`
          : `${syncStatusMessage}topology discovery started; live progress is available.`,
        hostnameSyncUnavailable || hostnameSyncFailureCount > 0 ? 'info' : 'success',
      );
    } catch (error) {
  ...
  }, [hostnameSyncing, language, showToast, topologyDiscoveryRunning, topologySiteFilter]);
