...
export const useMonitoring = ({ isAuthenticated, activeTab, pollOutbound = true, healthHistoryRange = 24 }: UseMonitoringProps) => {
  ...
  const fetchHostResources = useCallback(async (signal?: AbortSignal) => {
    try {
      const resp = await fetch('/api/health/resources', { signal, headers: authHeaders() });
      if (!resp.ok) return;
      const payload = await resp.json();
  ...
  const fetchMonitoringHealthDevices = useCallback(async (filters: {
    ...
    const { limit = 200, offset = 0, ...criteria } = filters;
    for (const [key, value] of Object.entries(criteria)) {
      if (value && value !== 'all') params.set(key, value);
    }
    const boundedLimit = Number.isFinite(limit) ? Math.trunc(limit) : 200;
    const safeOffset = Number.isFinite(offset) ? Math.trunc(offset) : 0;
    params.set('limit', String(Math.min(500, Math.max(1, boundedLimit))));
    params.set('offset', String(Math.max(0, safeOffset)));
    ...
    if (!resp.ok) {
      ...
      const error = new Error(detail ? String(detail) : 'Unable to load health drilldown') as Error & { status: number };
  ...
  const fetchMonitoringRealtime = useCallback(async (deviceId: string, signal?: AbortSignal) => {
    const reqEpoch = monitorRequestEpochRef.current;
    const resp = await fetch(`/api/monitoring/device/${deviceId}/realtime?window_minutes=15&limit=1000`, { signal, headers: authHeaders() });
  ...
  const fetchMonitoringTrend = useCallback(async (
    ...
    const resp = await fetch(`/api/monitoring/device/${deviceId}/trend?${params.toString()}`, { signal, headers: authHeaders() });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        const resp = await fetch(`/api/monitoring/search-devices?q=${encodeURIComponent(q)}&limit=20`, { signal: searchController.signal, headers: authHeaders() });
        if (!resp.ok) return;
        const data = await resp.json();
        if (!cancelled) {
