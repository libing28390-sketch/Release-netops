...
export const useDeviceConnectionAndSnmp = ({
    ...
    try {
      const response = await fetch('/api/devices/connect', {
        ...
        body: JSON.stringify({
          ...
          check_mode: mode,
          management_port: deviceToTest.management_port,
        }),
      });
      ...
      if (response.ok) {
        ...
        setTestResult({
          success: false,
          message: buildConnectionTestMessage(data.detail || data.error, data.error_code, deviceToTest.management_port || 22),
          output: data.output,
  ...
  const handleSnmpTest = useCallback(async (deviceId: string) => {
    ...
    try {
      const resp = await fetch(`/api/devices/${deviceId}/snmp-test`, { method: 'POST', headers: authHeaders() });
