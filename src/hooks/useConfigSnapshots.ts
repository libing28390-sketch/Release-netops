...
import { getDefaultConfigBackupCommands } from '../utils/configBackupCommands';

const MOCK_IPS = ['127.0.0.1', '0.0.0.0', 'localhost'];
const NETWORK_PLATFORM_KEYWORDS = [
  'cisco', 'arista', 'rgos', 'ruijie', 'juniper', 'junos', 'huawei', 'vrp', 'h3c', 'comware',
  'zte', 'zxros', 'raisecom', 'maipu', 'mypower', 'dptech', 'conplat', 'fortinet', 'fortios',
];

const isNetworkDevice = (platform?: string, vendor?: string) => {
  const identity = `${platform || ''} ${vendor || ''}`.toLowerCase();
  return NETWORK_PLATFORM_KEYWORDS.some((kw) => identity.includes(kw));
};
...
export const useConfigSnapshots = ({
  ...
  const retentionDays = useConfigStore((s) => s.retentionDays);
  const setRetentionDays = useConfigStore((s) => s.setRetentionDays);
  const retentionMaxPerDevice = useConfigStore((s) => s.retentionMaxPerDevice);
  const setRetentionMaxPerDevice = useConfigStore((s) => s.setRetentionMaxPerDevice);
  ...
  const loadSnapshotContent = useCallback(async (snap: ConfigSnapshot): Promise<string> => {
    ...
    try {
      const resp = await fetch(`/api/configs/snapshots/${snap.id}/content`, { headers: authHeaders() });
      if (resp.ok) {
  ...
  const loadScheduleConfig = useCallback(async () => {
    try {
      const resp = await fetch('/api/configs/schedule', { headers: authHeaders() });
      if (resp.ok) {
  ...
  const loadRetentionConfig = useCallback(async () => {
    try {
      const resp = await fetch('/api/configs/retention', { headers: authHeaders() });
      if (resp.ok) {
        const cfg = await resp.json();
        setRetentionDays(Number(cfg.max_days) || 90);
        setRetentionMaxPerDevice(Number(cfg.max_per_device) || 30);
      }
    } catch {
      // ignore
    }
  }, [setRetentionDays, setRetentionMaxPerDevice]);
  ...
  }, [isAuthenticated, loadScheduleConfig, loadRetentionConfig]);
  ...
  const takeConfigSnapshot = useCallback(async (
    ...
    if (!isNetworkDevice(device.platform, device.vendor)) {
      showToast(
        language === 'zh'
    ...
    try {
      ...
      try {
        const backupCommands = getDefaultConfigBackupCommands(device.vendor, device.platform);
        if (!backupCommands) {
          showToast(
            language === 'zh'
              ? `${device.hostname} 的厂商尚无默认配置备份命令`
              : `No default config backup command is mapped for ${device.hostname}`,
            'error',
          );
          return null;
        }
        const execResp = await fetch('/api/execute', {
          ...
          body: JSON.stringify({
            device_id: device.id,
            command: backupCommands.running,
            isConfig: false,
  ...
  const saveScheduleConfig = useCallback(async () => {
    ...
    try {
      ...
      const retentionResp = await fetch('/api/configs/retention', {
        method: 'PUT',
        headers: authHeaders(true),
        body: JSON.stringify({
          max_days: Math.max(1, Math.round(retentionDays)),
          max_per_device: Math.max(1, Math.round(retentionMaxPerDevice)),
        }),
      });
      if (!retentionResp.ok) {
        const err = await retentionResp.json().catch(() => ({}));
        showToast(err.detail || 'Invalid retention policy', 'error');
        return;
      }
  ...
  }, [scheduleEnabled, scheduleCron, retentionDays, retentionMaxPerDevice, setScheduleLoading, showToast, t]);
  ...
  const handleRunBackupAllOnline = useCallback(async () => {
    const online = devices.filter(
      (device) =>
        ...
        && isNetworkDevice(device.platform, device.vendor),
    );
    if (online.length === 0) {
      showToast(
        language === 'zh'
