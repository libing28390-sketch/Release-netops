...
export const useCommandPalette = ({ language, t }: UseCommandPaletteArgs): UseCommandPaletteResult => {
  ...
  const allNavItems = useMemo<CommandPaletteItem[]>(() => {
    ...
    return [
      ...
      { path: '/access/workspace', label: zh ? '操作工作台' : 'Operation Workspace', group: zh ? '终端接入' : 'Access' },
      { path: '/access/favorites', label: zh ? '我的收藏' : 'My Favorites', group: zh ? '终端接入' : 'Access' },
      { path: '/access/history', label: zh ? '我的历史记录' : 'My History', group: zh ? '终端接入' : 'Access' },
      { path: '/access/pam-audit', label: zh ? '受控审计' : 'PAM Audit', group: zh ? '终端接入' : 'Access' },
      ...
      { path: '/ipam/locate', label: zh ? 'IP/VLAN 管理' : 'IP/VLAN Mgmt', group: zh ? '设备清单' : 'Inventory' },
      ...
      { path: '/cmdb/resources', label: zh ? '资源检索' : 'Resource Search', group: zh ? 'CMDB' : 'CMDB' },
