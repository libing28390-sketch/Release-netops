...
import { Plus, Server, CheckCircle, CheckCircle2, XCircle, RotateCcw, Play, Activity, LayoutDashboard, Database, Zap, ShieldCheck, History, LogOut, Search, Bell, Settings, Upload, FileText, ChevronLeft, ChevronRight, Filter, Globe, TrendingUp, PieChart as PieChartIcon, Clock, AlertCircle, FolderOpen, Eye, EyeOff, Sun, Moon, User as UserIcon, ChevronDown, Menu, PanelLeftClose, Monitor, ExternalLink, Wrench, Maximize2, Minimize2, BarChart3, GitCompareArrows, Cpu, Network, Pin, AlertTriangle, X } from 'lucide-react';
...
const TerminalWindow = lazy(() => import('./components/access/TerminalWindow'));
const AppModals = lazy(() => import('./components/modals/AppModals').then(({ AppModals: Component }) => ({ default: Component })));
...
const App: React.FC = () => {
  ...
  const {
    ...
    retentionMaxPerDevice, setRetentionMaxPerDevice,
  ...
  const deviceFetchMode = useMemo<'light' | 'full' | 'paged-light'>(() => {
    if (activeTab === 'network-monitoring') return 'paged-light';
  ...
  } = useAutomation({ currentUser, isAuthenticated, language, showToast, navigate, setSelectedDevice, devices, selectedDevice, activeTab, automationPage });
  useEscapeClose(saveQuickQueryModalOpen, () => setSaveQuickQueryModalOpen(false));
  ...
  const loadExecDevices = useCallback(async (executionId: string, page = 1, statusFilter = 'all', search = '') => {
    ...
    try {
      ...
      const resp = await fetch(`/api/playbooks/${executionId}/devices?${params}`, { headers: authHeaders() });
      if (resp.ok) {
  ...
  const {
    ...
    showToast,
    isAuthenticated,
    enabled: isAuthenticated && activeTab === 'topology',
    refreshDevices: fetchDevicesData,
  });
  ...
  React.useEffect(() => {
    ...
    const redirects: Record<string, string> = {
      ...
      '/automation': '/automation/quick-ops',
      '/automation/tasks': '/automation/quick-ops',
      '/automation/execute': '/automation/quick-ops',
      ...
      '/inventory/devices': '/assets/network-devices',
      '/assets/devices': '/assets/network-devices',
      '/inventory/servers': '/assets/servers',
      '/ip-locator': '/ipam/locate',
      '/assets/toolbox': '/ipam/locate',
    ...
    if (p === '/ipam') {
      navigate('/ipam/locate', { replace: true });
    } else if (p === '/cmdb') {
  ...
  useEffect(() => {
    ...
    // ``isAuthenticated`` can briefly lead localStorage during startup,
    // cross-tab logout, or token rotation. Never issue an anonymous request
    // to the administrator-only users endpoint in that window.
    if (!token) return;
  ...
  useEffect(() => {
    ...
    fetchSharedLookups();
    fetchHotData();
    if (activeTab !== 'network-monitoring' && !(activeTab === 'inventory' && inventorySubPage === 'devices')) {
      fetchDevicesData();
    }
  ...
  const {
    ...
    hostResources,
    outboundHealth, outboundLoading, outboundTargetHistory, outboundTargetHistoryLoading, outboundHistoryHours, setOutboundHistoryHours,
    fetchOutboundHealth, fetchOutboundTargetHistory, triggerOutboundProbe, saveOutboundTarget, deleteOutboundTarget,
  ...
  const handleSelectExecution = async (execution: any) => {
    ...
    try {
      const response = await fetch(`/api/playbooks/${execution.id}/summary`, { headers: authHeaders() });
      if (response.ok) {
  ...
  const handleDeleteExecution = async (execution: any) => {
    ...
    try {
      const response = await fetch(`/api/playbooks/${execution.id}`, { method: 'DELETE', headers: authHeaders() });
      if (!response.ok) {
  ...
  const handleSelectExecDevice = async (deviceId: string) => {
    ...
    setSelectedDeviceDetail(null);
    try {
      const response = await fetch(`/api/playbooks/${activeExecutionId}/devices/${deviceId}`, { headers: authHeaders() });
      if (response.ok) {
  ...
  if (activeTab === 'terminal') {
    ...
    return (
      <Suspense fallback={<div className="flex h-screen w-screen items-center justify-center bg-black text-sm text-white/60">Loading terminal…</div>}>
  ...
  const configValue = {
    ...
    retentionDays, setRetentionDays, retentionMaxPerDevice, setRetentionMaxPerDevice,
    saveScheduleConfig, configSnapshots,
    ...
    diffOnlyChanges, diffShowFullBoth, diffMode, renderedDiffLines, fullSideBySideRows, diffChangeBlocks, filteredDiffChangeBlocks,
    ...
    setDiffOnlyChanges, setDiffShowFullBoth, setDiffMode, setDiffBlockQuery,
  ...
  const topologyValue = {
    ...
    topologyDiscoveryProgress, topologyDiscoveryDevices, topologySites, topologyDataError,
    topologyStats, topologyLinkStats,
    topologySearch, topologySiteFilter, topologyGraphView, topologyTagFilter, topologyRoleFilter, topologyStatusFilter,
    topologyLinkStatusFilter, topologyProtocolFilter,
    topologySiteOptions, topologyTagOptions, topologyTagCandidateDevices, topologyRoleOptions, topologyVisibleDevices, topologyVisibleLinks,
    ...
    hideStaleLinks, setHideStaleLinks, hideOrphanDevices, setHideOrphanDevices,
     handleTriggerDiscovery, handleCancelDiscovery, hostnameSyncing, handleExportMap,
    handleConfirmTopologyRelation, loadTopologyHistory,
    setTopologySearch, setTopologySiteFilter, setTopologyGraphView, setTopologyTagFilter, setTopologyRoleFilter, setTopologyStatusFilter,
    setTopologyLinkStatusFilter, setTopologyProtocolFilter,
    ...
    <AppRuntimeBoundary language={language}>
      ...
      <Suspense fallback={null}>
        <AppModals
          ...
          setSelectedDevice, setViewingDevice, setShowDetailsModal,
