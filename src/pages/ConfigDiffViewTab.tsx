...
const ConfigDiffViewTab: React.FC<ConfigDiffViewTabProps> = ({
  ...
  const loadDevices = useCallback(async (q = '', pg = devPage, ps = devPageSize, branch = deviceBranch) => {
    ...
    try {
      ...
      const resp = await fetch(`/api/configs/devices-with-backups?${params}`, { headers: authHeaders(), signal: controller.signal });
      if (resp.ok) {
                  ...
                  <div className="flex-1 min-h-0 flex flex-col rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden">
                    ...
                    {devicesTotal > 0 && (
                      <div className="border-t border-slate-100 bg-white px-3 py-2 shrink-0">
                        <Pagination currentPage={devPage} totalItems={devicesTotal} itemsPerPage={devPageSize} onPageChange={setDevPage} onItemsPerPageChange={(v) => { setDevPage(1); setDevPageSize(v); }} language={language} alwaysVisible itemLabel={zh ? '台设备' : 'devices'} />
                      </div>
                    )}
                  </div>
                </div>
