...
export const EMPTY_FORM = {
  ...
  platform: '', connection_method: 'ssh', ssh_algorithm_profile: 'auto',
  username: '', password: '',
  ...
  winrm_enabled: false,
  winrm_port: '5986',
  winrm_auth_mode: 'ntlm',
  winrm_tls_mode: 'pin_fingerprint',
...
export const VENDOR_PLATFORMS: Record<string, { value: string; label: string }[]> = {
  ...
  // Keep one canonical H3C platform. V3/V5/V7/V9 labels are accepted as
  // legacy import aliases and normalized to h3c_comware below.
  H3C:        [{ value: 'h3c_comware', label: 'H3C Comware' }],
  ...
  // `ruijie_os` is a legacy alias of the same RGOS family. Keep one
  // user-facing selection while retaining the alias in the import map below.
...
// These values are intentionally kept separate from the network vendor
// catalogue.  A few names (for example Huawei and Dell) can be valid in both
// domains, so the asset type remains the source of truth when filtering or
// validating an import row.
export const SERVER_ONLY_VENDOR_VALUES = [
  'Linux', 'Dell', 'HP', 'Lenovo', 'Inspur', 'Generic Server', 'generic',
] as const;

export const SSH_ALGORITHM_PROFILE_OPTIONS = [
  { value: 'auto', label: { zh: '自动（现代优先，兼容安全旧算法）', en: 'Auto (modern first, safe legacy fallback)' } },
  { value: 'modern', label: { zh: '现代算法（严格）', en: 'Modern algorithms (strict)' } },
  { value: 'legacy_compat', label: { zh: '旧算法兼容（受控）', en: 'Legacy compatibility (controlled)' } },
  { value: 'legacy_break_glass', label: { zh: '极旧设备紧急兼容（高风险）', en: 'Legacy break-glass (high risk)' } },
] as const;

export const SSH_ALGORITHM_PROFILE_VALUES = SSH_ALGORITHM_PROFILE_OPTIONS.map(option => option.value);
...
export const getPlatformsForVendor = (vendor?: string) => {
  // This helper is used by the network-device editor.  Never expose server
  // platforms from the empty/unknown-vendor state.
  if (!vendor) return NETWORK_PLATFORM_PLATFORMS;
  const rawVendor = vendor.trim();
  const canonicalVendor = VENDOR_IMPORT_ALIASES[rawVendor]
    || Object.entries(VENDOR_IMPORT_ALIASES).find(([alias]) => alias.toLowerCase() === rawVendor.toLowerCase())?.[1]
    || rawVendor;
  const v = canonicalVendor.toLowerCase();
  ...
  return NETWORK_PLATFORM_PLATFORMS;
};
...
export const COL_MAP: Record<string, string> = {
  ...
  'SSH算法策略': 'ssh_algorithm_profile', 'SSH 算法策略': 'ssh_algorithm_profile', 'SSH算法配置': 'ssh_algorithm_profile',
  'SSH Algorithm Profile': 'ssh_algorithm_profile', 'SSH Algorithm Policy': 'ssh_algorithm_profile', 'ssh_algorithm_profile': 'ssh_algorithm_profile',
  'SNMP社区名': 'snmp_community', 'SNMP Community': 'snmp_community', 'SNMP Community String': 'snmp_community', 'SNMP Community Name': 'snmp_community',
  '社区名': 'snmp_community', '团体字': 'snmp_community', '团体字符串': 'snmp_community', 'SNMP团体字': 'snmp_community', 'SNMP 团体字': 'snmp_community', 'SNMP团体字符串': 'snmp_community', 'SNMP 团体字符串': 'snmp_community',
  Community: 'snmp_community', community: 'snmp_community', 'Community String': 'snmp_community', 'Community Name': 'snmp_community', 'snmp_community': 'snmp_community',
  'SNMP端口': 'snmp_port', 'SNMP Port': 'snmp_port', 'snmp_port': 'snmp_port',
  // SNMP credentials are referenced by vault name (or ID for compatibility),
  // never by a community string in the template. Keep generic Credential
  // mapped to SSH above so existing imports remain unambiguous.
  'SNMP凭据': 'snmp_credential_name', 'SNMP凭据名称': 'snmp_credential_name', 'SNMP凭据（推荐）': 'snmp_credential_name', 'SNMPv2凭据名称': 'snmp_credential_name', 'SNMPv2凭据名称（选填）': 'snmp_credential_name', '团体字凭据': 'snmp_credential_name', 'SNMP团体凭据': 'snmp_credential_name', 'community的凭据': 'snmp_credential_name', 'Community凭据': 'snmp_credential_name',
  'SNMP Credential': 'snmp_credential_name', 'SNMP Credential Name': 'snmp_credential_name', 'SNMP Credential (recommended)': 'snmp_credential_name', 'SNMPv2 Credential Name': 'snmp_credential_name', 'SNMPv2 Credential Name (optional)': 'snmp_credential_name', 'Community Credential': 'snmp_credential_name', 'Community Credential Name': 'snmp_credential_name',
  'snmp_credential_name': 'snmp_credential_name',
  'SNMP凭据ID': 'snmp_credential_id', 'SNMP Credential ID': 'snmp_credential_id',
  ...
  '绑定凭据': 'credential_id', '绑定普通用户凭据': 'credential_id', '绑定普通用户凭据（推荐）': 'credential_id', '普通用户凭据': 'credential_id', '凭据名称': 'credential_id', '凭据ID': 'credential_id', 'Credential': 'credential_id', 'Normal Credential': 'credential_id', 'Normal Credential ID': 'credential_id', 'credential_id': 'credential_id',
...
export const IMPORT_VALUE_MAP: Record<string, Record<string, string>> = {
  ...
  connection_method: {
    ...
    'WinRM HTTPS': 'winrm', WinRM: 'winrm', winrm: 'winrm',
    '仅 Web（HTTP/HTTPS）': 'web', 'Web管理': 'web', 'Web管理入口': 'web', 'Web管理（HTTP/HTTPS）': 'web', 'Web Management': 'web', 'Web only': 'web', web: 'web',
    '无登录通道': 'none', 'No login channel': 'none', none: 'none',
  },
  ssh_algorithm_profile: {
    auto: 'auto', '自动': 'auto', '默认': 'auto', 'Auto': 'auto', 'Default': 'auto',
    modern: 'modern', '现代': 'modern', '严格': 'modern', 'Modern': 'modern', 'Strict': 'modern',
    legacy_compat: 'legacy_compat', '兼容旧算法': 'legacy_compat', '旧算法兼容': 'legacy_compat', 'Legacy Compatible': 'legacy_compat', 'Legacy Compat': 'legacy_compat',
    legacy_break_glass: 'legacy_break_glass', '极旧设备兼容': 'legacy_break_glass', '紧急兼容': 'legacy_break_glass', 'Break Glass': 'legacy_break_glass', 'Legacy Break Glass': 'legacy_break_glass',
  },
  platform: {
    '思科 IOS': 'cisco_ios', '思科 NX-OS': 'cisco_nxos', '思科 IOS-XE': 'cisco_xe',
    '华为 VRPv5': 'huawei_vrp', '华为 VRPv8': 'huawei_vrpv8',
    '华三 Comware V3': 'h3c_comware', '华三 Comware V5': 'h3c_comware', '华三 Comware V7': 'h3c_comware', '华三 Comware V9': 'h3c_comware', 'hp_comware': 'h3c_comware', 'h3c_comware_v3': 'h3c_comware',
    '阿里斯塔 EOS': 'arista_eos', '瞻博 JunOS': 'juniper_junos', '锐捷 RGOS': 'ruijie_rgos', '锐捷 OS': 'ruijie_rgos', 'Ruijie OS': 'ruijie_rgos', 'ruijie_os': 'ruijie_rgos',
    ...
    'H3C Comware': 'h3c_comware', 'H3C Comware V3': 'h3c_comware', 'H3C Comware V5': 'h3c_comware', 'H3C Comware V7': 'h3c_comware', 'H3C Comware V9': 'h3c_comware', 'h3c_comware9': 'h3c_comware',
...
export const normalizeImportedVendor = (vendor: string): string => {
  const trimmed = vendor.trim();
  if (!trimmed) return '';
  const lower = trimmed.toLowerCase();
  const canonicalName = [...NETWORK_VENDOR_NAMES, ...SECURITY_VENDOR_NAMES]
    .find(name => name.toLowerCase() === lower);
  if (canonicalName) return canonicalName;
  const alias = Object.entries(VENDOR_IMPORT_ALIASES)
    .find(([value]) => value.toLowerCase() === lower);
  return alias?.[1] || trimmed;
};

export const isNetworkVendor = (vendor: string): boolean => {
  const normalized = normalizeImportedVendor(vendor).toLowerCase();
  return [...NETWORK_VENDOR_NAMES, ...SECURITY_VENDOR_NAMES]
    .some(name => name.toLowerCase() === normalized);
};

export const getValidPlatformsForAsset = (assetType: string, vendor?: string) => {
  if (assetType === 'server') return SERVER_PLATFORMS;
  return getPlatformsForVendor(vendor);
};

export const isValidVendorPlatform = (assetType: string, vendor: string, platform: string): boolean => {
  const normalizedPlatform = platform.trim();
  if (!normalizedPlatform) return true;
  const allowed = getValidPlatformsForAsset(assetType, vendor);
  return allowed.some(option => option.value === normalizedPlatform);
};
