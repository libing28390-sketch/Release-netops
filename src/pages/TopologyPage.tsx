...
import { Activity, AlertTriangle, ArrowRight, Clock3, Database, Download, EyeOff, GitMerge, Globe, Info, LayoutGrid, MapPin, Network, RefreshCw, RotateCcw, Search, Tag, X } from 'lucide-react';
...
import { excludeServerTopologyDevices } from '../utils/topologyDevices';
...
const TopologySiteOverview: React.FC<TopologySiteOverviewProps> = ({ language, sites, devices, links, onSelectSite }) => {
  ...
  const [sitePageSize, setSitePageSize] = React.useState(20);
  const siteTotalPages = Math.max(1, Math.ceil(overview.sites.length / sitePageSize));
  const pagedSites = overview.sites.slice((sitePage - 1) * sitePageSize, sitePage * sitePageSize);
  React.useEffect(() => { setSitePage(1); }, [overview.sites.length, sitePageSize]);
  React.useEffect(() => { setSitePage(current => Math.min(current, siteTotalPages)); }, [siteTotalPages]);
    ...
    <div className="h-full overflow-auto bg-slate-50/70 p-5">
      <div className="mx-auto max-w-[1500px]">
        ...
        {overview.sites.length === 0 ? (
          ...
          <Pagination currentPage={sitePage} totalItems={overview.sites.length} itemsPerPage={sitePageSize} onPageChange={setSitePage} onItemsPerPageChange={size => { setSitePageSize(size); setSitePage(1); }} language={language === 'zh' ? 'zh' : 'en'} itemLabel={language === 'zh' ? '个站点' : 'sites'} />
          </>
        )}
...
const TopologyPage: React.FC<TopologyPageProps> = ({
  ...
  const topologyMapLinks = React.useMemo(
  ...
  return (
    <div className="topology-page-shell h-full flex flex-col overflow-hidden">
      <PageHero
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={onSyncHostnames}
              disabled={hostnameSyncing || topologyDiscoveryRunning}
              title={language === 'zh' ? '通过 SNMP sysName 同步全部网络设备主机名' : 'Sync all network device hostnames via SNMP sysName'}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
            >
              <RefreshCw size={13} className={hostnameSyncing ? 'animate-spin' : ''} />
              {hostnameSyncing
                ? (language === 'zh' ? '同步名称中...' : 'Syncing Names...')
                : (language === 'zh' ? '同步全部设备名称' : 'Sync All Names')}
            </button>
            <ActionButton
              type="button"
              icon={Download}
      ...
      <div className="bg-white dark:bg-zinc-900/90 border border-gray-200/70 dark:border-zinc-800/80 rounded-2xl p-3.5 shadow-2xs flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
          {[
            ...
            { label: language === 'zh' ? '管理链路' : 'Managed Links', value: topologyStats.managedLinkCount ?? topologyStats.linkCount, color: 'text-gray-800 dark:text-zinc-200' },
            { label: language === 'zh' ? '外部链路' : 'External Links', value: topologyStats.externalLinkCount ?? 0, color: 'text-sky-600 dark:text-sky-400' },
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="bg-white dark:bg-zinc-900/90 border border-gray-200/70 dark:border-zinc-800/80 relative flex h-[calc(100vh-270px)] min-h-[660px] flex-col overflow-hidden rounded-2xl shadow-2xs" ref={topologyCanvasRef}>
          ...
          <div className="relative flex-1 min-h-0">
            {canvasView === 'overview' ? (
              <TopologySiteOverview
                language={language}
                sites={topologySites}
                devices={topologyNetworkScope.devices}
                links={topologyMapLinks}
                onSelectSite={handleSiteFilterChange}
              />
            ) : topologyNetworkScope.devices.length === 0 ? (
