...
const generateSequenceEvents = (result: any, lanes?: any[]) => {
  ...
  const isPortProbeFailed = result.report && result.report.conclusion === 'interrupted' && result.report.interrupted_at === '目标主机/策略层';
  ...
  const isReachable = result.report && result.report.conclusion === 'reachable';
...
const IPLocatorPage: React.FC<IPLocatorPageProps> = ({ language, t, mode }) => {
  ...
  const [nsotPageSize, setNsotPageSize] = useState(15);
  ...
  const [diagSourceIp, setDiagSourceIp] = useState('10.1.1.10');
  const [diagTargetIp, setDiagTargetIp] = useState('172.16.1.100');
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <PageHero
        ...
        title={mode === 'diagnose-only' ? (zh ? 'NPA 智能路径诊断' : 'NPA Path Diagnostics') : mode === 'nsot-only' ? (zh ? '网络事实库' : 'Network Source of Truth') : (zh ? 'IP 工具箱' : 'IP Toolbox')}
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  <div className="flex items-center gap-2.5 bg-white dark:bg-white/[0.06] rounded-xl px-4 py-3 border border-black/8 dark:border-white/10 shadow-sm">
                    ...
                    <div>
                      ...
                      <p className="text-base font-bold text-[#164e63] dark:text-[var(--app-text)] font-mono">{result.mac_display}</p>
                      {result.arp_source && (
                      ...
                      <div className="flex items-center gap-2.5 bg-white dark:bg-white/[0.06] rounded-xl px-4 py-3 border border-[#06b6d4]/30 dark:border-[#06b6d4]/20 shadow-sm ring-1 ring-[#06b6d4]/10">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            {loc.vlan && <span className="text-[10px] bg-cyan-50 dark:bg-cyan-500/10 text-cyan-700 dark:text-cyan-400 rounded px-1.5 py-0.5 font-medium">VLAN {loc.vlan}</span>}
                            {loc.type === 'ARP_DIRECT'
                ...
                <div>
                  ...
                  {result.mac && (
                    <p className="text-xs text-amber-600 mt-1.5">
                      {zh ? `MAC 地址 ${result.mac_display} 已从 ARP 表获取，但未在任何交换机 MAC 表中匹配到端口` : `MAC ${result.mac_display} resolved from ARP but not found in any switch MAC table`}
                    </p>
                  )}
                </div>
              </div>
      ...
      {!arpLoading && !arpError && arpTable && arpTable.total === 0 && (
        <div className="bg-white rounded-2xl border border-black/5 shadow-sm p-8 text-center">
          ...
          <p className="text-sm text-black/40 dark:text-white/40">{zh ? '暂无 ARP 缓存数据。点击「立即采集」从网关设备获取 ARP 表。' : 'No ARP cache data. Click "Sweep Now" to collect from gateways.'}</p>
        </div>
      )}
      ...
      {!arpLoading && arpTable && arpTable.total > 0 && (() => {
        ...
        return (
          <div className="bg-white rounded-2xl border border-black/5 shadow-sm overflow-hidden">
            {/* Table header */}
            <div className="grid grid-cols-[1fr_1fr_6rem_6rem_10rem_8rem] gap-2 px-5 py-2.5 text-[10px] font-bold text-black/40 dark:text-white/40 uppercase tracking-wider border-b border-black/5 dark:border-white/8 bg-black/[0.01] dark:bg-white/[0.02]">
              <span>IP</span>
              <span>MAC</span>
            ...
            <div>
              {filtered.length === 0 ? (
                ...
                pageEntries.map((entry, idx) => (
                  <div
                    key={entry.ip}
                    className={`grid grid-cols-[1fr_1fr_6rem_6rem_10rem_8rem] gap-2 px-5 py-2 text-xs items-center border-b border-black/[0.03] dark:border-white/[0.05] hover:bg-[#ecfeff]/30 dark:hover:bg-cyan-500/5 transition-colors cursor-pointer ${idx % 2 ? 'bg-black/[0.008] dark:bg-white/[0.015]' : ''}`}
                  ...
                  >
                    ...
                    <span className="font-mono text-black/50">{entry.mac}</span>
                    ...
                    <span className="text-black/30" title={entry.cached_at}>{formatRelativeTime(entry.cached_at, zh)}</span>
                  </div>
                ))
              )}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <button onClick={() => { setNsotSubTab('routes'); setNsotPage(1); setNsotFilter(''); resetNsotAdvancedFilters(); }} className={`text-left bg-white dark:bg-[#1f2937]/30 rounded-2xl border shadow-sm p-5 transition-all ${nsotSubTab === 'routes' ? 'border-violet-400 ring-2 ring-violet-400/20' : 'border-black/5 dark:border-white/5 hover:border-violet-400/30'}`}>
          ...
          <div className="text-3xl font-bold text-violet-500 mb-1">{nsotRoutes?.total ?? '—'}</div>
          <div className="text-[11px] text-black/40 dark:text-white/40">
            {nsotRoutes?.protocol_stats ? Object.entries(nsotRoutes.protocol_stats).map(([k, v]) => `${k}: ${v}`).join(' · ') : `${zh ? '字段' : 'Cols'}: 11`}
          </div>
        ...
        <button onClick={() => { setNsotSubTab('neighbors'); setNsotPage(1); setNsotFilter(''); resetNsotAdvancedFilters(); }} className={`text-left bg-white dark:bg-[#1f2937]/30 rounded-2xl border shadow-sm p-5 transition-all ${nsotSubTab === 'neighbors' ? 'border-indigo-400 ring-2 ring-indigo-400/20' : 'border-black/5 dark:border-white/5 hover:border-indigo-400/30'}`}>
          ...
          <div className="text-3xl font-bold text-indigo-500 mb-1">{nsotNeighbors?.total ?? '—'}</div>
          <div className="text-[11px] text-black/40 dark:text-white/40">
            {nsotNeighbors?.protocol_stats ? Object.entries(nsotNeighbors.protocol_stats).map(([k, v]) => `${k}: ${v}`).join(' · ') : `${zh ? '字段' : 'Cols'}: 12`}
          </div>
        ...
        <button onClick={() => { setNsotSubTab('bgp_routes'); setNsotPage(1); setNsotFilter(''); resetNsotAdvancedFilters(); }} className={`text-left bg-white dark:bg-[#1f2937]/30 rounded-2xl border shadow-sm p-5 transition-all ${nsotSubTab === 'bgp_routes' ? 'border-amber-400 ring-2 ring-amber-400/20' : 'border-black/5 dark:border-white/5 hover:border-amber-400/30'}`}>
          ...
          <div className="text-3xl font-bold text-amber-500 mb-1">{nsotBgpRoutes?.total ?? '—'}</div>
          <div className="text-[11px] text-black/40 dark:text-white/40">
            {zh ? '最佳路径' : 'Best'}: {nsotBgpRoutes?.entries ? nsotBgpRoutes.entries.filter((e: any) => e.is_best).length : 0} · {zh ? '字段' : 'Cols'}: 13
          </div>
      ...
      {!nsotLoading && (() => {
        ...
        const normalizeRouteProtocol = (proto: string): string => {
          ...
          if (p === 'connected' || p === 'direct' || p === 'c' || p === 'l') return 'connected';
          if (p === 'static' || p === 's') return 'static';
          if (p.startsWith('o')) return 'ospf';
          if (p.startsWith('b')) return 'bgp';
          return p;
        };
        ...
        const filtered = (currentData.entries || []).filter((e: any) => {
          ...
          if (nsotShowAdvanced) {
            if (nsotSubTab === 'endpoints') {
              ...
              if (epFilterSwitch && !String(e.site ?? '').toLowerCase().includes(epFilterSwitch.toLowerCase())) return false;
                    ...
                    {paged.map((row: any, idx: number) => (
                      <tr key={idx} className="border-b border-black/[0.03] dark:border-white/[0.03] hover:bg-black/[0.015] dark:hover:bg-white/[0.015] transition-colors">
                        {columns.map(col => {
                          ...
                          if (col.key === 'protocol') {
                            const colors: Record<string, string> = { ospf: 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400', bgp: 'bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400', static: 'bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400', connected: 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400', direct: 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400' };
      ...
      <div className="bg-white dark:bg-[#1f2937]/30 rounded-2xl border border-black/5 dark:border-white/5 shadow-sm p-5 space-y-4">
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="text-[11px] text-black/35 dark:text-white/35 flex items-center gap-1.5">
            ...
            <span>{zh ? '提示：输入源 10.1.1.10 与目标 172.16.1.100 可进行 NPA 智能路径诊断效果演示。' : 'Tip: Input source 10.1.1.10 and target 172.16.1.100 for mock NPA Path Diagnostics demo.'}</span>
          </div>
          <button
            onClick={() => doDiagnose()}
      ...
      {diagResult && diagResult.hops && (() => {
        ...
        return (
          <div className="bg-white dark:bg-[#1f2937]/30 rounded-2xl border border-black/5 dark:border-white/5 shadow-sm p-6 space-y-5">
            ...
            {viewMode === 'topology' ? (
              <div className="flex flex-col sm:flex-row items-center justify-center gap-2 pt-6 pb-4 overflow-x-auto">
                {/* Source Node */}
                <div className="flex flex-col items-center flex-shrink-0 bg-white dark:bg-zinc-900 border border-violet-200 dark:border-violet-500/20 rounded-2xl p-4 w-36 shadow-sm">
                    ...
                    <React.Fragment key={idx}>
                      ...
                      <div className="flex items-center justify-center flex-shrink-0 w-8">
                        <div className={`h-0.5 flex-1 ${
                          isBlocked || isTimeout ? "border-t-2 border-dashed border-rose-400" : "border-t-2 border-emerald-400"
                        }`} />
                        <ArrowRight size={14} className={isBlocked || isTimeout ? "text-rose-400" : "text-emerald-400"} />
                      </div>

                      {/* Device Node (Interactive) */}
                      <div className="relative flex-shrink-0">
                        <button
                          onClick={() => setActiveHopIdx(isActive ? null : idx)}
                          className={`flex flex-col items-center bg-white dark:bg-zinc-900 border rounded-2xl p-4 w-36 shadow-sm transition-all cursor-pointer hover:shadow-md hover:scale-[1.03] ${
                ...
                {diagResult.report.conclusion === 'reachable' && (
                  <>
                    <div className="flex items-center justify-center flex-shrink-0 w-8">
                      <div className="h-0.5 flex-1 border-t-2 border-emerald-400" />
                      <ArrowRight size={14} className="text-emerald-400" />
                    </div>
                    <div className="flex flex-col items-center flex-shrink-0 bg-white dark:bg-zinc-900 border border-emerald-200 dark:border-emerald-500/20 rounded-2xl p-4 w-36 shadow-sm">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs text-neutral-500 hover:text-[#0891b2] dark:text-neutral-400 dark:hover:text-[#06b6d4] bg-neutral-100 dark:bg-white/[0.05] hover:bg-neutral-200/60 dark:hover:bg-white/[0.08] transition-colors px-3 py-1.5 rounded-lg border border-neutral-200 dark:border-white/5 font-semibold"
              ...
              <span className={`text-xs px-3 py-1.5 rounded-full font-bold uppercase tracking-wider border ${
                ...
                {diagResult.report.conclusion === 'interrupted' ? (zh ? '检测到阻断' : 'Blocked') : (zh ? '路径可达' : 'Reachable')}
              </span>
            </div>
          </div>
              ...
              <div className="bg-neutral-900 dark:bg-black/40 border border-neutral-800 rounded-xl p-5 shadow-inner space-y-3 relative group">
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1 text-[10px] text-neutral-400 hover:text-white bg-neutral-800 hover:bg-neutral-700 transition-colors px-2.5 py-1 rounded border border-neutral-700/50"
                  >
      ...
      {diagResult && diagResult.hops && activeHopIdx !== null && (() => {
        ...
        const cmdbDevice = findDeviceByIp(activeHop.ip);
            ...
            >
              ...
              <div className="px-5 py-4 border-b border-black/5 dark:border-white/5 bg-gradient-to-r from-[#0891b2]/5 to-transparent flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-bold text-[#164e63] dark:text-[var(--app-text)]">{activeHop.device_name}</div>
                    <div className="text-xs text-black/45 dark:text-white/45 font-mono">{activeHop.ip}</div>
                  </div>
                </div>
                <button 
                  onClick={() => setActiveHopIdx(null)} 
