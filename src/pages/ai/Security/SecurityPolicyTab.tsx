...
export const SecurityPolicyTab: React.FC = () => {
  ...
  const save = async (patch: Partial<AISecurityPolicy>) => {
    ...
    setNotice(null);
    try {
      const nextPolicy = await updateAISecurityPolicy({ ...policy, ...patch });
  ...
  }, [policy.external_ai_enabled, policy.kill_switch]);
      ...
      <div className="mx-auto w-full max-w-[1500px] space-y-6">
        <header className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            ...
            <h1 className="text-2xl font-bold tracking-tight text-slate-950 dark:text-white sm:text-3xl">AI 安全网关</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500 dark:text-slate-400">统一控制外部 AI 出口。默认拒绝、可审计、可演练；Provider 连接测试和所有 AI 调用都必须经过这里。</p>
          </div>
          <button
            type="button"
            onClick={() => void load()}
        ...
        <section className={`relative overflow-hidden rounded-3xl border p-5 shadow-sm sm:p-6 ${statusTone}`}>
          ...
          <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
            ...
            <div className="flex shrink-0 items-center gap-2 rounded-full bg-white/70 px-3 py-2 text-xs font-bold uppercase tracking-wider shadow-sm dark:bg-slate-950/40">
              ...
              {gatewayReady ? 'Protected / ready' : 'Blocked by policy'}
            </div>
          </div>
        </section>
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50/70 p-4 dark:border-slate-700 dark:bg-slate-950/60">
              ...
              {dryRun ? (
                <div className="mt-4 space-y-3">
                  <div className={`rounded-xl border px-3 py-2.5 text-sm font-bold ${decisionClass(dryRun.decision)}`}>
