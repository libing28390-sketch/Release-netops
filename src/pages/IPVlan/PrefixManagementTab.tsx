...
const PrefixManagementTab: React.FC<PrefixManagementTabProps> = ({ language, t }) => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/ipam/subnets?q=${encodeURIComponent(search)}`, {
        headers: authHeaders(),
      });
      if (res.ok) {
  ...
  const filteredPrefixes = prefixes.filter(matchFamily);
      ...
      <React.Fragment key={node.id}>
        <tr className="hover:bg-gray-50/50 transition-colors border-b border-gray-100 group">
          ...
          <td className="px-4 py-4">
            {(() => {
              const nt = NETWORK_TYPES[node.network_type] || NETWORK_TYPES['server'];
              return (
          ...
          <td className="px-4 py-4">
            <div className="flex flex-col gap-1 w-28">
              <div className="flex justify-between text-[10px] font-bold text-gray-400">
                <span>{node.used_ips}/{node.active_ips || 0}/{node.total_ips}</span>
          ...
          <div className="flex items-center gap-2">
            ...
            {selectedIds.size > 0 && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 bg-rose-500 hover:bg-rose-600 text-white rounded-xl text-xs font-bold shadow-lg shadow-rose-500/20 active:scale-95 transition-all animate-pulse"
              >
                <Trash2 size={14} />
      ...
      <div className="flex-1 min-h-0 bg-white rounded-[28px] border border-black/5 shadow-[0_16px_36px_rgba(11,35,64,0.06)] overflow-hidden flex flex-col">
        ...
        {viewMode === 'table' && (
          <div className="px-6 py-4 border-t border-gray-100">
            <Pagination
              currentPage={page}
              totalItems={filteredPrefixes.length}
