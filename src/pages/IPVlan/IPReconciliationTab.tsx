...
const IPReconciliationTab: React.FC<IPReconciliationTabProps> = ({ language, t }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/ipam/reconciliation', { headers: hdrs });
      if (res.ok) {
  ...
  const handleOpenRegister = (item: UndocumentedEndpoint) => {
    setRegisterForm({
      ...
      interface_name: item.switch_port ? `${item.switch_id}:${item.switch_port}` : '',
      status: 'active',
  ...
  const submitRelease = async () => {
    ...
    setApiError('');
    try {
      const res = await fetch(`/api/ipam/addresses/${releasingItem.id}`, {
        method: 'DELETE',
        headers: authHeaders(),
      });
      if (res.ok) {
                      ...
                      (paginatedList as UndocumentedEndpoint[]).map((node) => (
                        <tr key={node.ip} className="hover:bg-gray-50/50 transition-colors border-b border-gray-100 group">
                          ...
                          <td className="px-4 py-4 text-xs font-semibold text-cyan-600">
                            {node.switch_id ? `${node.switch_id} [${node.switch_port}]` : '-'}
                          </td>
            ...
            >
              ...
              <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-900">{zh ? '将活跃终端登记录入 IPAM' : 'Register Discovered Host'}</h3>
                    <p className="text-xs text-gray-500 mt-0.5">{zh ? '新建一个IP分配记录并保存入库' : 'Create an IPAM allocation record for the online host.'}</p>
                  </div>
                </div>
