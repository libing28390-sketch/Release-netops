...
const IPVlanTab: React.FC<IPVlanTabProps> = ({ language, t }) => {
  ...
  const [subnetPageSizeOpt, setSubnetPageSizeOpt] = useState(20);
  const [addrPageSizeOpt, setAddrPageSizeOpt] = useState(20);
  ...
  const loadAddresses = async (subnet: Subnet, pageOverride = addrPage) => {
    setSelectedSubnet(subnet);
    try {
      const params = new URLSearchParams({ q: addrSearch.trim(), status: addrStatusFilter, device_type: addrDeviceTypeFilter || 'all', page: String(pageOverride), page_size: String(addrPageSizeOpt) });
      const r = await fetch(`/api/ipam/subnets/${subnet.id}/addresses?${params.toString()}`, { headers: authHeaders() });
      if (r.ok) {
        const payload = await r.json();
        const data = payload?.data && !Array.isArray(payload.data) ? payload.data : payload;
        if (Array.isArray(data)) { setAddresses(data); setAddrTotal(data.length); }
        else { setAddresses(Array.isArray(data?.items) ? data.items : []); setAddrTotal(Number(data?.total || 0)); }
      }
  ...
  useEffect(() => {
    if (selectedSubnet) void loadAddresses(selectedSubnet, addrPage);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedSubnet, addrPage, addrPageSizeOpt, addrSearch, addrStatusFilter, addrDeviceTypeFilter]);
  ...
  const addrTotalPages = Math.max(1, Math.ceil(addrTotal / addrPageSizeOpt));
  const safeAddrPage = Math.min(addrPage, addrTotalPages);
  const pagedAddresses = filteredAddresses;
                                ...
                                <td className="px-3 py-2">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span
                                      ...
                                      className="nx-action-icon nx-action-icon--xs nx-action-icon--default opacity-0 group-hover/row:opacity-100 transition-opacity"
                                    >
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span
                                          ...
                                          onClick={(e) => openEditSubnet(subnet, e)}
                                          className="nx-action-icon nx-action-icon--xs nx-action-icon--default opacity-0 group-hover/subnet:opacity-100 transition-opacity"
                                        >
                                          <Pencil size={14} strokeWidth={1.8} />
                                        </span>
                                      </div>
                      ...
                      <div className="px-4 py-3 border-b border-black/5 bg-gradient-to-r from-[#00bceb]/5 to-transparent flex-shrink-0">
                        ...
                        <div className="flex gap-2 items-center flex-shrink-0 justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <ActionIconButton
                              icon={Trash2}
                              label={language === 'zh' ? '删除子网' : 'Delete Subnet'}
                              variant="danger"
                              onClick={() =>
                                setConfirmDelete({
                                  type: 'subnet',
                              ...
                              {pagedAddresses.map((addr) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="px-3 py-2">
                                      <ActionIconGroup label={language === 'zh' ? 'IP 操作' : 'IP actions'} className="opacity-0 transition-opacity group-hover/row:opacity-100">
                                        <ActionIconButton
                                          icon={Pencil}
                                          label={language === 'zh' ? '编辑' : 'Edit'}
                            ...
                            {pagedAddresses.map((addr) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {/* Card hover actions */}
                                  <ActionIconGroup label={language === 'zh' ? 'IP 操作' : 'IP actions'} className="absolute right-1.5 top-1.5 opacity-0 transition-opacity group-hover/card:opacity-100">
                                    <ActionIconButton
                                      icon={Pencil}
                                      label={language === 'zh' ? '编辑' : 'Edit'}
                                      onClick={() => openEditIP(addr)}
