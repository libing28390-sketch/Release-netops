...
export async function fetchDeviceTelemetry(
  ...
  // Match device in network monitoring registry
  const matched = networkDevices.find((d: any) => {
    ...
    return (
      h === lowerName ||
      id === lowerName ||
      (rawDeviceId && id === rawDeviceId.toLowerCase()) ||
      (rawDeviceId && assetId === rawDeviceId.toLowerCase()) ||
      ip === lowerName ||
  ...
  try {
    ...
    const rawInterfaces = Array.isArray(devInfo.interface_data) ? devInfo.interface_data : [];
