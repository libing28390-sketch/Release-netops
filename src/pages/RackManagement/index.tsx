...
const RackFleetOverview = React.lazy(() => import('./Rack3D/RackFleetOverview').then(m => ({ default: m.RackFleetOverview })));
import { DevicePortMatrix } from './components/DevicePortMatrix';
import { RackReadonlyInspector } from './components/RackReadonlyInspector';
import RackElevation from './components/RackElevation';
import { useRackReadModel } from './hooks/useRackReadModel';
...
interface Asset {
  ...
  rack_mount_kind?: string | null;
  rack_height_u?: number | null;
  rack_placement_status?: string | null;
  rack_location_note?: string | null;
}
...
export const RackManagementTab: React.FC<Props> = ({ language }) => {
  ...
  const [viewMode, setViewMode] = useState<'2d' | '3d' | 'overview'>('2d');
  ...
  const [overviewFilterDraft, setOverviewFilterDraft] = useState({ floor: '', room: '', row: '', health: '' });
  const [overviewFilters, setOverviewFilters] = useState({ floor: '', room: '', row: '', health: '' });
  const rackPageSize = viewMode === 'overview' ? 500 : 50;
  ...
  const {
    ...
    page: rackPage,
    pageSize: rackPageSize,
    keyword: submittedSearch,
    siteId: dcFilter,
    floor: overviewFilters.floor,
    room: overviewFilters.room,
    row: overviewFilters.row,
    health: overviewFilters.health,
  });
  const loading = summaryLoading || layoutLoading;
  ...
  const rackWorkspaceRef = useRef<HTMLDivElement>(null);
  const installAssetsAbortRef = useRef<AbortController | null>(null);
  const installAssetsRequestSeqRef = useRef(0);
  const [isRackFullscreen, setIsRackFullscreen] = useState(false);
  ...
  const [deviceForm, setDeviceForm] = useState({
    ...
    device_type_id: '',
    start_u: 1,
    mount_kind: 'u_mount' as string,
    position: 'front' as string,
    location_note: '',
    status: 'active',
    serial_number: '',
    asset_id: '',
  });
  const [dtForm, setDtForm] = useState({ model: '', vendor: '', u_height: 1, device_role: 'switch', default_mount_kind: 'u_mount', dimension_status: 'unknown', description: '', power_watts: 0 });
  ...
  // Fast local hint for the selected rack. Cross-rack duplicate installation
  // is enforced transactionally by the backend and no longer requires loading
  // every rack layout into the browser.
  ...
  const loadHealthOverview = useCallback(async (rackId: string) => {
    if (!rackId) {
      setHealthMap({});
      return;
    }
    try {
      const params = new URLSearchParams({ rack_id: rackId });
      const res = await fetchJson<any>(`${API}/device-health/overview?${params.toString()}`);
  ...
  const fetchInstallAssets = useCallback(async (q: string, page: number, append: boolean) => {
    const requestSeq = ++installAssetsRequestSeqRef.current;
    installAssetsAbortRef.current?.abort();
    const controller = new AbortController();
    installAssetsAbortRef.current = controller;
    try {
      ...
      if (controller.signal.aborted || requestSeq !== installAssetsRequestSeqRef.current) return;
      ...
      if (controller.signal.aborted || requestSeq !== installAssetsRequestSeqRef.current || e?.name === 'AbortError') return;
      ...
      if (!controller.signal.aborted && requestSeq === installAssetsRequestSeqRef.current) {
  ...
  useEffect(() => () => {
    installAssetsRequestSeqRef.current += 1;
    installAssetsAbortRef.current?.abort();
  }, []);

  useEffect(() => { loadDeviceTypes(); loadRackTypes(); loadStats(); loadSites(); }, []);
  useEffect(() => {
    if (viewMode !== 'overview' && selectedRackId) {
      loadLayout(selectedRackId);
      loadHealthOverview(selectedRackId);
    }
  }, [selectedRackId, loadHealthOverview, loadLayout, viewMode]);

  useEffect(() => {
    const nextError = summaryError || layoutError;
    if (nextError) setError(nextError);
  }, [summaryError, layoutError]);
  ...
  }, [loadLayout, racks, selectedRackId, summaryLoading]);
  ...
  const refresh = () => {
    invalidateLayout();
    reloadSummaries();
    if (selectedRackId) void loadLayout(selectedRackId, true);
    loadRackTypes();
    loadStats();
    if (selectedRackId) loadHealthOverview(selectedRackId);
  };
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      setIsRackFullscreen(document.fullscreenElement === rackWorkspaceRef.current);
    };
    document.addEventListener('fullscreenchange', handleFullscreenChange);
    return () => document.removeEventListener('fullscreenchange', handleFullscreenChange);
  }, []);

  const handleToggleRackFullscreen = useCallback(async () => {
    const target = rackWorkspaceRef.current;
    if (!target) return;
    try {
      if (isRackFullscreen) {
        if (document.fullscreenElement) {
          ...
          // Some embedded browser surfaces resize the page without exposing a
          // document.fullscreenElement. Keep the shell toggle reversible there.
          setIsRackFullscreen(false);
        }
        return;
      }
      await target.requestFullscreen();
    } catch (fullscreenError) {
      console.error('Rack workspace fullscreen toggle failed:', fullscreenError);
      setError(zh ? '无法切换全屏，请检查浏览器权限' : 'Unable to toggle fullscreen. Check browser permissions.');
    }
  }, [isRackFullscreen, zh]);
  ...
  const handleExport = () => {
    try {
      const rows = filteredRacks.map(r => ({
          ...
          [zh ? '已用U位' : 'Used U']: r.used_u,
          [zh ? '可用U位' : 'Available U']: r.available_u,
  ...
  const openInstallDevice = () => {
    ...
    setAssetTotal(0);
    setAssetPage(1);
    setDeviceForm({ name: '', device_type_id: '', start_u: 1, mount_kind: 'u_mount', position: 'front', location_note: '', status: 'active', serial_number: '', asset_id: '' });
    setFormError('');
  ...
  const defaultPositionForMountKind = (mountKind: string): string => {
    switch (mountKind) {
      case 'side_mount': return 'left_side';
      case 'zero_u': return 'rear';
      case 'floor':
  ...
  const devicesForPosition = (position: string): RackDevice[] => {
    if (position === 'full_depth') {
      return layout?.devices.filter(d => ['front', 'rear', 'full_depth'].includes(d.position)) || [];
    }
    return layout?.devices.filter(d => d.position === position || d.position === 'full_depth') || [];
  };

  const resolveStartUForInstall = (deviceTypeId: string, position: string): number => {
    const uH = deviceTypes.find(dt => dt.id === deviceTypeId)?.u_height ?? 0;
    if (uH < 1) return 1;
    if (!layout || !selectedRackId) return 1;
    if (selectedAsset) {
      return pickStartUForAsset(layout, position, uH, selectedAsset);
    }
    const sideDevices = devicesForPosition(position);
  ...
  const handleAssetSelect = (asset: Asset) => {
    ...
    const matchedType = matchedId ? deviceTypes.find(dt => dt.id === matchedId) : undefined;
    const mount_kind = asset.rack_mount_kind || matchedType?.default_mount_kind || 'u_mount';
    const position = asset.rack_position || defaultPositionForMountKind(mount_kind);
    const uH = matchedType
      ? (matchedType.u_height ?? 0)
      : (asset.u_height != null && asset.u_height > 0 ? asset.u_height : 0);
    const startU =
      mount_kind === 'u_mount'
        ? (asset.planned_start_u ?? (layout && selectedRackId ? pickStartUForAsset(layout, position, uH, asset) : 1))
        : 1;
    setDeviceForm({
      ...
      mount_kind,
      position,
      location_note: asset.rack_location_note || '',
  ...
  const saveDevice = async () => {
    ...
    const uH = dt?.u_height ?? 0;
    const isUMount = deviceForm.mount_kind === 'u_mount';
    if (isUMount && uH < 1) {
      setFormError(zh ? '该设备类型未提供有效 U 高度，不能按标准 U 位安装；请先补齐型号尺寸或选择非标准安装方式。' : 'This device type has no valid U height. Add the verified height or choose a non-U mount kind.');
      return;
    }
    if (isUMount && layout) {
      const sideDevices = devicesForPosition(deviceForm.position);
    ...
    if (deviceForm.mount_kind === 'side_mount' && !['left_side', 'right_side'].includes(deviceForm.position)) {
      setFormError(zh ? '侧挂设备必须选择左侧或右侧' : 'Side-mounted equipment must use the left or right side');
      return;
    }
    if (isUMount && !['front', 'rear', 'full_depth'].includes(deviceForm.position)) {
      setFormError(zh ? '标准 U 位设备必须选择前面板、后面板或全深度' : 'Standard U equipment must use front, rear, or full-depth position');
      return;
    }
    if (deviceForm.mount_kind === 'zero_u' && !['rear', 'left_side', 'right_side', 'unknown'].includes(deviceForm.position)) {
      setFormError(zh ? '0U 设备只能放在后部、左右侧或待确认位置' : '0U equipment must use rear, left/right side, or unknown position');
      return;
    }
    if (deviceForm.mount_kind === 'floor' && !deviceForm.location_note.trim()) {
      setFormError(zh ? '落地设备必须填写位置说明' : 'Floor-standing equipment requires a location note');
      return;
    }
    try {
      await postJson(`${API}/racks/${encodeURIComponent(selectedRackId)}/placements`, {
        name: deviceForm.name,
        rack_id: selectedRackId,
        device_type_id: deviceForm.device_type_id,
        start_u: isUMount ? deviceForm.start_u : null,
        mount_kind: deviceForm.mount_kind,
        height_u: isUMount ? uH : null,
        position: deviceForm.position,
        placement_status: isUMount ? 'confirmed' : 'estimated',
        placement_source: 'manual',
        location_note: deviceForm.location_note || '',
        status: deviceForm.status,
        serial_number: deviceForm.serial_number || '',
        asset_id: deviceForm.asset_id || '',
      });
      closeInstallModal();
      invalidateLayout(selectedRackId);
      await loadLayout(selectedRackId, true);
      reloadSummaries();
      loadStats();
    } catch (e: any) { setFormError(e.message); }
  };
  ...
  const confirmRemoveDeviceAction = async () => {
    ...
    setError('');
    try {
      await deleteJson(`${API}/rack-placements/${deviceId}`);
  ...
  const openNewDeviceType = () => {
    setDtForm({ model: '', vendor: '', u_height: 1, device_role: 'switch', default_mount_kind: 'u_mount', dimension_status: 'unknown', description: '', power_watts: 0 });
  ...
  const confirmMoveDeviceAction = useCallback(async () => {
    ...
    try {
      await patchJson(`${API}/rack-placements/${deviceId}`, { start_u: newU });
      invalidateLayout(selectedRackId);
      await loadLayout(selectedRackId, true);
      reloadSummaries();
    } catch (e: any) {
  ...
  const handleDownloadRack = useCallback(() => {
    ...
    const side = viewSide;
    const devices = layout.devices.filter(d =>
      (d.position === side || d.position === 'full_depth')
      && (d.mount_kind || 'u_mount') === 'u_mount'
      && d.start_u != null
    ...
    for (const d of devices) {
      ...
      devMap.set(startU, d);
      for (let u = startU + 1; u < startU + heightU; u++) spanned.add(u);
    }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div
        ref={rackWorkspaceRef}
        className={`flex gap-3 flex-1 min-h-0 ${isRackFullscreen ? 'p-3 bg-[#090d16]' : ''}`}
      >
        {/* Left: rack list */}
        <div className={`${viewMode === '3d' ? 'hidden md:flex' : 'flex'} w-56 flex-shrink-0 rounded-xl overflow-hidden flex-col`} style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
          <div className="p-3 space-y-2 border-b" style={{ borderColor: 'var(--card-border)' }}>
            <div className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5" style={{ background: 'var(--app-hover-bg)' }}>
              ...
              <input
                ...
                placeholder={zh ? '机柜、设备、型号或序列号' : 'Rack, device, model or serial'}
              ...
              {searchQuery && (
                ...
                >
                  <X size={12} style={{ color: 'var(--muted-text)' }} />
                </button>
              )}
              <button
                type="button"
                ...
                title={zh ? '搜索' : 'Search'}
                className="rounded p-0.5 hover:bg-cyan-500/10"
              >
                <Search size={12} className="text-cyan-500" />
              </button>
            </div>
            {siteOptions.length > 1 && (
              <select value={dcFilter} onChange={e => { setDcFilter(e.target.value); setRackPage(1); }} className="w-full text-[11px] rounded-lg px-2 py-1.5 outline-none" style={{ background: 'var(--app-hover-bg)', color: 'var(--body-text)', border: '1px solid var(--card-border)' }}>
          ...
          <div className="flex-1 overflow-auto p-1.5 space-y-0.5">
            {summaryLoading ? (
              <div className="flex items-center justify-center gap-2 py-8 text-xs" style={{ color: 'var(--muted-text)' }}>
                <RefreshCw size={13} className="animate-spin" /> {zh ? '正在加载机柜摘要…' : 'Loading rack summaries…'}
              </div>
            ) : summaryError ? (
              <div className="px-3 py-8 text-center text-xs text-amber-600">
                <p>{zh ? '机柜摘要加载失败' : 'Rack summaries failed to load'}</p>
                <button type="button" onClick={reloadSummaries} className="mt-2 rounded border border-amber-500/40 px-2 py-1 text-[10px]">
                  {zh ? '重试' : 'Retry'}
                </button>
              </div>
            ) : filteredRacks.length === 0 ? (
              <p className="text-center text-xs py-8" style={{ color: 'var(--muted-text)' }}>
                {submittedSearch || dcFilter ? (zh ? '当前筛选没有匹配机柜' : 'No racks match the current filters') : (zh ? '暂无机柜' : 'No racks')}
              </p>
            ) : filteredRacks.map(rack => {
              const ly = rack;
              const pUsed = rack.power_used_watts;
              const pCap = rack.power_capacity_watts || rack.power_capacity_w || 0;
              const pPct = rack.power_utilization_pct ?? 0;
              return (
                <div
                  key={rack.id}
                  className={`relative w-full rounded-lg transition-all group ${rack.id === selectedRackId ? 'ring-1 ring-cyan-500/50' : ''}`}
                  style={{ background: rack.id === selectedRackId ? 'var(--app-hover-bg)' : 'transparent', color: 'var(--body-text)' }}
                >
                  <button
                    type="button"
                    onClick={() => setSelectedRackId(rack.id)}
                    className={`w-full text-left rounded-lg px-3 py-2 ${viewMode === '2d' ? 'pr-16' : ''}`}
                    aria-pressed={rack.id === selectedRackId}
                  >
                    <div className="flex items-center justify-between">
                  ...
                  <div className="mt-1 flex items-center justify-between text-[9px]" style={{ color: 'var(--muted-text)' }}>
                    <span>{rack.device_count} {zh ? '台设备' : 'devices'} · {rack.used_u}/{rack.total_u}U</span>
                    <span className={rack.data_quality_status === 'complete' ? 'text-emerald-500' : rack.data_quality_status === 'invalid' ? 'text-red-500' : 'text-amber-500'}>
                      {rack.data_quality_status === 'complete' ? (zh ? '数据完整' : 'Complete') : rack.data_quality_status === 'invalid' ? (zh ? '数据异常' : 'Invalid') : rack.data_quality_status === 'empty' ? (zh ? '空机柜' : 'Empty') : (zh ? '数据部分缺失' : 'Partial')}
                    </span>
                  </div>
                  {ly && (
                  ...
                  {viewMode === '2d' && (
                    <ActionIconGroup className="absolute right-2 top-1.5 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
                      <ActionIconButton icon={Pencil} label={zh ? `编辑${rack.name}` : `Edit ${rack.name}`} size="xs" onClick={() => openEditRack(rack)} />
                      <ActionIconButton icon={Trash2} label={zh ? `删除${rack.name}` : `Delete ${rack.name}`} size="xs" variant="danger" onClick={() => handleDeleteRack(rack.id)} />
                    </ActionIconGroup>
                  )}
                </div>
              );
          ...
          <div className="flex items-center justify-between border-t px-2.5 py-2 text-[10px]" style={{ borderColor: 'var(--card-border)', color: 'var(--muted-text)' }}>
            <span>{zh ? `共 ${rackTotal} 个` : `${rackTotal} total`}</span>
            <div className="flex items-center gap-1">
              <button
                type="button"
                disabled={rackPage <= 1 || summaryLoading}
                onClick={() => setRackPage(page => Math.max(1, page - 1))}
                className="rounded border px-1.5 py-0.5 disabled:opacity-35"
                style={{ borderColor: 'var(--card-border)' }}
              >
                ‹
              </button>
              <span>{rackPage}/{Math.max(1, Math.ceil(rackTotal / rackPageSize))}</span>
              <button
                type="button"
                disabled={rackPage * rackPageSize >= rackTotal || summaryLoading}
                onClick={() => setRackPage(page => page + 1)}
                className="rounded border px-1.5 py-0.5 disabled:opacity-35"
                style={{ borderColor: 'var(--card-border)' }}
              >
                ›
        ...
        <div className="flex-1 rounded-xl overflow-hidden flex flex-col relative" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
          {viewMode === 'overview' ? (
            <>
              <div className="flex flex-wrap items-center justify-between gap-2 border-b px-4 py-2.5" style={{ borderColor: 'var(--card-border)' }}>
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="text-sm font-bold" style={{ color: 'var(--heading-text)' }}>
                    {zh ? '多机柜逻辑总览' : 'Multi-rack Logical Overview'}
                  </h3>
                  <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2 py-1 text-[10px] font-semibold text-amber-600">
                    {zh ? '推断位置 · 只读摘要' : 'Inferred positions · read-only summaries'}
                  </span>
                  <div className="flex items-center rounded-lg p-0.5" style={{ background: 'var(--app-hover-bg)', border: '1px solid var(--card-border)' }}>
                    <button type="button" onClick={() => setViewMode('2d')} className="flex items-center gap-1 rounded-md px-2 py-1 text-[10px] font-semibold" style={{ color: 'var(--muted-text)' }}>
                      <Columns3 size={11} /> 2D
                    </button>
                    <button type="button" onClick={() => setViewMode('3d')} className="flex items-center gap-1 rounded-md px-2 py-1 text-[10px] font-semibold" style={{ color: 'var(--muted-text)' }}>
                      <Box size={11} /> {zh ? '单机柜 3D' : 'Single 3D'}
                    </button>
                    <button type="button" className="flex items-center gap-1 rounded-md bg-cyan-600 px-2 py-1 text-[10px] font-semibold text-white shadow-sm">
                      <LayoutGrid size={11} /> {zh ? '逻辑总览' : 'Overview'}
                    </button>
                  </div>
                </div>
                <div className="flex flex-wrap items-center justify-end gap-1.5">
                  {(['floor', 'room', 'row'] as const).map(field => (
                    <input
                      key={field}
                      value={overviewFilterDraft[field]}
                      onChange={event => setOverviewFilterDraft(current => ({ ...current, [field]: event.target.value }))}
                      placeholder={field === 'floor' ? (zh ? '楼层' : 'Floor') : field === 'room' ? (zh ? '机房' : 'Room') : (zh ? '行/列' : 'Row')}
                      className="w-20 rounded-lg border px-2 py-1 text-[10px] outline-none"
                      style={{ background: 'var(--app-hover-bg)', borderColor: 'var(--card-border)', color: 'var(--body-text)' }}
                    />
                  ))}
                  <select
                    value={overviewFilterDraft.health}
                    onChange={event => setOverviewFilterDraft(current => ({ ...current, health: event.target.value }))}
                    className="rounded-lg border px-2 py-1 text-[10px] outline-none"
                    style={{ background: 'var(--app-hover-bg)', borderColor: 'var(--card-border)', color: 'var(--body-text)' }}
                  >
                    <option value="">{zh ? '全部健康状态' : 'All health'}</option>
                    <option value="healthy">{zh ? '健康' : 'Healthy'}</option>
                    <option value="offline">{zh ? '离线' : 'Offline'}</option>
                    <option value="partial">{zh ? '部分监控' : 'Partial'}</option>
                    <option value="unknown">{zh ? '未知' : 'Unknown'}</option>
                    <option value="empty">{zh ? '空机柜' : 'Empty'}</option>
                  </select>
                  <button
                    type="button"
                    onClick={() => {
                      setOverviewFilters({ ...overviewFilterDraft });
                      setRackPage(1);
                    }}
                    className="rounded-lg bg-cyan-600 px-2.5 py-1 text-[10px] font-semibold text-white hover:bg-cyan-700"
                  >
                    {zh ? '应用' : 'Apply'}
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      const emptyFilters = { floor: '', room: '', row: '', health: '' };
                      setOverviewFilterDraft(emptyFilters);
                      setOverviewFilters(emptyFilters);
                      setRackPage(1);
                    }}
                  ...
                  <span className="text-[10px]" style={{ color: 'var(--muted-text)' }}>{rackTotal} {zh ? '个结果' : 'results'}</span>
                </div>
              </div>
              <div className="min-h-0 flex-1">
                {summaryLoading ? (
                  <div className="flex h-full items-center justify-center gap-2 text-xs" style={{ color: 'var(--muted-text)' }}>
                    <RefreshCw size={14} className="animate-spin" /> {zh ? '正在加载逻辑总览…' : 'Loading logical overview…'}
                  </div>
                ) : summaryError ? (
                  <div className="flex h-full flex-col items-center justify-center gap-2 text-xs text-amber-600">
                    <AlertTriangle size={18} />
                    <span>{zh ? '逻辑总览摘要加载失败' : 'Logical overview summaries failed to load'}</span>
                    <button type="button" onClick={reloadSummaries} className="rounded border border-amber-500/40 px-2 py-1">{zh ? '重试' : 'Retry'}</button>
                  </div>
                ) : (
                  <React.Suspense fallback={
                    <div className="flex h-full items-center justify-center gap-2 text-xs" style={{ color: 'var(--muted-text)' }}>
                      <RefreshCw size={14} className="animate-spin" /> {zh ? '正在加载 3D 引擎…' : 'Loading 3D engine…'}
                    </div>
                  }>
                    <RackFleetOverview
                      racks={filteredRacks}
            ...
            <>
              <div className="flex items-center justify-between px-4 py-2.5 border-b" style={{ borderColor: 'var(--card-border)' }}>
                <div className="flex min-w-0 flex-wrap items-center gap-3">
                  ...
                  <div className="flex items-center rounded-lg p-0.5" style={{ background: 'var(--app-hover-bg)', border: '1px solid var(--card-border)' }}>
                    ...
                    <button
                      onClick={() => { setRackPage(1); setViewMode('overview'); }}
                  ...
                  {viewMode === '3d' && (
                    <span className="inline-flex items-center gap-1 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-2 py-1 text-[10px] font-semibold text-cyan-500">
                      <Eye size={11} />
                      {zh ? '只读视图' : 'Read-only'}
                    </span>
                  )}
                  ...
                  {viewMode === '2d' && (
                    <>
                      ...
                      <div className="hidden items-center gap-1 ml-3 pl-3 border-l md:flex" style={{ borderColor: 'var(--card-border)' }}>
                        <button
                ...
                <div className="flex-1 min-h-0 relative" style={{ paddingRight: drawerVisible ? 396 : undefined }}>
                  <React.Suspense fallback={
                    ...
                    <Rack3DContainer
                      ...
                      isFullscreen={isRackFullscreen}
                      onToggleFullscreen={handleToggleRackFullscreen}
                      zh={zh}
                    />
                  </React.Suspense>
                </div>
              ...
              {drawerVisible && selectedDeviceId && (() => {
                ...
                const rackDeviceVM = rackVM?.devices.find(item => item.id === selectedDeviceId);
                const rc = getNormalizedRoleColor(device.device_role, device.name, device.model);
                const statusInfo = STATUS_LABEL[device.status] || { zh: device.status, en: device.status };
                const isUMount = (device.mount_kind || 'u_mount') === 'u_mount';
                const startU = device.start_u ?? 1;
                const heightU = device.height_u ?? device.u_height ?? 1;
                const endU = startU + heightU - 1;
                const placementLabel = isUMount
                  ? `U${startU}${endU !== startU ? `–U${endU}` : ''}`
                  : `${device.mount_kind || 'unknown'} / ${device.position || 'unknown'}`;
                return (
                  <AnimatePresence>
                    <motion.div
                      initial={{ opacity: 0 }}
                    ...
                    >
                      ...
                      <div className="flex-1 overflow-y-auto p-4 space-y-3">
                        ...
                        {viewMode === '3d' ? (
                          rackDeviceVM ? (
                            <RackReadonlyInspector
                              device={rackDeviceVM}
                              rackUpdatedAt={selectedRack?.updated_at}
                              zh={zh}
                            />
                          ) : (
                            <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 text-xs text-amber-600">
                              {zh ? '当前设备无法生成只读视图模型，请返回 2D 检查布局数据。' : 'A read-only view model could not be built for this device. Check the layout in 2D.'}
                            </div>
                          )
                        ) : drawerAssetDetail ? (
                          ...
                          <>
                            ...
                            <div className="p-3 rounded-xl border" style={{ background: 'var(--app-hover-bg)', borderColor: 'var(--card-border)' }}>
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="text-[10px] font-mono" style={{ color: 'var(--muted-text)' }}>
                                  {isUMount ? `${heightU}U ${zh ? '高度' : 'Height'}` : (zh ? '非标准 U 位' : 'Non-U placement')}
                                </span>
                              </div>
                              <div className="flex items-center gap-2">
                                <button
                                  type="button"
                                  onClick={() => {
                                    if (isUMount && startU > 1) {
                                      handleDeviceMove(device.id, startU - 1);
                                    }
                                  }}
                                  disabled={!isUMount || startU <= 1}
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    if (isUMount && layout && endU < layout.total_u) {
                                      handleDeviceMove(device.id, startU + 1);
                                    }
                                  }}
                                  disabled={!isUMount || !layout || endU >= layout.total_u}
                                  className="px-2.5 py-1.5 rounded-lg text-xs font-bold border disabled:opacity-35 hover:bg-cyan-500/10 transition-colors"
                                  style={{ borderColor: 'var(--card-border)', color: 'var(--body-text)' }}
                            ...
                            <div className="grid grid-cols-2 gap-x-2.5 gap-y-2 text-[11px]">
                              ...
                              <div>
                                <label className="block text-[10px] font-semibold mb-0.5" style={{ color: 'var(--muted-text)' }}>{zh ? '位置' : 'Placement'}</label>
                                <div className="px-2.5 py-1.5 rounded border" style={{ background: 'var(--app-hover-bg)', borderColor: 'var(--card-border)', color: 'var(--body-text)' }}>{placementLabel}</div>
                              </div>
                              <div>
                              ...
                              <div>
                                <label className="block text-[10px] font-semibold mb-0.5" style={{ color: 'var(--muted-text)' }}>{zh ? '安装方式' : 'Mount kind'}</label>
                                <div className="px-2.5 py-1.5 rounded border" style={{ background: 'var(--app-hover-bg)', borderColor: 'var(--card-border)', color: 'var(--body-text)' }}>{device.mount_kind || 'u_mount'}</div>
                              </div>
                              <div className="col-span-2">
                                <label className="block text-[10px] font-semibold mb-0.5" style={{ color: 'var(--muted-text)' }}>{zh ? '位置说明' : 'Location note'}</label>
                                <div className="px-2.5 py-1.5 rounded border" style={{ background: 'var(--app-hover-bg)', borderColor: 'var(--card-border)', color: 'var(--body-text)' }}>{device.location_note || '—'}</div>
                              </div>
                            </div>
                      ...
                      <div className="p-4 border-t" style={{ borderColor: 'var(--card-border)' }}>
                        {viewMode === '2d' ? (
                          ...
                          <button
                            type="button"
                            onClick={() => setViewMode('2d')}
                            className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-cyan-600 px-3 py-2 text-[11px] font-semibold text-white hover:bg-cyan-700"
                          >
                            <Columns3 size={12} /> {zh ? '在 2D 中查看当前设备' : 'View current device in 2D'}
                          </button>
                        )}
                      </div>
                    </motion.div>
        ...
        {showDeviceList && viewMode !== 'overview' && (
          <div className="hidden w-64 flex-shrink-0 rounded-xl overflow-hidden md:flex md:flex-col" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
            {!layout ? (
              <div className="flex-1 flex items-center justify-center" style={{ color: 'var(--muted-text)' }}>
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto p-2.5 space-y-1.5">
                  {layout.devices.length === 0 ? (
                    ...
                    const rackDevice = rackVM?.devices.find(item => item.id === d.id);
                    const isSelected = selectedDeviceId === d.id;
                    const isUMount = (d.mount_kind || 'u_mount') === 'u_mount';
                    const placement = isUMount
                      ? `U${d.start_u ?? '—'}${(d.height_u ?? d.u_height ?? 1) > 1 && d.start_u != null ? `–U${d.start_u + (d.height_u ?? d.u_height ?? 1) - 1}` : ''}`
                      : `${d.mount_kind || 'unknown'} / ${d.position || 'unknown'}`;
                    return (
                      <button
                        key={d.id}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-[10px]" style={{ color: 'var(--muted-text)' }}>
                            <span className={!isUMount ? 'text-amber-600 font-semibold' : undefined}>{placement}</span>
                            <span>•</span>
                        ...
                        <span
                          ...
                          title={rackDevice
                            ? `${d.status === 'active' ? (zh ? '电源：通电' : 'Power: on') : d.status === 'planned' ? (zh ? '电源：规划' : 'Power: planned') : (zh ? '电源：断电' : 'Power: off')} · ${rackDevice.healthStatus === 'healthy' ? (zh ? '健康：正常' : 'Health: normal') : rackDevice.healthStatus === 'warning' ? (zh ? '健康：警告' : 'Health: warning') : rackDevice.healthStatus === 'critical' ? (zh ? '健康：严重' : 'Health: critical') : rackDevice.healthStatus === 'offline' ? (zh ? '健康：离线' : 'Health: offline') : (zh ? '健康：无遥测' : 'Health: unknown')}`
                            : undefined}
                        >
                          <span
                            className="h-2 w-2 rounded-full"
                            style={{
                              background: d.status === 'active' ? '#4ade80' : d.status === 'planned' ? '#fbbf24' : '#ef4444',
                              boxShadow: d.status === 'active' ? '0 0 5px rgba(74, 222, 128, 0.75)' : undefined,
                            }}
                          />
                          {rackDevice && (
                            <span
                              className="absolute bottom-0 right-0 h-1.5 w-1.5 rounded-full border border-white/80"
                              style={{
                                background: rackDevice.healthStatus === 'healthy'
                                  ? '#34d399'
                                  : rackDevice.healthStatus === 'warning'
                                    ? '#fbbf24'
                                    : rackDevice.healthStatus === 'critical'
                                      ? '#f87171'
                                      : rackDevice.healthStatus === 'offline'
                                        ? '#64748b'
                                        : '#94a3b8',
                              }}
                            />
                ...
                {selectedAsset && (
                  <div className="space-y-3 pt-2 border-t" style={{ borderColor: 'var(--card-border)' }}>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={e => {
                          ...
                          const selectedType = deviceTypes.find(dt => dt.id === device_type_id);
                          const mount_kind = selectedType?.default_mount_kind || 'u_mount';
                          const position = defaultPositionForMountKind(mount_kind);
                          const start_u = mount_kind === 'u_mount'
                            ? resolveStartUForInstall(device_type_id, position)
                            : 1;
                          setDeviceForm({ ...deviceForm, device_type_id, mount_kind, position, start_u });
                        }}
                        className="mt-1 w-full rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 ring-cyan-500/30"
                        style={{ background: 'var(--app-hover-bg)', color: 'var(--body-text)', border: '1px solid var(--card-border)' }}
                      >
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      <div>
                        <label className="text-[11px] font-semibold" style={{ color: 'var(--muted-text)' }}>{zh ? '安装方式' : 'Mount kind'}</label>
                        <select
                          value={deviceForm.mount_kind}
                          onChange={e => {
                            const mount_kind = e.target.value;
                            const position = defaultPositionForMountKind(mount_kind);
                            const start_u = mount_kind === 'u_mount'
                              ? resolveStartUForInstall(deviceForm.device_type_id, position)
                              : 1;
                            setDeviceForm({ ...deviceForm, mount_kind, position, start_u });
                          }}
                          className="mt-1 w-full rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 ring-cyan-500/30"
                          style={{ background: 'var(--app-hover-bg)', color: 'var(--body-text)', border: '1px solid var(--card-border)' }}
                        >
                          <option value="u_mount">{zh ? '标准 U 位' : 'Standard U'}</option>
                          <option value="zero_u">0U</option>
                          <option value="side_mount">{zh ? '侧挂' : 'Side mount'}</option>
                          <option value="floor">{zh ? '落地' : 'Floor-standing'}</option>
                          <option value="unknown">{zh ? '待确认' : 'Unknown'}</option>
                        </select>
                      </div>
                      <div>
                        <label className="text-[11px] font-semibold" style={{ color: 'var(--muted-text)' }}>{zh ? '起始U位' : 'Start U'}{deviceForm.mount_kind === 'u_mount' ? ' *' : ''}</label>
                        <input
                          type="number"
                          min={1}
                          ...
                          disabled={deviceForm.mount_kind !== 'u_mount'}
                          className="mt-1 w-full rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 ring-cyan-500/30"
                          style={{ background: 'var(--app-hover-bg)', color: 'var(--body-text)', border: '1px solid var(--card-border)' }}
                        />
                      </div>
                      <div>
                        <label className="text-[11px] font-semibold" style={{ color: 'var(--muted-text)' }}>{zh ? '位置' : 'Position'}</label>
                        <select
                          value={deviceForm.position}
                          onChange={e => {
                            const position = e.target.value;
                            const start_u = deviceForm.mount_kind === 'u_mount'
                              ? resolveStartUForInstall(deviceForm.device_type_id, position)
                              : deviceForm.start_u;
                        ...
                        >
                          ...
                          <option value="full_depth">{zh ? '全深度' : 'Full depth'}</option>
                          <option value="left_side">{zh ? '左侧挂' : 'Left side'}</option>
                          <option value="right_side">{zh ? '右侧挂' : 'Right side'}</option>
                          <option value="unknown">{zh ? '待确认' : 'Unknown'}</option>
                        </select>
                      </div>
                      <div>
                    ...
                    {deviceForm.mount_kind !== 'u_mount' && (
                      <div>
                        <label className="text-[11px] font-semibold" style={{ color: 'var(--muted-text)' }}>
                          {zh ? '位置说明' : 'Location note'}{deviceForm.mount_kind === 'floor' ? ' *' : ''}
                        </label>
                        <input
                          value={deviceForm.location_note}
                          onChange={e => setDeviceForm({ ...deviceForm, location_note: e.target.value })}
                          placeholder={zh ? '例如：机柜底部左侧 / 后门 PDU 导轨' : 'e.g. lower-left floor area / rear-door PDU rail'}
                          className="mt-1 w-full rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 ring-cyan-500/30"
                          style={{ background: 'var(--app-hover-bg)', color: 'var(--body-text)', border: '1px solid var(--card-border)' }}
                        />
                        <p className="mt-1 text-[10px] text-amber-600 dark:text-amber-400">
                          {zh ? '非标准 U 位不会占用 U 容量，也不会被强行绘制到 U1。' : 'Non-U equipment does not consume U capacity or get coerced into U1.'}
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <div className="pt-3 border-t" style={{ borderColor: 'var(--card-border)' }}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input type="number" min={0} max={20} value={dtForm.u_height} onChange={e => setDtForm({ ...dtForm, u_height: Math.max(0, parseInt(e.target.value) || 0) })} className="w-16 rounded-lg px-2 py-1.5 text-xs outline-none" style={{ background: 'var(--app-hover-bg)', color: 'var(--body-text)', border: '1px solid var(--card-border)' }} />
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <select value={dtForm.default_mount_kind} onChange={e => setDtForm({ ...dtForm, default_mount_kind: e.target.value })} className="rounded-lg px-2 py-1.5 text-xs outline-none" style={{ background: 'var(--app-hover-bg)', color: 'var(--body-text)', border: '1px solid var(--card-border)' }}>
                    <option value="u_mount">{zh ? '默认：标准 U 位' : 'Default: standard U'}</option>
                    <option value="zero_u">{zh ? '默认：0U' : 'Default: 0U'}</option>
                    <option value="side_mount">{zh ? '默认：侧挂' : 'Default: side mount'}</option>
                    <option value="floor">{zh ? '默认：落地' : 'Default: floor-standing'}</option>
                    <option value="unknown">{zh ? '默认：待确认' : 'Default: unknown'}</option>
                  </select>
                </div>
              </div>
              {formError && <p className="text-xs text-red-500 mt-2">{formError}</p>}
