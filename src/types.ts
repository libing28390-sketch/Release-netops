...
/** Concrete command/parser profile selected for automation.
 *
 * This is intentionally separate from ``Device.version``: the latter is the
 * software release reported by SNMP/CLI, while this profile describes the
 * command catalog and TextFSM/parser adaptation used by NetOps.
 */
export interface DevicePlatformBinding {
  id: string;
...
export interface Device {
  ...
  platform_binding?: DevicePlatformBinding | null;
  ...
  /** Device-specific SSH compatibility policy; defaults to auto. */
  ssh_algorithm_profile?: 'auto' | 'modern' | 'legacy_compat' | 'legacy_break_glass';
...
export interface NotificationChannels {
  ...
  email?: NotificationChannel & { active_enabled?: boolean; recovery_enabled?: boolean };
}
...
export interface AlertDetailResponse {
  ...
  deliveries?: {
    items: AlertDeliveryAttempt[];
    summary?: Record<string, AlertDeliverySummary>;
  };
}
...
export interface AlertDeliveryAttempt {
  ...
  event_kind: string;
  attempt_no: number;
  status: 'queued' | 'sending' | 'retrying' | 'succeeded' | 'failed' | 'skipped' | string;
  error_code?: string;
...
export type AlertNotificationChannel = 'workspace' | 'feishu' | 'dingtalk' | 'wechat' | 'email' | 'global_webhook';
