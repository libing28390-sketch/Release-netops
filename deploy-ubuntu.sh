...
# 验证 license_auth 模块可以正常加载
info "验证 license_auth 模块..."
if "$VENV_DIR/bin/python" -c "
import sys
sys.path.insert(0, 'backend')
from license_auth.verifier import LicenseVerifier
from license_auth.crypto import verify_signature
from license_auth.middleware import LicenseMiddleware
" 2>/dev/null; then
    ok "license_auth 模块验证通过"
else
    fail "license_auth 模块加载失败，请检查依赖环境与 Python 版本。"
fi
...
# ============================================================
info "安装 Node.js 依赖..."
npm install --prefer-offline 2>&1 | tail -5 || npm install 2>&1 | tail -10 || fail "npm install 失败，请检查网络或 package.json"
ok "Node.js 依赖安装完成"
...
# 8. 生产环境安全与精简清理
# ============================================================
info "清理生产环境冗余测试与开发文档文件..."
rm -rf "$PROJECT_DIR/backend/tests" "$PROJECT_DIR/brain" "$PROJECT_DIR/scratch" "$PROJECT_DIR/skills" "$PROJECT_DIR/reports" "$PROJECT_DIR/docs" 2>/dev/null || true
ok "已安全移除后端单元测试及各类排障草稿目录"
find "$PROJECT_DIR" -maxdepth 2 -type f -name "*.md" ! -name "README.md" ! -name "DEPLOY.md" -exec rm -f {} + 2>/dev/null || true
ok "已安全清理冗余架构与规划文档 (保留 README.md 与 DEPLOY.md)"
