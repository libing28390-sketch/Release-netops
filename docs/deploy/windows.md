...
Windows Nexora 可以连接同机或局域网内 Docker Redis。将仓库中的 `docker-compose.redis.yml` 复制/保留在部署目录，在 PowerShell 中启动 Redis：
...
同机运行且使用本仓库 Redis Compose sidecar 时，在根目录 `.env` 设置：
