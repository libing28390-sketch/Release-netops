...
脚本默认将 `docker-compose.redis.yml` 合并进同一 Compose 项目，因此会一并校验和启动 Redis；首次安装在使用 sidecar 默认地址时会生成 URL-safe 密码。设置 `NETOPS_REDIS_ENABLED=0` 可关闭 sidecar。
...
该命令会自动合并 Redis sidecar，并使用 `--no-build --pull never` 启动所有已启用服务。
