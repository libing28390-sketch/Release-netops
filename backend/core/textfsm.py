...
def resolve_textfsm_parser_platform(platform: str | None, version: str | None = None) -> str | None:
    ...
    concrete V5/V7/V9 grammar is selected directly from the device profile,
    ...
    """
    raw = str(platform or '').strip().lower()
    if raw in {'h3c_comware_v5', 'h3c_comware_v7', 'h3c_comware_v9'}:
        return raw
    ...
    match = re.search(r'(?<!\d)([579])(?:\.|\b)', str(version or '').strip().lower())
    if match:
...
TEMPLATE_DEFAULT_SAMPLES = {
    'huawei_vrp5_display_memory_usage.textfsm': (
    ...
    'huawei_vrp5_display_cpu_usage.textfsm': (
    ...
    'huawei_vrp5_display_fan.textfsm': (
...
def list_template_suggestions(platform: str, query: str, limit: int = 8) -> list[dict[str, Any]]:
    ...
    if requested_h3c_version:
        templates = [
            ...
            or str(item.get('version') or '').strip().lower() in {requested_h3c_version, 'common'}
        ]
...
def _parse_template_filename(filename: str) -> dict[str, Any] | None:
    ...
    known_platforms = [
        ...
        'h3c_comware_unknown', 'h3c_comware_v5', 'h3c_comware_v7', 'h3c_comware_v9', 'h3c_comware',
