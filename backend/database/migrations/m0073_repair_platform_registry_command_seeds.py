...
def _template_version_id(cursor, parser_platform: str, command: str) -> str | None:
    expected = f"{parser_platform}_{command.replace(' ', '_')}.textfsm".lower()
    rows = cursor.execute(
    ...
    for row in rows:
        if str(row[1] or "").lower() == expected:
            return str(row[0])
    return None
...
def upgrade(cursor, use_pg: bool) -> None:
    ...
    for release_id, parser_platform in releases:
        commands = PLATFORM_ACTION_COMMANDS.get(str(parser_platform), {})
        for action_code, command in commands.items():
            ...
            template_version_id = _template_version_id(cursor, str(parser_platform), command)
            if existing:
