...
def upgrade(cursor, use_pg: bool) -> None:
    ...
    if use_pg:
        # Keep the existing table as a compatibility source, while new
        # PostgreSQL installations write to a range-partitioned authoritative
        # table. Partitions are created by the maintenance service as months
        # roll over; the current and next month are provisioned here.
        cursor.execute(
        ...
        # SQLite keeps the same logical contract and uses the indexed source
        # table; the rollup tables below remain portable for tests and local
        # deployments.
        cursor.execute("CREATE TABLE IF NOT EXISTS wan_link_samples_1m_partitioned AS SELECT * FROM wan_link_samples_1m WHERE 1 = 0")
