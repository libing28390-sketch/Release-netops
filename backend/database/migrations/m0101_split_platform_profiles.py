...
def _ensure_system_profiles(cursor, now: str) -> dict[str, str]:
    ...
    for profile in SYSTEM_PROFILES:
        ...
        desired_release_id = f"system-release-{profile_code}-v1"
        cursor.execute(
        ...
        if not release_row:
            raise RuntimeError(f"System release seed was not materialized for profile {profile_code}")
        release_id = str(release_row[0])
        cursor.execute(
            "UPDATE platform_profiles SET current_release_id = ? WHERE id = ?",
...
def _import_scoped_file_templates(cursor, profile_ids: dict[str, str], now: str) -> None:
    ...
    for path in sorted(root.glob("*.textfsm"), key=lambda item: item.name.lower()):
        ...
        source_prefix = next(
            (
                prefix for prefix in ("h3c_comware_v5", "h3c_comware_v9", profile_code)
                if filename_stem.lower().startswith(f"{prefix.lower()}_")
            ),
            profile_code,
        )
