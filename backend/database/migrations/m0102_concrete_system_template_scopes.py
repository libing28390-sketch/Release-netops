...
def _profile_for_template(profile_defs: list[dict], filename: str) -> dict | None:
    ...
    for prefix, profile_code in variant_profiles.items():
        if stem.startswith(f"{prefix}_"):
            return next(
                (item for item in profile_defs if item["platform_code"] == profile_code),
                None,
            )
    return next(
        (
...
def _scoped_identity(profile_code: str, source_filename: str) -> tuple[str, str, str]:
    ...
    # The canonical H3C BGP unicast grammar is a shared family input even
    # though its file carries the parser-family prefix.  Keep the original
    # basename in the scoped identity, but add the concrete Profile prefix so
    # it cannot be confused with the V5/V9 variants during registry review.
    shared_h3c_family = (
        profile_code.lower() == "h3c_comware"
        and stem.lower() == "h3c_comware_display_bgp_peer_ipv4_unicast"
    )
    if stem.lower().startswith(f"{profile_code.lower()}_") and not shared_h3c_family:
        scoped_stem = stem
    else:
...
def _allocate_scoped_identity(
    ...
    conflict_sql = """SELECT id, source_filename
           FROM parser_templates
           WHERE tenant_id IS NULL AND source = 'SYSTEM'
             AND platform_profile_id = ? AND platform_code = ?
             AND template_code = ?"""
    conflict_params: list[object] = [profile_id, parser_platform, scoped_code]
    if exclude_template_id:
        conflict_sql += " AND id <> ?"
        conflict_params.append(exclude_template_id)
    conflict_rows = cursor.execute(conflict_sql, tuple(conflict_params)).fetchall()
    ...
    candidate_sql = """SELECT 1
           ...
           WHERE tenant_id IS NULL AND source = 'SYSTEM'
             ...
             AND template_code = ?"""
    while True:
        candidate_params: list[object] = [profile_id, parser_platform, candidate]
        if exclude_template_id:
            candidate_sql_with_exclusion = candidate_sql + " AND id <> ?"
            candidate_params.append(exclude_template_id)
        else:
            candidate_sql_with_exclusion = candidate_sql
        if not cursor.execute(candidate_sql_with_exclusion, tuple(candidate_params)).fetchone():
            break
        counter += 1
...
def _scope_templates(cursor, profile_defs: list[dict], profile_ids: dict[str, str], now: str) -> None:
    ...
    for (_filename, _parser_platform, _template_code), group in groups.items():
        ...
        target_profile_ids = {
            str(profile_ids.get(code) or "") for code in target_codes
        }
        existing_by_profile = {
            str(row[1]): row for row in group if row[1] and str(row[1]) in target_profile_ids
        }
        ...
        for target in targets:
            ...
            if not profile_id:
                continue
            base_scoped_filename, _, _ = _scoped_identity(
                profile_code,
                str(source_row[4] or ""),
            )
            existing = existing_by_profile.get(profile_id)
            if existing and str(existing[4] or "").lower() not in {
                str(source_row[4] or "").lower(),
                base_scoped_filename.lower(),
            }:
                existing = None
            if not existing:
            ...
            scoped_filename, scoped_code, scoped_name = _allocate_scoped_identity(
                ...
                str(source_row[2]),
                str(source_row[4] or ""),
                exclude_template_id=str(existing[0]) if existing else None,
            )
            if existing:
                cursor.execute(
