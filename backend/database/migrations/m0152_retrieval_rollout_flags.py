"""Add get_startup_config action definition and TFTP backup metadata.

Version 152 was published first as this migration.  Keep that identity
immutable: a later branch accidentally reused version 152 for retrieval
rollout flags, so the historical name is retained here and the new feature
is repaired by migration 156.
"""
...
# Databases created from the short-lived conflicting branch may have recorded
# the replacement name at version 152.  Treat it as a known historical alias;
# migration 156 separately ensures the retrieval table exists.
LEGACY_NAMES = ("retrieval_rollout_flags",)
