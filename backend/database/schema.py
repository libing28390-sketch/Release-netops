...
SUPPORTED_POSTGRES_MAJOR_VERSION = 18


def _require_supported_postgres_version(cursor) -> None:
    """Fail before schema writes when the connected server is off baseline."""
    cursor.execute("SHOW server_version_num")
    row = cursor.fetchone()
    try:
        version_num = int(row[0])
    except (TypeError, ValueError, IndexError) as exc:
        raise RuntimeError("Unable to determine the PostgreSQL server version.") from exc

    major_version = version_num // 10000
    if major_version != SUPPORTED_POSTGRES_MAJOR_VERSION:
        raise RuntimeError(
            "Nexora requires PostgreSQL 18. "
            f"The connected server is PostgreSQL {major_version}; upgrade the database cluster "
            "with pg_upgrade or pg_dump/pg_restore before starting this application."
        )

def _ensure_initial_administrator(cursor, now: str) -> None:
    """Ensure the default administrator exists without overwriting valid passwords."""
    admin = cursor.execute(
        'SELECT id, password, tenant_id, role_id FROM users WHERE username = %s', ('admin',)
    ).fetchone()
    ...
    if admin is None:
        ...
        cursor.execute(
            ...
            (str(uuid.uuid4()), 'admin', hashed, 'Administrator', 'active', now, 'tenant-default', 'role-admin'),
        )
        logger.info('[Database] Default administrator created')
        return

    try:
        admin_id, stored_password = admin['id'], admin['password']
        tenant_id, role_id = admin['tenant_id'], admin['role_id']
    except (TypeError, IndexError):
        admin_id, stored_password, tenant_id, role_id = admin[0], admin[1], admin[2], admin[3]

    # Existing installations can contain a default admin created before tenant
    # ownership was required. Repair only empty fields; preserve explicit scope.
    if not str(tenant_id or '').strip() or not str(role_id or '').strip():
        cursor.execute(
            """
            UPDATE users
               SET tenant_id = CASE
                       WHEN tenant_id IS NULL OR BTRIM(tenant_id) = '' THEN %s
                       ELSE tenant_id
                   END,
                   role_id = CASE
                       WHEN role_id IS NULL OR BTRIM(role_id) = '' THEN %s
                       ELSE role_id
                   END
             WHERE id = %s
            """,
            ('tenant-default', 'role-admin', admin_id),
        )

    # The login endpoint still accepts legacy plaintext credentials. Preserve
    # those and valid bcrypt hashes; repair only empty or malformed bcrypt data.
    is_bcrypt_hash = isinstance(stored_password, str) and re.fullmatch(
        r'\$2[ab]\$\d{2}\$[./A-Za-z0-9]{53}', stored_password
    ) is not None
    has_bcrypt_prefix = isinstance(stored_password, str) and stored_password.startswith(('$2a$', '$2b$'))
    if stored_password and (not has_bcrypt_prefix or is_bcrypt_hash):
        return
    ...
    logger.warning('[Database] Repaired the default administrator password')

def _table_columns(cursor, table_name: str) -> set[str]:
    """Return PostgreSQL column names for *table_name*."""
    # The application and PostgreSQL test driver both use search_path-scoped
    # schemas.  Without the schema predicate, a same-named table in public (or
    # another isolated test schema) can make bootstrap skip a required column.
    where = "table_schema = current_schema() AND " if _USE_PG else ""
    cursor.execute(
        f"SELECT column_name FROM information_schema.columns WHERE {where}table_name = %s",
        (table_name,),
    )
    ...
    try:
        if _USE_PG and hasattr(conn, 'autocommit'):
            try:
                ...
                # A transaction-scoped PostgreSQL test connection may already
                # have an active savepoint.  DDL remains transactional and the
                # idempotent bootstrap can safely run inside that boundary;
                # never hide unrelated connection failures.
                if "transaction" not in str(autocommit_error).lower():
                    raise
                logger.debug("Keeping the existing PostgreSQL transaction for database bootstrap")

        if _USE_PG:
            with conn.cursor() as cursor:
                _require_supported_postgres_version(cursor)
...
def _init_db_logic(conn, *, allow_legacy_import: bool = True):
    ...
    # Scope metadata used by the vendor/platform/model/version resolver. The
    # additive migration owns rollout, but bootstrap remains tolerant while a
    # database is catching up.
    ensure_column('snmp_metric_profiles', 'platform_key', "TEXT NOT NULL DEFAULT ''")
    ensure_column('snmp_metric_profiles', 'platform_scope_json', "TEXT NOT NULL DEFAULT '[]'")
    ensure_column('snmp_metric_profiles', 'model_pattern', "TEXT NOT NULL DEFAULT ''")
    ensure_column('snmp_metric_profiles', 'software_version_scope_json', "TEXT NOT NULL DEFAULT '[]'")
    ensure_column('snmp_metric_profiles', 'min_version', "TEXT NOT NULL DEFAULT ''")
    ensure_column('snmp_metric_profiles', 'max_version', "TEXT NOT NULL DEFAULT ''")
    ensure_column('snmp_metric_profiles', 'excluded_versions_json', "TEXT NOT NULL DEFAULT '[]'")
    ensure_column('snmp_metric_profiles', 'scope_priority', "INTEGER NOT NULL DEFAULT 0")
    cursor.execute(
    ...
    CREATE TABLE IF NOT EXISTS interfaces (
        ...
        CONSTRAINT interfaces_monitoring_role_chk CHECK (
            interface_role IN (
                'uplink', 'wan', 'transit', 'server', 'storage',
                'management', 'access', 'loopback', 'other', 'unclassified'
            )
        ),
        CONSTRAINT interfaces_monitoring_utilization_order_chk CHECK (
    ...
    ensure_column('interfaces', 'interface_role', "TEXT NOT NULL DEFAULT 'unclassified'")
    ensure_column('interfaces', 'utilization_warn_pct', 'NUMERIC(5,2) NOT NULL DEFAULT 60')
    ensure_column('interfaces', 'utilization_high_pct', 'NUMERIC(5,2) NOT NULL DEFAULT 75')
    ensure_column('interfaces', 'utilization_critical_pct', 'NUMERIC(5,2) NOT NULL DEFAULT 90')
    cursor.execute(
    ...
    if _USE_PG:
        ...
        CREATE TABLE IF NOT EXISTS racks (
            ...
            mount_policy VARCHAR(20) NOT NULL DEFAULT 'front_rear',
    ...
    # Current baseline for outbound probe targets. m0024/m0025 remain
    # idempotent for deployed databases and continue to seed default targets.
    cursor.execute('''
    ...
    ''')
    ensure_column('outbound_probe_targets', 'tenant_id', "TEXT NOT NULL DEFAULT 'tenant-default'")
    cursor.execute(
        'CREATE INDEX IF NOT EXISTS idx_outbound_probe_targets_tenant_id '
        'ON outbound_probe_targets(tenant_id)'
    )

    # Durable alert delivery queue; the tracked migration creates the same
    # additive definition for installations upgraded from an older baseline.
    cursor.execute('''
    ...
    ensure_column('physical_assets', 'winrm_enabled', 'BOOLEAN NOT NULL DEFAULT FALSE')
    ensure_column('physical_assets', 'winrm_port', 'INTEGER NOT NULL DEFAULT 5986')
    ensure_column('physical_assets', 'winrm_auth_mode', "TEXT NOT NULL DEFAULT 'ntlm'")
    ensure_column('physical_assets', 'winrm_tls_mode', "TEXT NOT NULL DEFAULT 'pin_fingerprint'")
    ensure_column('physical_assets', 'winrm_cert_fingerprint', "TEXT NOT NULL DEFAULT ''")
    ensure_column('physical_assets', 'ssh_algorithm_profile', "TEXT NOT NULL DEFAULT 'auto'")
    ...
    ensure_column('devices', 'ssh_algorithm_profile', "TEXT NOT NULL DEFAULT 'auto'")
    ...
    # PostgreSQL foreign-key cascades preserve the expected cleanup behavior.
    ...
    _ensure_initial_administrator(cursor, now)
