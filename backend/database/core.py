...
def _make_raw_connection():
    if _USE_PG:
        ...
        try:
            ...
            # 池已满：创建临时溢出连接，避免突发并发（如批量巡检）直接失败。
            # 与 SQLite 路径行为对齐；溢出连接用完即关，不占用池容量。
            logger.warning(
                "PG pool exhausted (max=%d). Creating overflow connection.", _POOL_SIZE
            )
            raw = _pg_connect(dsn=_DATABASE_URL, **_pg_kwargs)
            overflow = True
