...
def _graph_rows(conn: Any, tenant_id: str, view: str, site_id: str | None, limit: int, include_stale: bool) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    view_relation_types = {
    ...
    # Older rolling-upgrade databases may not have the devices table yet, so
    # retain the additive migration's existing graceful fallback.
    try:
        current_device_ids = {
            str(row["id"])
...
def get_graph(
    *,
    ...
    with get_db_connection() as conn:
        nodes, edges = _graph_rows(conn, tenant_id, view, site_id, safe_limit, include_stale)
        graph = {"nodes": nodes, "edges": edges}
        groups: list[dict[str, Any]] = []
        group_members: list[dict[str, Any]] = []
        try:
            group_rows = conn.execute(
                "SELECT id, node_id, group_type, display_name, status, metadata_json FROM topology_groups WHERE tenant_id = ? AND status = 'active' LIMIT ?",
...
def get_edge_evidence(edge_id: str, *, tenant_id: str = "tenant-default", limit: int = 200) -> list[dict[str, Any]]:
    with get_db_connection() as conn:
        rows = conn.execute(
            """
            ...
            """,
            (tenant_id, edge_id, max(1, min(int(limit), 1000))),
        ).fetchall()
        result = []
        for row in rows:
            item = dict(row)
...
def get_topology_history(
    *,
    ...
    if edge_id:
        clauses.append("(entity_id = ? OR entity_type = 'edge' AND entity_id = ?)")
    ...
    with get_db_connection() as conn:
        rows = conn.execute(
            "SELECT id, tenant_id, entity_type, entity_id, event_type, before_json, after_json, source, actor, created_at FROM topology_history WHERE "
