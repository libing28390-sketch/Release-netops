...
from services.neighbor_collection_contract import (
    assert_lldp_command,
...
class _NeighborCollectionBatch(list[dict[str, Any]]):
    """List-compatible LLDP output with an explicit full-snapshot status."""

    def __init__(self, records: list[dict[str, Any]], collection_status: str) -> None:
        super().__init__(records)
        self.collection_status = str(collection_status or "parse_failed")
...
def _link_operational_state(link: dict[str, Any], now: datetime | None = None) -> tuple[str, str]:
    ...
    observed_status = str(link.get('operational_state') or '').lower()
    if status not in {'up', 'down', 'degraded', 'stale'} and observed_status in {'up', 'down', 'degraded'}:
...
def _normalize_interface_oper_status(value: Any, admin_status: Any = None) -> str:
    """Normalize vendor interface state strings for physical-link coloring."""
    status = str(value or '').strip().lower().replace('_', ' ')
    admin = str(admin_status or '').strip().lower().replace('_', ' ')
    if status in {'up', 'up/up', 'online', 'active', 'connected', 'forwarding', '1', 'true'}:
        return 'up'
    if status in {
        'down', 'down/down', 'offline', 'inactive', 'disabled', 'notconnect',
        'not connected', 'lowerlayerdown', '0', 'false',
    } or admin in {'down', 'disabled', 'shutdown', '0', 'false'}:
        return 'down'
    if status in {'degraded', 'testing', 'dormant', 'err-disabled'}:
        return 'degraded'
    return 'unknown'


def _load_interface_oper_status(conn) -> tuple[dict[str, str], dict[tuple[str, str], str]]:
    """Build ID and canonical-name indexes without mutating interface inventory."""
    by_id: dict[str, str] = {}
    by_key: dict[tuple[str, str], str] = {}
    try:
        rows = conn.execute(
            'SELECT id, device_id, interface_name, oper_status, admin_status FROM interfaces'
        ).fetchall()
    except Exception:
        return by_id, by_key
    for row in rows:
        item = dict(row)
        state = _normalize_interface_oper_status(item.get('oper_status'), item.get('admin_status'))
        interface_id = str(item.get('id') or '').strip()
        device_id = str(item.get('device_id') or '').strip()
        interface_name = normalize_interface_name(item.get('interface_name') or '').lower()
        if interface_id:
            by_id[interface_id] = state
        if device_id and interface_name:
            key = (device_id, interface_name)
            # Prefer a concrete state when an inventory contains both a
            # placeholder alias and a populated native-name row.
            previous = by_key.get(key)
            if previous is None or (previous == 'unknown' and state != 'unknown'):
                by_key[key] = state
    return by_id, by_key


def _lookup_interface_oper_status(
    ...
    *,
    ...
    interface_key = str(interface_id or '').strip()
    if interface_key and interface_key in status_by_id:
        return status_by_id[interface_key]
    device_key = str(device_id or '').strip()
    name_key = normalize_interface_name(interface_name or '').lower()
    return status_by_key.get((device_key, name_key), 'unknown') if device_key and name_key else 'unknown'


def _combine_interface_oper_status(left: str, right: str) -> str:
    states = {left, right}
    if 'down' in states:
        return 'down'
...
def strip_domain_suffix(value: str | None) -> str:
    """Strip domain suffix (e.g., .lab.local, .eve.lab, .corp.local) preserving case.

    Leaves IPv4 and IPv6 addresses untouched.
    """
    if not value:
        return ''
...
def _extract_observation_fields(raw: dict[str, Any], protocol: str) -> dict[str, Any]:
    ...
    neighbor_identity = strip_domain_suffix(neighbor_name) or chassis_id
...
def _get_device(device_id: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        collector_credentials = _collector_credentials_for_device(device)
...
def _collector_credentials_for_device(device: dict[str, Any]) -> dict[str, Any]:
    """Resolve and memoize asset-backed collector credentials for one run."""
    cached = device.get('_collector_credentials')
    if isinstance(cached, dict):
        return cached
    resolved = resolve_collector_credentials(device, ssh_role='normal')
    device['_collector_credentials'] = resolved
    return resolved
...
def _match_target_device_with_evidence(
    ...
    rows = conn.execute(
        ...
        SELECT d.id, d.hostname,
               COALESCE(NULLIF(pa.management_ip, ''), NULLIF(d.ip_address, '')) AS ip_address,
               d.sys_name,
               COALESCE(NULLIF(d.sn, ''), NULLIF(pa.serial_number, '')) AS sn,
               d.model, d.site_id
        FROM devices d
    ...
    for row in rows:
        ...
        norm_clean = re.sub(r'[-_\s]', '', normalized_name)
        host_clean = re.sub(r'[-_\s]', '', hostname)
        sys_clean = re.sub(r'[-_\s]', '', sys_name)
        if norm_clean and (norm_clean == host_clean or (sys_clean and norm_clean == sys_clean)):
            add_candidate(device, 0.93, 'system_name_relaxed')
...
def _is_logical_aggregation_interface(row: dict[str, Any]) -> bool:
    ...
    return bool(re.match(
        r'^(?:bridge-aggregation|route-aggregation|link-aggregation|eth-trunk|port-channel|portchannel|bundle-ether|bundleether|aggregateport|aggregate-port|bagg|ragg|po|be|ag|ae|bond|team|aggr|aggregate|trunk|lag)\d+$',
        compact,
    ))
...
def _aggregation_family_key(value: Any) -> tuple[str, str] | None:
    ...
    match = re.match(
        r'^(bridgeaggregation|bagg|linkaggregation|lag|routeaggregation|ragg|ethtrunk|trunk|portchannel|po|bundleether|be|aggregateport|ag|ae|bond|team|aggr|aggregate)(\d+)$',
        compact,
    )
    if not match:
        return None
    prefix, number = match.groups()
    if prefix in {'bridgeaggregation', 'bagg', 'linkaggregation', 'lag'}:
        family = 'bridge'
...
def _synthesise_runtime_aggregation_links(
    ...
    existing_aggregation_pairs: set[tuple[str, str]] = set()
    for item in items:
        ...
        if str(item.get('link_kind') or '').lower() == 'aggregation':
            existing_aggregation_pairs.add(pair)
            continue
    ...
    for pair, group in groups.items():
        if pair in existing_aggregation_pairs:
...
def _sync_aggregation_inventory(device: dict[str, Any], platform_action_session=None) -> dict[str, int]:
    ...
    if device_id:
        conn = get_db_connection()
        try:
            has_lag = conn.execute(
                "SELECT 1 FROM interfaces WHERE device_id = ? AND parent_interface_id IS NOT NULL AND parent_interface_id <> '' LIMIT 1",
                (device_id,)
            ).fetchone()
            if has_lag:
                return {'parents': 0, 'members': 0}
            ...
            for record in parent_records:
                ...
                conn.execute(
                    ...
                    UPDATE interfaces SET parent_interface_id = ?, channel_group = ?,
                        admin_status = 'up', oper_status = ?, aggregation_protocol = ?
                    WHERE id = ?
                    ''',
...
async def _collect_neighbors_with_netmiko(
...
) -> tuple[_NeighborCollectionBatch, str]:
    if device.get('platform_profile_id'):
        ...
        collection_status = classify_lldp_status(
            raw_output=raw_output,
            parsed_count=len(records),
        )
        if parse_status in {"failed", "parse_failed", "device_error"}:
        ...
        if records and len(observations) != len(records):
            collection_status = "parse_failed"
        return _NeighborCollectionBatch(observations, collection_status), 'platform-registry'
    ...
    netmiko_device.update(
        build_netmiko_compatibility_kwargs(
            profile=device.get('ssh_algorithm_profile')
        )
    )

    def _run_commands() -> _NeighborCollectionBatch:
        ...
        with limited_connect_handler(device, ConnectHandler, **netmiko_device) as client:
            for protocol, command in commands:
                ...
                if isinstance(parsed, list) and parsed:
                    ...
                    statuses.append(
                        "success" if len(collected) - before == len(parsed) else "parse_failed"
                    )
                else:
                    ...
                    if regex_entries:
                        statuses.append(
                            "success" if len(collected) - before == len(regex_entries) else "parse_failed"
                        )
                    else:
                        statuses.append(classify_lldp_status(raw_output=raw_text, parsed_count=0))
                    if raw_text and classify_lldp_status(
                        raw_output=raw_text,
                        parsed_count=len(regex_entries),
                    ) in {"truncated", "unsupported_command", "lldp_disabled"}:
                        statuses[-1] = classify_lldp_status(
                            raw_output=raw_text,
                            parsed_count=len(regex_entries),
                        )
                if raw_text and isinstance(parsed, list) and parsed:
                    output_status = classify_lldp_status(
                        raw_output=raw_text,
                        parsed_count=len(parsed),
                    )
                    if output_status in {"truncated", "unsupported_command", "lldp_disabled"}:
                        statuses[-1] = output_status
        if not statuses:
            status = "parse_failed"
        elif all(item in {"success", "no_neighbors"} for item in statuses):
            status = "success" if "success" in statuses else "no_neighbors"
        else:
            status = next(item for item in statuses if item not in {"success", "no_neighbors"})
        return _NeighborCollectionBatch(collected, status)
...
async def _collect_device_observations(
    device: dict[str, Any],
    platform_action_session=None,
    *,
    ...
    # 1. Primary path: Fast, non-intrusive, zero-SSH SNMP LLDP/CDP discovery
    # ``resolve_collector_credentials`` follows the linked physical asset and
    # its SNMP credential binding, then falls back to the legacy asset-local
    # community.  Reading ``devices.snmp_community`` directly here can use a
    # stale copy and bypass an updated credential-center value.
    collector_credentials = _collector_credentials_for_device(device)
    snmp_credentials = collector_credentials.get('snmp') or {}
    snmp_ip = str(snmp_credentials.get('server') or ip or '').strip()
    snmp_community = str(snmp_credentials.get('community') or '').strip()
    snmp_port = int(snmp_credentials.get('port') or 161)
    snmp_version = str(snmp_credentials.get('version') or '2c').strip().lower() or '2c'
    if snmp_community and snmp_ip:
        if attempted_methods is not None:
            attempted_methods.append('snmp')
        try:
            from services.snmp_service import collect_snmp_lldp_neighbors
            snmp_records = await collect_snmp_lldp_neighbors(
                snmp_ip,
                snmp_community,
                port=snmp_port,
            ...
            if snmp_records:
                observations: list[dict[str, Any]] = []
                for item in snmp_records:
                    obs = _extract_observation_fields(item, item.get("protocol", "lldp"))
                    if obs.get("neighbor_name_raw") and obs.get("source_port_raw"):
                        observations.append(obs)
                if observations:
                    logger.info("Discovered %d LLDP/CDP neighbors via SNMP for %s (%s)", len(observations), device.get("hostname"), snmp_ip)
                    return _NeighborCollectionBatch(observations, "success"), "snmp"
            logger.info(
                'SNMP LLDP discovery returned no usable neighbors for %s (%s); '
                'falling back to SSH CLI (SNMP may be unreachable, LLDP/CDP may be disabled, or the MIB may be empty)',
                device.get('hostname'),
                snmp_ip,
            )
        except Exception as snmp_err:
            logger.warning(
                'SNMP LLDP discovery failed for %s (%s), falling back to SSH CLI (error_type=%s)',
                device.get('hostname'),
                snmp_ip,
                type(snmp_err).__name__,
            )
    else:
        logger.info(
            'SNMP LLDP discovery skipped for %s; falling back to SSH CLI (SNMP credential or address unavailable)',
            device.get('hostname'),
        )

    # 2. Secondary path: Fallback to SSH CLI + TextFSM if SNMP produced no observations
    if attempted_methods is not None:
        attempted_methods.append('ssh')
    port = int(device.get('management_port') or device.get('port') or 22)
...
def _replace_device_observations(
    ...
    *,
    ...
    try:
            ...
            if existing:
                conn.execute(
                    ...
                    (
                        ...
                        target_device.get('hostname') if target_device else strip_domain_suffix(observation['neighbor_name_raw']),
        ...
        from services.topology_lldp_snapshot_service import complete_lldp_snapshot_in_transaction
...
def _rebuild_links_from_observations_unlocked() -> int:
    ...
    try:
        ...
        for link_key, bundle in grouped_with_contract.items():
            ...
            if src_dev and src_port_norm:
                if not src_intf_id:
                    ...
                    conn.execute(
                        "UPDATE interfaces SET oper_status = 'up', admin_status = 'up' WHERE id = ? AND (oper_status IN ('down', 'unknown') OR admin_status IN ('down', 'unknown'))",
                        (src_intf_id,)
                    )
            ...
            conn.execute(
                ...
                (
                    ...
                    '+'.join(protocols),
                    confidence,
                    aggregation.get('status') if is_aggregation else ('up' if len(observations) > 0 else 'unknown'),
                    0,
                    len(observations),
                    _safe_json(metadata),
        ...
        active_aggregated_member_keys: set[str] = set()
        for bundle in grouped_with_contract.values():
            aggregation = bundle.get('aggregation') or {}
            for member in aggregation.get('members') or []:
                if isinstance(member, dict):
                    src = member.get('source') or {}
                    tgt = member.get('target') or {}
                    m_key = _build_link_key(
                        str(src.get('device_id') or ''),
                        str(src.get('normalized') or src.get('name') or ''),
                        str(tgt.get('device_id') or ''),
                        str(tgt.get('normalized') or tgt.get('name') or ''),
                    )
                    active_aggregated_member_keys.add(m_key)
        ...
        for link_key, existing in existing_by_key.items():
            ...
            if existing_canonical_key in active_link_keys:
                continue
            # Links that are members of an active aggregation link are superseded, not stale
            if link_key in active_aggregated_member_keys or existing_canonical_key in active_aggregated_member_keys:
                continue
            # Exclude obsolete links generated from port descriptions (e.g. TO-xxx)
            if (
                str(existing.get('source_port_raw') or '').upper().startswith('TO-')
                or str(existing.get('target_port_raw') or '').upper().startswith('TO-')
                or str(existing.get('source_port_normalized') or '').upper().startswith('TO-')
                or str(existing.get('target_port_normalized') or '').upper().startswith('TO-')
            ):
                continue
    ...
    *,
    ...
    try:
        ...
        clauses = [
            ...
            f"NOT {server_device_sql('d')}",
        ]
        params: list[Any] = list(server_device_sql_params())
        ...
        rows = conn.execute(
            f'''
            SELECT d.id, d.hostname, d.ip_address, d.platform, d.device_category, d.role,
        ...
        devices = [dict(row) for row in rows if not is_server_device(dict(row))]
...
async def discover_device_neighbors(
    ...
    *,
    ...
    snapshot_attempt: dict[str, Any] | None = None
    method = 'unknown'
    ...
    try:
        ...
        if not device.get('ip_address'):
            raise RuntimeError('device_ip_missing')
        # Resolve the canonical asset/credential-center contract before the
        # eligibility check as well as before the actual SNMP walk.  Looking
        # only at the legacy devices cache can reject an asset whose SNMP
        # community is configured exclusively in the import template.
        collector_credentials = _collector_credentials_for_device(device)
        snmp_credentials = collector_credentials.get('snmp') or {}
        has_snmp = bool(snmp_credentials.get('configured') or snmp_credentials.get('community'))
        ssh_credentials = collector_credentials.get('ssh') or {}
        has_ssh = bool(ssh_credentials.get('username') and ssh_credentials.get('password'))
        ...
        snapshot_versions = lldp_snapshot_versions(device)
        snapshot_attempt = begin_lldp_snapshot_attempt(device, snapshot_versions)
        snapshot_generation = int(snapshot_attempt.get('generation') or 0)
        if snapshot_generation <= 0:
            raise RuntimeError('topology_lldp_snapshot_generation_unavailable')

        # Fast, non-intrusive discovery: SNMP-first LLDP/CDP, safe fallback to single-command CLI LLDP only if needed
        observations, method = await _collect_device_observations(
            device,
            attempted_methods=attempted_methods,
        )
        aggregation_summary = {'parents': 0, 'members': 0}
        if method != 'snmp' and has_ssh:
            try:
                aggregation_summary = await asyncio.to_thread(_sync_aggregation_inventory, device)
            except Exception:
                pass
        protocol_summary = {
            'success': True,
        ...
        collection_status = str(getattr(observations, 'collection_status', 'parse_failed') or 'parse_failed')
        if collection_status not in {'success', 'no_neighbors'}:
            raise RuntimeError(f'topology_lldp_snapshot_{collection_status}')
        observations = _deduplicate_observations(list(observations))
        match_summary = _replace_device_observations(
            device,
            observations,
            run_id,
        ...
        record_collection_result(
            ...
            transport='snmp' if method == 'snmp' else 'ssh',
        ...
        redis_publish_confirmed = False
        try:
            redis_published = publish_completed_lldp_snapshot(
                device,
                generation=snapshot_generation,
            )
            redis_publish_confirmed = redis_published is True
            if redis_published:
                logger.info(
                    '[netops-cache] event=lldp_snapshot_publish device_id=%s generation=%s '
                    'record_count=%s status=confirmed',
                    device_id,
                    snapshot_generation,
                    len(observations),
                )
                ...
                logger.debug(
                    '[netops-cache] event=lldp_snapshot_publish device_id=%s generation=%s '
                    'record_count=%s status=unavailable',
                    device_id,
                    snapshot_generation,
                    len(observations),
                )
        except Exception as cache_exc:
            # PostgreSQL observations remain authoritative; a Redis projection
            # failure is reported separately from collection completion.
            logger.debug('LLDP Redis snapshot projection skipped (%s)', type(cache_exc).__name__)
        return {
            'device_id': device_id,
            'hostname': device.get('hostname') or '',
            'collection_status': collection_status,
            'redis_publish_confirmed': redis_publish_confirmed,
        ...
        if snapshot_attempt and snapshot_attempt.get('generation'):
            try:
                from services.topology_lldp_snapshot_service import fail_lldp_snapshot_attempt

                fail_lldp_snapshot_attempt(device_id, int(snapshot_attempt['generation']))
            except Exception as state_exc:
                logger.debug('Failed to fence unsuccessful LLDP snapshot attempt (%s)', type(state_exc).__name__)
        record_collection_result(
            device_id,
            'topology_lldp',
            status='failed',
            ...
            transport=attempted_methods[-1] if attempted_methods else '',
        )
        raise
...
async def discover_lldp_neighbors(device_id: str):
    try:
        ...
        # 周期任务中返回脱敏状态，避免隐藏缺失或泄露设备错误细节。
        error_code = topology_error_code(exc)
        status = (
            'skipped'
            if error_code in {'device_not_found', 'device_offline', 'credentials_incomplete', 'platform_unsupported'}
            else 'failed'
        )
        logger.debug('Periodic LLDP discovery %s for device %s (%s)', status, device_id, error_code)
        return {
            'device_id': device_id,
            'status': status,
...
def get_current_links(
    ...
    *,
    ...
    try:
        ...
        now = datetime.now(timezone.utc)
        aggregation_map = _load_interface_aggregation_map(conn)
        interface_status_by_id, interface_status_by_key = _load_interface_oper_status(conn)
        result = []
        for row in rows:
            item = dict(row)
            try:
            ...
            if item.get('link_kind') == 'aggregation' and isinstance(item.get('members'), list):
                member_rows = item['members']
                for member in member_rows:
                    if not isinstance(member, dict):
                        continue
                    source_endpoint = member.get('source') if isinstance(member.get('source'), dict) else {}
                    target_endpoint = member.get('target') if isinstance(member.get('target'), dict) else {}
                    source_state = _lookup_interface_oper_status(
                        interface_status_by_id,
                        interface_status_by_key,
                        device_id=source_endpoint.get('device_id'),
                        interface_name=source_endpoint.get('normalized') or source_endpoint.get('name'),
                    )
                    target_state = _lookup_interface_oper_status(
                        interface_status_by_id,
                        interface_status_by_key,
                        device_id=target_endpoint.get('device_id'),
                        interface_name=target_endpoint.get('normalized') or target_endpoint.get('name'),
                    )
                    source_endpoint['oper_status'] = source_state
                    target_endpoint['oper_status'] = target_state
                    if source_state != 'unknown':
                        source_endpoint['up'] = source_state == 'up'
                    if target_state != 'unknown':
                        target_endpoint['up'] = target_state == 'up'
                concrete_members = [
                    member for member in member_rows
                    if isinstance(member, dict)
                    and isinstance(member.get('source'), dict)
                    and isinstance(member.get('target'), dict)
                    and member['source'].get('oper_status') != 'unknown'
                    and member['target'].get('oper_status') != 'unknown'
                ]
                if member_rows and len(concrete_members) == len(member_rows):
                    item['member_count'] = len(member_rows)
                    item['active_member_count'] = sum(
                        1
                        for member in member_rows
                        if isinstance(member, dict)
                        and isinstance(member.get('source'), dict)
                        and isinstance(member.get('target'), dict)
                        and member['source'].get('up') is True
                        and member['target'].get('up') is True
                    )
                    if item['active_member_count'] == item['member_count']:
                        item['operational_state'] = 'up'
                        item['status'] = 'up'
                    elif item['active_member_count'] > 0:
                        item['operational_state'] = 'degraded'
                        item['status'] = 'degraded'
                    else:
                        item['operational_state'] = 'down'
                        item['status'] = 'down'
            else:
                source_state = _lookup_interface_oper_status(
                    interface_status_by_id,
                    interface_status_by_key,
                    device_id=item.get('source_device_id'),
                    interface_id=item.get('source_interface_id'),
                    interface_name=item.get('source_port_normalized') or item.get('source_port'),
                )
                target_state = _lookup_interface_oper_status(
                    interface_status_by_id,
                    interface_status_by_key,
                    device_id=item.get('target_device_id'),
                    interface_id=item.get('target_interface_id'),
                    interface_name=item.get('target_port_normalized') or item.get('target_port'),
                )
                item_state = _combine_interface_oper_status(source_state, target_state)
                if item_state != 'unknown':
                    item['operational_state'] = item_state
    ...
    try:
        ...
        with get_db_connection() as conn:
            device_rows = conn.execute(
                f"""
                SELECT d.id, d.hostname, d.ip_address, d.site_id, d.role, d.device_category,
                       d.function, d.zone, d.status
                FROM devices d
                WHERE NOT {server_device_sql('d')}
                """,
                server_device_sql_params(),
            ).fetchall()
            ...
            try:
                protocol_rows = conn.execute(
                    ...
                    WHERE is_active = 1 AND COALESCE(protocol, '') NOT IN ('interface_description', 'description')
                    """
                ).fetchall()
            except Exception:
                protocol_rows = []
    ...
    *,
    ...
    try:
        clauses = [
            ...
            "n.device_id <> ''",
            f"EXISTS (SELECT 1 FROM devices d WHERE d.id = n.device_id AND NOT {server_device_sql('d')})",
        ]
        params: list[Any] = list(server_device_sql_params())
        if site_id:
...
def get_unmanaged_neighbors(
    ...
    *,
    ...
    try:
        ...
        if not include_stale:
            clauses.append("COALESCE(o.is_active, 1) = 1")
            clauses.append("COALESCE(o.status, 'active') <> 'stale'")
        params: list[Any] = []
        if site_id:
            clauses.append(
        ...
        for neighbor_key, observations in neighbor_groups.items():
            ...
            display_name = strip_domain_suffix(first.get('neighbor_name_raw') or first.get('neighbor_name_normalized') or 'Unknown')
    ...
    try:
        ...
        device_rows = conn.execute(
            ...
            SELECT d.id,
                   ...
                   d.status, s.tenant_id, d.device_category, d.function, d.platform, d.role
            FROM devices d
            LEFT JOIN physical_assets pa ON pa.id = d.asset_id
            LEFT JOIN sites s ON s.id = COALESCE(NULLIF(pa.site_id, ''), NULLIF(d.site_id, ''))
            ORDER BY d.id
        ...
        device_rows = [row for row in device_rows if not is_server_device(dict(row))]
...
def get_topology_generation_status(
    *,
    ...
    try:
        ...
        obs_clauses = ["COALESCE(o.is_active, 1) = 1", "COALESCE(o.status, 'active') <> 'stale'"]
        if observation_filter:
            obs_clauses.append(observation_filter)
        observation_where = f'WHERE {" AND ".join(obs_clauses)}'
    ...
    return {
        ...
        'warnings': warnings,
        # An LLDP neighbor outside the CMDB is valid external topology
        # evidence. Keep it visible as an informational warning without
        # making the discovery pipeline look unhealthy.
        'health': 'empty' if total_observations == 0 else ('degraded' if blocking_warnings else 'healthy'),
    }
