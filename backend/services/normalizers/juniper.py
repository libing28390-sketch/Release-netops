...
class JuniperNormalizer(BaseNormalizer):
    def parse_device_info(self, device_id: str, raw_version: str) -> Device:
        records = textfsm_records("juniper_junos", "show version", raw_version)
        record = records[0] if records else {}
        hostname = record_value(record, "hostname") or first_match(raw_version, r"^Hostname\s*:\s*(\S+)")
        model = record_value(record, "model") or first_match(raw_version, r"^Model\s*:\s*(\S+)")
        version = record_value(record, "version", "junos") or first_match(raw_version, r"^Junos\s*:\s*(\S+)")
    ...
    def parse_interfaces(self, device_id: str, raw_interfaces: str, raw_ip_brief: str = "") -> list[Interface]:
        ...
        for line in raw_ip_brief.splitlines():
            ...
            if len(parts) < 3 or parts[0].lower() == "interface" or parts[1].lower() not in {"up", "down"}:
                continue
            name, admin, oper = parts[0], parts[1], parts[2]
            ip_value = None
            if "inet" in parts:
                index = parts.index("inet")
                if index + 1 < len(parts):
                    ip_value = clean_ip(parts[index + 1])
        ...
        records = textfsm_records("juniper_junos", "show interfaces detail", raw_interfaces)
        for record in records:
            name = record_value(record, "interface", "physical_interface")
            if name:
                self._merge_detail(interfaces, device_id, name, record)

        header = re.compile(
            r"(?m)^Physical interface:\s*([^,]+),\s*(Enabled|Disabled),\s*Physical link is\s*(\S+)"
        )
        ...
        for index, match in enumerate(matches):
            ...
            record = {
                ...
                "speed": first_match(block, r"\bSpeed:\s*([^,\r\n]+)"),
                "mac_address": first_match(block, r"Current address:\s*([0-9a-f:.-]+)"),
            }
    ...
    def parse_neighbors(self, device_id: str, raw_lldp: str) -> list[Neighbor]:
        records = textfsm_records("juniper_junos", "show lldp neighbors detail", raw_lldp)
    ...
    def _merge_detail(interfaces: dict[str, Interface], device_id: str, name: str, record: dict) -> None:
        ...
        if interface is None:
            ...
            interfaces[key] = interface
        admin = record_value(record, "admin_status", "admin", "link_status")
        oper = record_value(record, "oper_status", "link", "protocol_status")
        if admin:
        ...
        mac = record_value(record, "mac_address", "current_address", "hardware_address")
        if mac:
            interface.mac = mac
        speed = parse_speed_bps(record_value(record, "speed", "link_speed"))
        if speed:
