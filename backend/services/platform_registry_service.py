...
ALLOWED_CONNECTION_DRIVERS = {
    ...
    "raisecom_ros", "hillstone_stoneos",
}
CONNECTION_DRIVER_TO_IMPLEMENTATION = {
    ...
    "raisecom_ros": "netmiko", "hillstone_stoneos": "netmiko",
}
...
ACTION_DEFINITIONS: tuple[dict[str, Any], ...] = (
    ...
    {"action_code": "get_transceivers", "name_zh": "光模块状态", "name_en": "Transceiver Status", "purpose": "光模块采集", "risk": "low", "consumers": ["inspection"], "fields": ["interface"], "optional_fields": ["vendor", "serial", "ifIndex", "entityIndex", "rx_power_dbm", "tx_power_dbm", "temperature_c", "voltage_v", "bias_ma"], "field_types": {"interface": "string", "vendor": "string", "serial": "string", "ifIndex": "integer", "entityIndex": "integer", "rx_power_dbm": "number", "tx_power_dbm": "number", "temperature_c": "number", "voltage_v": "number", "bias_ma": "number"}, "max_records": 2000},
...
SYSTEM_PROFILES: tuple[dict[str, Any], ...] = (
    ...
    {"platform_code": "hillstone_stoneos", "name_zh": "山石 StoneOS", "name_en": "Hillstone StoneOS", "vendor": "Hillstone", "connection_driver": "hillstone_stoneos", "parser_platform": "hillstone_stoneos"},
...
def list_platform_vendor_names(conn=None) -> list[str]:
    """Return vendor names supported by built-in or active platform profiles."""
    owns_connection = conn is None
    connection = get_db_connection() if owns_connection else conn
    vendors: dict[str, str] = {}
    try:
        values = [str(profile.get("vendor") or "").strip() for profile in SYSTEM_PROFILES]
        rows = connection.execute(
            "SELECT DISTINCT TRIM(vendor) AS vendor FROM platform_profiles "
            "WHERE COALESCE(UPPER(status), 'ACTIVE') <> 'ARCHIVED' "
            "AND TRIM(COALESCE(vendor, '')) <> ''"
        ).fetchall()
        values.extend(
            str((row[0] if isinstance(row, tuple) else row["vendor"]) or "").strip()
            for row in rows
        )
        for value in values:
            if value:
                vendors.setdefault(value.casefold(), value)
        return sorted(vendors.values(), key=str.casefold)
    finally:
        if owns_connection:
            connection.close()
...
PLATFORM_ALIASES = {
    ...
    "hillstone": "hillstone_stoneos", "stoneos": "hillstone_stoneos", "山石": "hillstone_stoneos",
}

PLATFORM_CATALOG_METADATA: dict[str, dict[str, str]] = {
    "huawei_vrp": {"catalog_vendor": "huawei", "platform_family": "huawei_vrp", "version": "v5"},
    ...
    "hp_comware": {"catalog_vendor": "h3c", "platform_family": "h3c_comware", "version": "v5"},
    "h3c_comware": {"catalog_vendor": "h3c", "platform_family": "h3c_comware", "version": "v7"},
    "h3c_comware9": {"catalog_vendor": "h3c", "platform_family": "h3c_comware", "version": "v9"},
    ...
    "ruijie_eg_rgos11": {"catalog_vendor": "ruijie", "platform_family": "ruijie_rgos", "version": "v11"},
    "ruijie_s6k_rgos12": {"catalog_vendor": "ruijie", "platform_family": "ruijie_rgos", "version": "v12"},
    ...
    "zte_5900_v6": {"catalog_vendor": "zte", "platform_family": "zte_zxros", "version": "common"},
    "zte_zsrv2_v3": {"catalog_vendor": "zte", "platform_family": "zte_rosng", "version": "common"},
    ...
    "dptech_fw_s211": {"catalog_vendor": "dptech", "platform_family": "dptech_conplat", "version": "common"},
    ...
    "maipu_s3330_v9": {"catalog_vendor": "maipu", "platform_family": "maipu_mypower", "version": "v9"},
    "hillstone_stoneos": {"catalog_vendor": "hillstone", "platform_family": "hillstone_stoneos", "version": "common"},
}
...
PLATFORM_FAMILY_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "hillstone_stoneos": ("hillstone", "stoneos", "hillstone_stoneos"),
...
PLATFORM_ACTION_COMMANDS: dict[str, dict[str, str]] = {
    ...
    "dptech_ios": {"get_version": "show version", "get_lldp_neighbors": "show lldp neighbors", "get_interface_brief": "show interface status", "get_ip_interfaces": "show ip interface brief", "get_arp_table": "show arp all", "get_mac_table": "show mac-address-table", "get_vlan_table": "show vlan", "get_route_table": "show ip route", "get_stp": "show spanning-tree", "get_cpu": "show cpu-usage", "get_memory": "show memory", "get_temperature": "show environment", "get_ntp_status": "show ntp status", "get_bfd_sessions": "show bfd session", "get_logbuffer": "show logging operlog recent", "get_interface_description": "show ip interface brief", "get_clock": "show clock", "get_uptime": "show version", "get_running_config": "show running-config", "get_startup_config": "display saved-configuration"},
    "maipu": {"get_version": "show version", "get_lldp_neighbors": "show lldp neighbors", "get_interface_brief": "show interface brief", "get_ip_interfaces": "show ip interface brief", "get_arp_table": "show arp", "get_mac_table": "show mac address all", "get_vlan_table": "show vlan", "get_route_table": "show ip route", "get_bgp_neighbors": "show ip bgp summary", "get_ospf_neighbors": "show ip ospf neighbor", "get_isis_neighbors": "show isis neighbors", "get_stp": "show spanning-tree", "get_cpu": "show cpu monitor", "get_memory": "show memory utilization", "get_fans": "show system fan", "get_power": "show system power", "get_clock": "show clock", "get_running_config": "show running-config", "get_startup_config": "show startup-config"},
    "hillstone_stoneos": {"get_running_config": "show configuration", "get_startup_config": "show configuration startup"},
}

# Optical DOM is collected through confirmed vendor MIBs. Do not seed or
# publish legacy CLI commands for this action; execution is intercepted by
# the SNMP-only collector below.
for _action_commands in PLATFORM_ACTION_COMMANDS.values():
    _action_commands.pop("get_transceivers", None)
...
def platform_vendors_match(device_vendor: Any, profile_vendor: Any) -> bool:
    """Reject a Profile bound to a different explicit inventory vendor."""
    device_value = str(device_vendor or "").strip()
    if not device_value or device_value.lower() == "other":
        return True
    profile_value = str(profile_vendor or "").strip()
    if not profile_value:
        return False
    from services.config_vendor_platform_service import canonical_vendor

    def normalized(value: str) -> str:
        canonical = canonical_vendor(value)
        return re.sub(r"[^a-z0-9\u4e00-\u9fff]+", "", canonical.lower())

    return normalized(device_value) == normalized(profile_value)
...
def preview_platform_action(
    ...
    *,
    ...
    try:
        row = conn.execute(
            """SELECT d.*, p.id AS profile_id, p.vendor AS profile_vendor, p.platform_code, p.connection_driver, p.parser_platform, p.current_release_id,
        ...
        if not authorize_resource(
            ...
            raise PlatformRegistryError("DEVICE_SCOPE_DENIED", "Device is outside the current resource scope", status_code=403)
        if action_code == "get_transceivers":
            # Older published releases store a CLI command for this action;
            # optical data is now explicitly an SNMP-only operation.
            return {
                "success": True,
                "action_code": action_code,
                "command": None,
                "execution_mode": "snmp",
                "message": "Optical module data is collected through SNMP.",
            }
        if not platform_vendors_match(device.get("vendor"), device.get("profile_vendor")):
            raise PlatformRegistryError(
                "PLATFORM_VENDOR_MISMATCH",
                "Device vendor does not match its bound platform Profile",
                status_code=409,
            )
        if not platform_codes_match(device.get("platform"), device.get("platform_code"), device.get("parser_platform")):
            raise PlatformRegistryError(
...
def _collect_optical_snmp_action(device: dict[str, Any], *, max_records: int = 2000) -> dict[str, Any]:
    """Execute the built-in transceiver action through SNMP only."""
    from services.snmp_optical_service import collect_optical_snmp

    collected = collect_optical_snmp(device)
    records = (collected.get("records") or [])[:max(1, min(2000, int(max_records or 2000)))]
    error_code = str(collected.get("error_code") or "")
    success = bool(collected.get("success"))
    response = {
        "success": success,
        "action_code": "get_transceivers",
        "command": None,
        "source": "snmp",
        "adapter": collected.get("adapter") or {},
        "parser": "snmp-optical",
        "parse_status": (
            "matched" if success and records
            else "no_data" if success
    ...
    if collected.get("error"):
        response["error"] = collected["error"]
    return response
...
def execute_platform_action(
    ...
    *,
    ...
    def _audit(event_type: str, metadata: dict[str, Any], raw_output: str = "") -> None:
        ...
        try:
            if metadata.get("platform_release_id"):
                conn.execute(
                    "INSERT INTO platform_release_audit_logs (id, release_id, event_type, actor_id, actor_username, metadata_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (str(uuid.uuid4()), str(metadata["platform_release_id"]), event_type, user.get("id"), user.get("username"), _json(metadata), _now()),
                )
            conn.commit()
        except Exception:
            conn.rollback()
    ...
    try:
        row = conn.execute(
            """SELECT d.*, p.id AS profile_id, p.vendor AS profile_vendor, p.platform_code, p.connection_driver, p.parser_platform, p.current_release_id, r.checksum AS release_checksum,
                      r.release_number, a.command, a.field_contract_json, a.command_checksum,
                      a.parser_template_version_id,
        ...
        if not authorize_resource(
            ...
            raise PlatformRegistryError("DEVICE_SCOPE_DENIED", "Device is outside the current resource scope", status_code=403)
        if device.get("sensitive_level") == "sensitive" and user.get("role") != "Administrator":
            raise PlatformRegistryError("SENSITIVE_ACTION_FORBIDDEN", f"{action_code} requires Administrator permission", status_code=403)
        if action_code == "get_transceivers":
            # Optical DOM is a vendor MIB read and must not depend on a
            # published CLI release or dispatch a legacy transceiver command.
            response = _collect_optical_snmp_action(device, max_records=device.get("max_records") or 2000)
            response.update({
                "platform_release_id": device.get("current_release_id"),
                "release_number": device.get("release_number"),
                "release_checksum": device.get("release_checksum"),
            })
            _audit(
                "ACTION_EXECUTED",
                {
        ...
        if not is_safe_read_command(resolved_command, device.get("connection_driver") or ""):
            raise PlatformRegistryError("UNSAFE_COMMAND", "Release contains an unsafe command")
        logger.info(
            "[netops-cli] event=platform_action_start device_id=%s action_code=%s "
            "public_platform=%s profile_platform=%s connection_driver=%s "
            "parser_platform=%s release_id=%s release_number=%s "
            "parser_template_version_id=%s command=%r",
            device_id,
            action_code,
            device.get("platform"),
            device.get("platform_code"),
            device.get("connection_driver"),
            device.get("parser_platform"),
            device.get("current_release_id"),
            device.get("release_number"),
            device.get("parser_template_version_id"),
            resolved_command,
        )
        if _session is not None:
            ...
            logger.info(
                "[netops-cli] event=platform_action_dispatch device_id=%s action_code=%s "
                "driver_type=%s connection_driver=%s command=%r",
                device_id,
                action_code,
                driver_type,
                device.get("connection_driver"),
        ...
        if not getattr(result, "success", False):
            logger.warning(
                "[netops-cli] event=platform_action_failed device_id=%s action_code=%s "
                "release_id=%s command=%r result_command=%r result_type=%s error=%s",
                device_id,
                action_code,
                device.get("current_release_id"),
                resolved_command,
                ...
                getattr(result, "error", "Command execution failed"),
            )
            ...
            return response
        logger.info(
            "[netops-cli] event=platform_action_command_success device_id=%s "
            "action_code=%s command=%r output_bytes=%s output_lines=%s",
            device_id,
            action_code,
            resolved_command,
            len(output.encode("utf-8", errors="ignore")),
        ...
        try:
            ...
            logger.info(
                "[netops-cli] event=platform_action_parse_result device_id=%s "
                "action_code=%s parser=%s parser_platform=%s template=%s "
                "template_source=%s parse_status=%s record_count=%s",
                device_id,
                action_code,
                parser_label,
                auto_parse.get("parser_platform"),
                auto_parse.get("template"),
                auto_parse.get("template_source"),
                auto_parse.get("parse_status"),
                len(normalized_records),
            )
        except Exception as parse_exc:
            logger.warning(
                "[netops-cli] event=platform_action_parse_failed action=%s device=%s "
                "parser_platform=%s parser_template_version_id=%s error_type=%s error=%s",
                action_code,
                device_id,
                device.get("parser_platform"),
                device.get("parser_template_version_id"),
