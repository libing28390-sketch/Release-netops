...
from services.snmp_metric_profile_service import HEALTH_METRIC_KEYS, resolve_health_metric_profiles
from services.snmp_discovery_service import (
    discover_device_oid_config,
    load_discovery,
    persist_discovery,
)
...
from services.alert_control_service import AlertState, find_active_silence_for_alert, transition_state
from core.crypto import decrypt_credential

logger = logging.getLogger(__name__)


def _sync_snmp_hostname_projection(device_id: str, raw_sys_name: Any) -> None:
    """Project background SNMP sysName into the linked asset record."""
    ...
    try:
        ...
        sync_result = sync_device_hostname_metadata(conn, str(device_id), raw_sys_name)
        conn.commit()
        schedule_monitoring_target_sync_if_hostname_changed(sync_result)
    except Exception as exc:
        conn.rollback()
        logger.debug("[SNMP] Hostname asset projection failed for %s: %s", device_id, type(exc).__name__)
    finally:
        conn.close()
    ...
    *,
    ...
    if device:
        for field in ("vendor", "platform", "model", "version"):
            current = str(device.get(field) or "").strip().casefold()
            observed = str(discovery.get("identity", {}).get("software_version" if field == "version" else field) or discovery.get(field) or "").strip().casefold()
            if current and observed and current != observed:
                return True
    observed = discovery.get("observed_at")
    try:
...
def _resolved_metrics_from_discovery(discovery: dict[str, Any], device_id: str) -> dict[str, Any]:
    status = str(discovery.get("status") or "pending")
    usable = status in {"matched", "partial"}
    metrics = dict(discovery.get("metrics") or {}) if usable else {}
    interface = dict(discovery.get("interface") or {}) if usable else {}
    return {
        "profile_id": f"discovery:{device_id}:{discovery.get('rule_version') or 'current'}" if usable else "",
        "metrics": metrics,
        "metric_keys": sorted(metrics),
        "metric_sources": {key: "snmp_discovery" for key in metrics},
        "profile_source": "snmp_discovery" if usable else "template_not_applied",
        "interface": interface,
        "interface_source": "snmp_discovery" if interface else "template_not_applied",
        "template_applied": usable and bool(metrics or interface),
        "template_required": True,
        "profile_status": status,
        "profile_binding_status": status,
        "profile_match_reason": str(discovery.get("reason") or ""),
...
def _sync_device_status_tag(device_id: str, status: str) -> None:
    """Synchronize the fixed availability tag without duplicating steady-state writes."""
    conn = get_db_connection()
    try:
        tag_service.sync_device_status_tag(conn, device_id, status)
        conn.commit()
    except Exception:
        conn.rollback()
        logger.warning("Failed to synchronize availability tag for device %s", device_id, exc_info=True)
    finally:
        conn.close()


def _schedule_monitoring_target_sync() -> None:
    """Refresh SNMP file-SD targets after a device reachability transition."""
    try:
        ...
        logger.warning(
            "Failed to schedule monitoring target sync after device status change",
            exc_info=True,
        )
...
def _resolve_alert_event(dedupe_key: str):
    now = _utc_now_iso()
    row = _db_quick(
        "SELECT id FROM alert_events WHERE dedupe_key = ? AND resolved_at IS NULL ORDER BY created_at DESC LIMIT 1",
...
def _persist_alert_transition(dedupe_key: str, transition: AlertState, rule: dict, device_id: str, interface_name: str | None):
    """Persist hysteresis state so a worker restart cannot erase FOR/recovery state."""
    try:
        _db_quick(
            """
            INSERT INTO alert_rule_states
                (id, rule_id, target_key, state, pending_since, recovery_since,
                 breach_count, recovery_count, flap_count, last_status, last_transition_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(rule_id, target_key) DO UPDATE SET
                state=excluded.state, pending_since=excluded.pending_since,
                recovery_since=excluded.recovery_since, breach_count=excluded.breach_count,
                recovery_count=excluded.recovery_count, flap_count=excluded.flap_count,
                last_status=excluded.last_status, last_transition_at=excluded.last_transition_at,
                updated_at=excluded.updated_at
            """,
            (
                f"ars_{uuid.uuid4().hex[:16]}",
                str(rule.get('id') or dedupe_key),
                f"{device_id}:{interface_name or ''}",
                transition.state,
                transition.pending_since.isoformat() if transition.pending_since else None,
                transition.recovery_since.isoformat() if transition.recovery_since else None,
                transition.breach_count,
                transition.recovery_count,
                transition.flap_count,
                'active' if transition.state in {'PENDING', 'ACTIVE', 'RECOVERING'} else 'resolved',
                _utc_now_iso() if transition.transitions else None,
                _utc_now_iso(),
            ),
        )
    except Exception:
        # m0105 may not have been applied on an older development database;
        # the in-memory state remains authoritative until the next migration.
        logger.debug("Alert transition state table unavailable", exc_info=True)


def _load_alert_transition_state(dedupe_key: str, rule: dict, device_id: str, interface_name: str | None) -> AlertState:
    """Restore persisted FOR/recovery counters after a monitor restart."""
    rule_id = str(rule.get('id') or dedupe_key)
    target_key = f"{device_id}:{interface_name or ''}"
    try:
        row = _db_quick(
            """
            SELECT state, pending_since, recovery_since, breach_count,
                   recovery_count, flap_count
            FROM alert_rule_states
            WHERE rule_id = ? AND target_key = ?
            LIMIT 1
            """,
            (rule_id, target_key),
            fetch=True,
        ...
        return AlertState(
            ...
            pending_since=parse_timestamp(row['pending_since']),
            recovery_since=parse_timestamp(row['recovery_since']),
            breach_count=int(row['breach_count'] or 0),
            recovery_count=int(row['recovery_count'] or 0),
            flap_count=int(row['flap_count'] or 0),
        )
    except Exception:
        logger.debug("Alert transition state could not be restored", exc_info=True)
        return AlertState()


def _record_alert_activity(alert_id: str | None, event_type: str, *, from_state: str | None = None, to_state: str | None = None, metadata: dict | None = None):
    if not alert_id:
        return
    try:
        _db_quick(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (f"aact_{uuid.uuid4().hex[:16]}", alert_id, event_type, from_state, to_state, json.dumps(metadata or {}, ensure_ascii=False), _utc_now_iso()),
        )
    except Exception:
        logger.debug("Alert activity table unavailable", exc_info=True)
...
def _run_telemetry_maintenance():
    ...
    _db_quick('''
        INSERT INTO interface_telemetry_1m (
            ...
            err_delta_sum, discard_delta_sum, fcs_sum
        )
        SELECT
            ...
            SUM(COALESCE(fcs_errors, 0))
        FROM interface_telemetry_raw
        WHERE ts >= ? AND LOWER(COALESCE(status, '')) = 'up'
    ...
    # Delivery attempts and terminal outbox rows follow the same one-year
    # retention window. Open/retryable deliveries remain available for audit
    # and recovery even when their original alert is old.
    try:
        _db_quick(
            "DELETE FROM alert_delivery_attempts WHERE created_at < ?",
            (alert_cutoff,),
        )
        _db_quick(
        ...
        logger.debug("Notification delivery retention cleanup skipped", exc_info=True)
...
open_alerts = {}
alert_transition_states: dict[str, AlertState] = {}
protocol_peer_cache = {}
...
intf_flap_history = {}
prev_intf_octets = {}
prev_intf_config_hash: dict[str, str] = {}
webhook_last_sent = {}
current_alert_rules = []
current_alert_rules_loaded_at = 0.0
ALERT_RULE_CACHE_TTL_SECONDS = 15.0
role_poll_profiles = {
    'core': {
        'interface': 1,
        'info': 10,
...
def _init_telemetry_state():
    ...
    try:
        ...
        pass
    _refresh_runtime_alert_rules(force=True)


def _refresh_runtime_alert_rules(*, force: bool = False) -> list[dict[str, Any]]:
    """Refresh enabled rules without making every SNMP metric query the DB.

    Alert rules are edited from the UI while the monitor worker stays alive.
    A short cache keeps the hot polling path cheap while ensuring a rule
    change reaches the next few collection cycles instead of requiring a
    process restart.  If the database is temporarily unavailable, retain the
    last known-good rule set so an incidental connection failure does not
    disable all alert evaluation.
    """
    global current_alert_rules, current_alert_rules_loaded_at
    now = _time.monotonic()
    if not force and current_alert_rules_loaded_at and (now - current_alert_rules_loaded_at) < ALERT_RULE_CACHE_TTL_SECONDS:
        return current_alert_rules
    try:
        refreshed = alert_rule_service.get_runtime_rules()
    except Exception:
        current_alert_rules_loaded_at = now
        logger.warning("[Alert] Runtime rule refresh failed; retaining the last known-good rules", exc_info=True)
        return current_alert_rules
    current_alert_rules = refreshed
    current_alert_rules_loaded_at = now
    return current_alert_rules


def _component_failure_state(value: Any) -> bool | None:
    """Normalize template/legacy fan and PSU values to failure/healthy/unknown.

    The typed SNMP collector returns ``True`` for healthy and ``False`` for
    failed status codes.  Older vendor collectors may still return labels.
    Unknown values are deliberately ignored by the alert evaluator so a
    missing or unsupported sensor cannot create a false failure or recovery.
    """
    if isinstance(value, bool):
        return not value
    normalized = str(value or '').strip().casefold()
    if normalized in {'fail', 'failed', 'failure', 'warning', 'down', 'offline', 'alarm', 'false', '0'}:
        return True
    if normalized in {'ok', 'normal', 'redundant', 'single', 'up', 'running', 'ready', 'true', '1'}:
        return False
    return None
def resolve_alert_rule(metric_type: str, *, device_row=None, ip_address: str = '', interface_name: str | None = None):
    _refresh_runtime_alert_rules()
    context = {
...
def maybe_notify_webhook(dedupe_key: str, severity: str, title: str, message: str,
                           ...
                           device_id: str = ''):
    # A development/test launcher must not replay existing alert state into a
    # real webhook while bootstrapping the background monitor. Keep this guard
    # before throttling state changes so opting back in does not inherit a
    # stale suppression window from a disabled run.
    if not notification_service.automatic_notifications_enabled():
        logger.info("[Alert] Automatic notification dispatch disabled by runtime policy")
        return
    ...
    alert_info = {
        ...
        'event_kind':       'reopened' if reopened_after_maintenance else 'resolved' if status == 'resolved' else 'active',
        'rule_id':          str((rule or {}).get('id') or ''),
        'destination_key':  dedupe_key,
    }
    ...
    try:
        tenant_row = _db_quick(
            "SELECT COALESCE(tenant_id, 'tenant-default') AS tenant_id FROM devices WHERE id = ?",
            (device_id,), fetch=True,
        )
        tenant_id = str(tenant_row['tenant_id'] or 'tenant-default') if tenant_row else tenant_id
    except Exception:
        logger.warning('[Alert] Unable to resolve tenant for notification %s', dedupe_key)
    alert_info['tenant_id'] = tenant_id

    from services.monitoring_notification_outbox_service import enqueue_alert_delivery

    configured_channels = (rule or {}).get('notification_channels')
    if isinstance(configured_channels, str):
        try:
            configured_channels = json.loads(configured_channels)
        except (TypeError, ValueError):
            configured_channels = [part.strip() for part in configured_channels.split(',') if part.strip()]
    if not isinstance(configured_channels, (list, tuple, set)) or not configured_channels:
        configured_channels = ['workspace']
    channels = []
    ...
    event_kind = str(alert_info['event_kind'])
    common_payload = {**alert_info, 'lang': 'zh'}

    # Keep the legacy .env webhook attached to workspace rules. An explicit
    # channel policy can opt into global_webhook independently.
    if 'workspace' in channels and (settings.ALERT_NOTIFY_WEBHOOK_URL or '').strip():
        plain = (
        ...
        enqueue_alert_delivery(
            delivery_key=f"{dedupe_key}:global_webhook:{event_kind}:{created_at}",
            channel='global_webhook',
            payload=payload,
        )

    for channel in channels:
        if channel == 'global_webhook' and not (settings.ALERT_NOTIFY_WEBHOOK_URL or '').strip():
            continue
        if channel not in {'workspace', 'feishu', 'dingtalk', 'wechat', 'email', 'global_webhook'}:
            continue
        enqueue_alert_delivery(
            delivery_key=f"{dedupe_key}:{channel}:{event_kind}:{created_at}",
            channel=channel,
...
def set_alert_state(dedupe_key: str, is_active: bool, severity: str, title: str, message: str,
    ...
    if currently_open is None:
        # Rehydrate the open projection as well as the hysteresis state so a
        # worker restart does not create duplicate alerts or lose recovery.
        try:
            existing = _db_quick(
                "SELECT created_at FROM alert_events WHERE dedupe_key = ? AND resolved_at IS NULL ORDER BY created_at ASC LIMIT 1",
                (dedupe_key,),
                fetch=True,
            )
            if existing:
                currently_open = str(existing['created_at'] or '')
                open_alerts[dedupe_key] = currently_open
        except Exception:
            currently_open = None
    transition_state_row = alert_transition_states.get(dedupe_key)
    if transition_state_row is None:
        transition_state_row = _load_alert_transition_state(dedupe_key, alert_rule, device_id, interface_name)
        alert_transition_states[dedupe_key] = transition_state_row
    transition = transition_state(
        transition_state_row,
        breached=bool(is_active),
        for_seconds=int(alert_rule.get('for_duration_seconds') or 0),
        recovery_threshold=int(alert_rule.get('recovery_threshold') or alert_rule.get('recover_after_count') or 1),
        recovery_seconds=int(alert_rule.get('recovery_duration_seconds') or 0),
    )
    _persist_alert_transition(dedupe_key, transition_state_row, alert_rule, device_id, interface_name)
    # Raw observations remain available to the monitor; only the operator
    # alert projection waits for FOR/recovery hysteresis.
    if is_active and not transition.emit and transition.state != 'ACTIVE':
        return
    if (not is_active) and not transition.emit:
        return
    if is_active and not currently_open:
        ...
        maintenance_window = alert_maintenance_service.find_active_window_for_alert(alert_context)
        silence = find_active_silence_for_alert(alert_context)
        workflow_status = 'suppressed' if maintenance_window else 'open'
        note = f"Suppressed by maintenance window: {maintenance_window['name']}" if maintenance_window else f"Notification silenced: {silence['reason']}" if silence else ''
        alert_id, created_at = _create_alert_event(
            dedupe_key, severity, title, message, device_id, interface_name,
            workflow_status=workflow_status,
            note=note,
        )
        _record_alert_activity(alert_id, 'SUPPRESSED' if maintenance_window else 'SILENCED' if silence else 'OPENED', to_state=workflow_status, metadata={"dedupe_key": dedupe_key, "source": "network_monitor", "silence_id": silence.get('id') if silence else None})
        open_alerts[dedupe_key] = created_at  # store first-occurrence time
        if maintenance_window or silence:
            return
        ...
        resolved_alert_id = _resolve_alert_event(dedupe_key)
        _record_alert_activity(resolved_alert_id, 'RESOLVED', from_state='open', to_state='resolved', metadata={"dedupe_key": dedupe_key, "resolve_type": "automatic_recovery"})
...
async def _process_device_raw(device, cycle_tick: int):
    async with _get_semaphore():
        ...
        collect_protocol = (
            _should_collect(cycle_tick, poll_profile['protocol'])
            and any(should_collect(device, category) for category in ('bgp', 'ospf', 'bfd'))
        )
        ...
        def mark_offline_and_clear_cache():
            ...
            _db_quick('''
                ...
                WHERE id = ?
            ''', ('offline', 0, 0, dev_id))
            _sync_device_status_tag(dev_id, 'offline')
            prev_intf_octets.pop(dev_id, None)
            prev_intf_config_hash.pop(dev_id, None)

        if not ip or ip == '0.0.0.0':
            mark_offline_and_clear_cache()
            if str(device.get('status') or '').lower() != 'offline':
                _schedule_monitoring_target_sync()
            return
        ...
        try:
            ...
            async with get_network_access_limiter().async_probe():
            ...
            if device['status'] != new_status:
                if new_status == 'offline':
                    ...
                    _sync_device_status_tag(dev_id, new_status)
                    set_alert_state(
        ...
        collection_started = _time.perf_counter()
        collector_credentials = resolve_collector_credentials(device)
        snmp_credentials = collector_credentials['snmp']
        snmp_ip = snmp_credentials.get('server') or ip
        snmp_community = snmp_credentials.get('community') or ''
        snmp_port = snmp_credentials.get('port') or 161
        ...
        if snmp_community:
            try:
                discovery = load_discovery(str(dev_id))
            except Exception as discovery_store_err:
                logger.debug(f"[SNMP] OID discovery store unavailable for {ip}: {type(discovery_store_err).__name__}")
                discovery = None
            if _discovery_is_stale(discovery, device=device):
                try:
                    discovery = await discover_device_oid_config(
                        ...
                        snmp_version=str(device.get('snmp_version') or '2c'),
                    )
                    persist_discovery(str(dev_id), discovery)
                except Exception as discovery_err:
                    logger.debug(f"[SNMP] {ip} OID discovery failed: {discovery_err}")
                    discovery = discovery or {
                        'status': 'snmp_error',
                        'reason': type(discovery_err).__name__,
                        'metrics': {},
                        'interface': {},
                    }

        resolved_metrics = resolve_health_metric_profiles(device)
        profile_verified = str(resolved_metrics.get("profile_status") or "").casefold() in {"tested", "published", "verified", "passed", "approved"}
        if discovery and str(discovery.get("status") or "") in {"matched", "partial"} and not profile_verified:
            # A successful device probe is the authoritative source for this
            # polling cycle.  An unverified model profile remains visible as
            # metadata but cannot silently override observed table columns.
            resolved_metrics = _resolved_metrics_from_discovery(discovery, str(dev_id))
        elif discovery and str(resolved_metrics.get("profile_status") or "").casefold() not in {"tested", "published", "verified", "passed", "approved"}:
            # Keep unknown or unsupported vendors on the safe generic baseline;
            # a previously verified operator profile remains usable as an
            # explicit exception when discovery cannot identify a table.
            resolved_metrics = _resolved_metrics_from_discovery(discovery, str(dev_id))
        if snmp_community:
            try:
                ...
                metric_details = metrics.get('metric_details') or {}
                hardware_metrics = metrics.get('hardware_metrics') or {}
                temp = metrics.get('temp')
                fan = metrics.get('fan_status')
                psu = metrics.get('psu_status')
                snmp_available = any(
                    hardware_metrics.get(metric) is not None
                    for metric in HEALTH_METRIC_KEYS
                )
            except Exception as snmp_err:
                snmp_error_code = 'snmp_metrics_failed'
                snmp_error_message = type(snmp_err).__name__
                logger.debug(f"[SNMP] {ip} metrics failed: {snmp_err}")
        else:
            snmp_error_code = 'snmp_not_configured'
            snmp_error_message = 'SNMP credentials are not configured'
        ...
        if snmp_community and not snmp_available:
            try:
                from services.snmp_service import _snmp_get, SYS_UPTIME
                test_val = await _snmp_get(snmp_ip, snmp_community, SYS_UPTIME, snmp_port, timeout=1.5)
                if test_val is not None:
                    snmp_available = True
            except Exception:
                if not snmp_error_code:
                    snmp_error_code = 'snmp_no_response'
                    snmp_error_message = 'No response from standard SNMP system OID'
        ...
        fan_db = 'true' if fan is True else 'false' if fan is False else fan
        psu_db = 'true' if psu is True else 'false' if psu is False else psu
        _db_quick('''
            UPDATE devices 
            SET cpu_usage = ?, memory_usage = ?, cpu_history = ?, memory_history = ?, temp = ?, fan_status = ?, psu_status = ?
            WHERE id = ?
        ''', (cpu_usage, memory_usage, json.dumps(cpu_history), json.dumps(mem_history), temp, fan_db, psu_db, dev_id))
        ...
        if fan is not None:
            ...
            fan_failed = _component_failure_state(fan)
            if fan_failed is not None:
                set_alert_state(
                    fan_key,
                    fan_failed,
        ...
        if psu is not None:
            ...
            psu_failed = _component_failure_state(psu)
            if psu_failed is not None:
                set_alert_state(
                    psu_key,
                    psu_failed,
        ...
        if collect_info and snmp_community:
            try:
                dev_info = await collect_device_info(snmp_ip, snmp_community, snmp_port)
                if any(value is not None for value in dev_info.values()):
                    snmp_available = True
                updates = {}
                if dev_info.get('sys_name'):
                    updates['sys_name'] = dev_info['sys_name']
                    _sync_snmp_hostname_projection(str(dev_id), dev_info['sys_name'])
        ...
        if collect_intf and snmp_community:
            try:
                now_ts = _time.monotonic()
                now_iso = _utc_now_iso()
                interface_config = resolved_metrics.get('interface') or None
                interface_profile_id = str(resolved_metrics.get('profile_id') or 'template-not-applied')
                interface_config_hash = hashlib.sha256(
                    json.dumps(interface_config or {}, sort_keys=True, separators=(',', ':'), default=str).encode('utf-8')
                ).hexdigest()[:32]
                # Device health uses the lightweight identity/status collector,
                # but the monitoring page needs real IF-MIB counters to derive
                # throughput and utilization.  Keep these two collection paths
                # separate so health polling stays cheap without making the
                # traffic view look empty.
                intf_data = await collect_interface_data(
                    snmp_ip,
                    snmp_community,
                    snmp_port,
                    interface_config,
                ...
                if not intf_data:
                    record_collection_result(
                        dev_id,
                        'snmp_interfaces',
                        status='template_required' if not interface_config else 'failed',
                        transport='udp',
                        source='snmp',
                        error_code='snmp_template_not_applied' if not interface_config else 'snmp_interface_table_empty',
                        error_message=(
                            'No applied SNMP interface template is available for this device model'
                            if not interface_config else 'SNMP interface table is empty'
                        ),
                    )
                if intf_data:
                    snmp_available = True
                    record_collection_result(
                        ...
                        transport='udp',
                        source='snmp',
                        coverage_total=len(intf_data),
                        coverage_supported=sum(
                            1 for row in intf_data
                            if row.get('counter_width') in (32, 64)
                            and row.get('in_octets') is not None
                            and row.get('out_octets') is not None
                        ),
                        metadata={
                            'collection_mode': 'traffic',
                            'counter_collection': 'counter64_preferred_counter32_fallback',
                            'counter_widths': sorted({
                                row.get('counter_width')
                                for row in intf_data
                                if row.get('counter_width') in (32, 64)
                            }),
                            'status_rows': sum(
                                1 for row in intf_data
                                if row.get('status') in {'up', 'down', 'testing'}
                            ),
                            'interface_source': resolved_metrics.get('interface_source', 'template_not_applied'),
                        },
                    )
                    topology_ports = load_topology_ports(dev_id)
                    # Calculate real-time throughput and bandwidth utilization from previous snapshot.
                    previous_config_hash = prev_intf_config_hash.get(str(dev_id))
                    if previous_config_hash and previous_config_hash != interface_config_hash:
                        # A template edit changes the meaning of the stored
                        # counters.  Start a fresh baseline instead of
                        # calculating a rate across two different OID maps.
                        prev_intf_octets.pop(dev_id, None)
                    prev = prev_intf_octets.get(dev_id, {})
                    durable = load_interface_counter_sample(dev_id, interface_profile_id) if not prev else None
                    if durable and str(durable.get('config_hash') or '') == interface_config_hash:
                        durable_epoch = _iso_epoch(durable.get('sampled_at'))
                        elapsed_since_durable = max(0.0, _time.time() - durable_epoch) if durable_epoch is not None else None
                        if elapsed_since_durable is not None:
                            prev = {
                                str(index): {
                                    **dict(snapshot),
                                    'ts': now_ts - elapsed_since_durable,
                                }
                                for index, snapshot in (durable.get('values') or {}).items()
                    ...
                    for iface in intf_data:
                        iname = iface['name']
                        # ifIndex is stable across an ifName rename; use it as
                        # the counter key and retain the name for display/
                        # alerting.  Name-only fallback keeps legacy rows
                        # usable when an agent omits a numeric index.
                        interface_key = str(iface.get('if_index') or iname)
                        effective_prev_status = str(prev.get(interface_key, {}).get('status', '') or persisted_statuses.get(iname, '')).lower()
                        cur_snapshot[interface_key] = {
                            'name': iname,
                            'in_octets': iface.get('in_octets'),
                            'out_octets': iface.get('out_octets'),
                            'counter_width': iface.get('counter_width'),
                            'counter_source': iface.get('counter_source'),
                            'counter_quality': iface.get('counter_quality'),
                            'counter_quality_reason': iface.get('counter_quality_reason'),
                            'counter_mode': iface.get('counter_mode'),
                            'speed_mbps': iface.get('speed_mbps', 0),
                            'device_uptime_cs': iface.get('device_uptime_cs'),
                            # Preserve the values returned by SNMP for both
                            # the persisted sample and the next delta baseline.
                            'in_ucast_pkts': iface.get('in_ucast_pkts', 0),
                            'out_ucast_pkts': iface.get('out_ucast_pkts', 0),
                            'in_packets_total': iface.get('in_packets_total', iface.get('in_ucast_pkts', 0)),
                            'out_packets_total': iface.get('out_packets_total', iface.get('out_ucast_pkts', 0)),
                            'packet_counter_source': iface.get('packet_counter_source', 'not_collected'),
                            ...
                            'fcs_errors': iface.get('fcs_errors', 0),
                            'frame_too_long_errors': iface.get('frame_too_long_errors', 0),
                            'mac_rx_errors': iface.get('mac_rx_errors', 0),
                            'symbol_errors': iface.get('symbol_errors', 0),
                            'fcs_source': iface.get('fcs_source', 'not_collected'),
                        ...
                        if interface_key in prev:
                            previous = prev[interface_key]
                            dt = now_ts - float(previous.get('ts') or now_ts)
                            width = iface.get('counter_width')
                            previous_width = previous.get('counter_width')
                            if dt > 0 and width in (32, 64) and previous_width == width:
                                speed_bps = float(iface.get('speed_mbps') or 0) * 1_000_000
                                max_rate = speed_bps / 8 if speed_bps > 0 else None
                                in_delta = calculate_counter_delta(
                                    iface.get('in_octets'),
                                    previous.get('in_octets'),
                                    dt,
                                    width,
                                    max_rate_per_sec=max_rate,
                                    current_uptime_cs=iface.get('device_uptime_cs'),
                                    previous_uptime_cs=previous.get('device_uptime_cs'),
                                )
                                out_delta = calculate_counter_delta(
                                    iface.get('out_octets'),
                                    previous.get('out_octets'),
                                    dt,
                                    width,
                                    max_rate_per_sec=max_rate,
                                    current_uptime_cs=iface.get('device_uptime_cs'),
                                ...
                                if in_delta.get('rate_per_sec') is not None and out_delta.get('rate_per_sec') is not None:
                                    iface['in_bps'] = round(float(in_delta['rate_per_sec']) * 8, 1)
                                    iface['out_bps'] = round(float(out_delta['rate_per_sec']) * 8, 1)
                                    iface['counter_delta_status'] = {
                                        'in': in_delta.get('status'),
                                        'out': out_delta.get('status'),
                                    }
                                    if speed_bps > 0:
                                        bw_in = round((float(in_delta['rate_per_sec']) / speed_bps) * 800, 1)
                                        bw_out = round((float(out_delta['rate_per_sec']) / speed_bps) * 800, 1)
                                        iface['bw_in_pct'] = min(bw_in, 100.0)
                                        iface['bw_out_pct'] = min(bw_out, 100.0)

                                    counter_bits = width or 32
                                    error_deltas = [
                                        calculate_counter_delta(iface.get(field), previous.get(field), dt, counter_bits,
                                                                 current_uptime_cs=iface.get('device_uptime_cs'),
                                                                 previous_uptime_cs=previous.get('device_uptime_cs'))
                                        for field in ('in_errors', 'out_errors', 'in_discards', 'out_discards')
                                    ]
                                    packet_deltas = [
                                        calculate_counter_delta(
                                            iface.get(field, iface.get(fallback, 0)),
                                            previous.get(field, previous.get(fallback, 0)),
                                            dt, counter_bits,
                                            current_uptime_cs=iface.get('device_uptime_cs'),
                                            previous_uptime_cs=previous.get('device_uptime_cs'),
                                        )
                                        for field, fallback in (
                                            ('in_packets_total', 'in_ucast_pkts'),
                                            ('out_packets_total', 'out_ucast_pkts'),
                                        )
                                    ]
                                    if all(item.get('rate_per_sec') is not None for item in (*error_deltas, *packet_deltas)):
                                        delta_error_events = sum(int(item['delta'] or 0) for item in error_deltas)
                                        delta_packets = sum(int(item['delta'] or 0) for item in packet_deltas)
                                        if delta_packets > 0:
                                            iface['error_rate_pct'] = round((delta_error_events / delta_packets) * 100, 3)
                                    else:
                                        iface['counter_delta_status'] = {
                                            **iface.get('counter_delta_status', {}),
                                            'error': 'invalid_or_reset',
                                        }
                        ...
                        raw_rows.append((
                            ...
                            iface.get('in_packets_total', iface.get('in_ucast_pkts')),
                            iface.get('out_packets_total', iface.get('out_ucast_pkts')),
                            iface.get('in_errors'),
                            iface.get('out_errors'),
                            iface.get('in_discards'),
                            iface.get('out_discards'),
                            iface.get('fcs_errors'),
                            iface.get('frame_too_long_errors'),
                            iface.get('mac_rx_errors'),
                            iface.get('symbol_errors'),
                            iface.get('packet_counter_source'),
                            iface.get('fcs_source'),
                        ))
                        ...
                        current_error_rate_value = iface.get('error_rate_pct')
                        current_error_rate = float(current_error_rate_value) if current_error_rate_value is not None else None
                        ...
                        utilization_values = [
                            float(value) for value in (iface.get('bw_in_pct'), iface.get('bw_out_pct'))
                    ...
                    prev_intf_config_hash[str(dev_id)] = interface_config_hash
                    save_interface_counter_sample(
                        device_id=str(dev_id),
                        profile_id=interface_profile_id,
                        config_hash=interface_config_hash,
                        values=cur_snapshot,
                        sampled_at=now_iso,
                        device_uptime_cs=next(
                            (row.get('device_uptime_cs') for row in intf_data if row.get('device_uptime_cs') is not None),
                            None,
                        ),
                    )
                    _db_many('''
                ...
                record_collection_result(
                    ...
                    transport='udp',
                    source='snmp',
                    error_code='snmp_interface_collection_failed',
                    error_message=type(intf_err).__name__,
                )
        ...
        metric_values = tuple(hardware_metrics.get(metric) for metric in HEALTH_METRIC_KEYS)
        if not any(value is not None for value in metric_values):
            metric_values = (cpu_usage, memory_usage, temp, fan, psu)
        if is_server_asset:
            record_collection_result(
                dev_id,
                'snmp_metrics',
                ...
                duration_ms=(_time.perf_counter() - collection_started) * 1000,
                metadata={'reason': 'server_asset'},
            )
        else:
            metric_count = sum(value is not None for value in metric_values)
            profile_metrics = resolved_metrics.get('metrics') or {}
            profile_status = str(resolved_metrics.get('profile_status') or '').casefold()
            if profile_status == 'failed':
                collection_status = 'failed'
                collection_error_code = 'snmp_template_verification_failed'
                collection_error_message = 'Applied SNMP hardware template verification failed'
            elif not profile_metrics:
                collection_status = 'template_required'
                collection_error_code = 'snmp_template_not_applied'
                collection_error_message = 'No applied SNMP hardware metric template is available for this device model'
            elif not snmp_available:
                collection_status = 'failed'
                collection_error_code = snmp_error_code or 'snmp_no_response'
                collection_error_message = snmp_error_message or 'SNMP collection returned no data'
            elif metric_count < len(metric_values):
                collection_status = 'partial'
                collection_error_code = 'snmp_metrics_partial'
                collection_error_message = 'Applied SNMP template returned only a subset of health metrics'
            else:
            ...
            record_collection_result(
                dev_id,
                'snmp_metrics',
                status=collection_status if snmp_community else 'not_configured',
                transport='udp',
                source='snmp',
                duration_ms=(_time.perf_counter() - collection_started) * 1000,
                coverage_total=len(metric_values),
                coverage_supported=metric_count,
                error_code=collection_error_code if snmp_community else 'snmp_not_configured',
                error_message=collection_error_message if snmp_community else 'SNMP credentials are not configured',
                metadata={
        ...
        if snmp_available:
            ...
            failure_detail = (
                f"{dev_label} SNMP 凭据未配置"
                if not snmp_community
                else f"{dev_label} 设备在线，但连续 {snmp_failure_streak[dev_id]} 次 SNMP 采集失败"
            )
            set_alert_state(
                snmp_key,
                snmp_active,
...
async def process_device_fast(device):
    ...
    def mark_offline_and_clear_cache():
        ...
        _sync_device_status_tag(dev_id, 'offline')
        prev_intf_octets.pop(dev_id, None)
        prev_intf_config_hash.pop(str(dev_id), None)
    ...
    if not ip or ip == '0.0.0.0':
        ...
        record_collection_result(
            ...
            status='not_configured',
            source='icmp_tcp',
            error_code='management_ip_missing',
            error_message='Management IP address is not configured',
        )
        return
    ...
    try:
        ...
        record_collection_result(
            ...
            transport=probe_method,
            source='icmp_tcp',
            error_code='' if reachable else 'device_unreachable',
            error_message='' if reachable else 'ICMP and management-port probes failed',
        )
...
async def process_device_slow(device, cycle_tick: int = 0):
    """Run a low-frequency pass using the device role poll profile.

    The initial cycle (0) collects metadata and protocol state immediately.
    Subsequent cycles keep SNMP/interface telemetry frequent while applying
    the configured role-based intervals for metadata and CLI protocols.
    """
    device = _as_dict(device)
    if str(device.get('status')).lower() != 'online':
        return

    await process_device(device, cycle_tick)
