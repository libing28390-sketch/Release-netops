...
async def _process_device_raw(device, cycle_tick: int):
    async with _get_semaphore():
        ...
        if snmp_community:
            try:
                metrics = await collect_device_metrics(snmp_ip, platform, snmp_community, snmp_port)
        ...
        if collect_intf and snmp_community:
            try:
                ...
                now_iso = _utc_now_iso()
                intf_data = await collect_interface_data(snmp_ip, snmp_community, snmp_port)
                if not intf_data:
                ...
                if intf_data:
                    ...
                    record_collection_result(
                        ...
                        coverage_supported=len(intf_data),
                        metadata={'counter_mode': 'ifHC-preferred'},
                    )
