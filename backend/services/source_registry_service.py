...
def _load_official_allowlist() -> dict[str, list[dict[str, Any]]]:
    architecture_dir = Path(__file__).resolve().parents[2] / "docs" / "knowledge-engine-v2" / "architecture"
    paths = [
        architecture_dir / "CAT-001-OFFICIAL-SOURCE-ALLOWLIST.yaml",
        # Vendor/product additions are intentionally additive.  Keeping them
        # in a separate reviewed file preserves the CAT-001 baseline contract
        # while allowing the catalog to grow without rewriting its evidence.
...
def list_source_refresh_observations(
    ...
    *,
    ...
    try:
        ...
        rows = conn.execute(
            "SELECT * FROM kb_source_refresh_observation WHERE tenant_id = ? AND source_registry_id = ? ORDER BY checked_at DESC, CASE WHEN detection_type = 'none' THEN 0 ELSE 1 END DESC, id DESC LIMIT ?",
...
def create_source(payload: dict[str, Any], user: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        conn.execute(
            ...
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'https', ?, 'exact', ?, ?, FALSE, ?, ?, ?, ?, 1, ?, ?, 'unvalidated', '{}', ?, ?, ?, ?)
            """,
            (
...
def enable_source(source_id: str, user: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        conn.execute(
            "UPDATE kb_source_registry SET status = 'active', fetch_enabled = TRUE, validation_status = 'valid', validation_json = ?, updated_at = ?, updated_by = ?, lock_version = lock_version + 1, disabled_at = NULL, disabled_by = NULL, disable_reason = NULL WHERE id = ? AND tenant_id = ?",
            (_json_dumps(result), now, _actor(user), source_id, tenant_id),
        )
        updated = _get_source_in_conn(conn, source_id, user, tenant_id=tenant_id)
...
def disable_source(source_id: str, user: dict[str, Any], *, reason: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        now = _now()
        conn.execute(
                    "UPDATE kb_source_registry SET status = 'disabled', fetch_enabled = FALSE, disabled_at = ?, disabled_by = ?, disable_reason = ?, updated_at = ?, updated_by = ?, lock_version = lock_version + 1 WHERE id = ? AND tenant_id = ?",
...
def quarantine_source_for_change(
    ...
    *,
    ...
    try:
        ...
        conn.execute(
            "UPDATE kb_source_registry SET status = 'quarantined', fetch_enabled = FALSE, validation_status = 'invalid', validation_json = ?, updated_at = ?, updated_by = ?, lock_version = lock_version + 1 WHERE id = ? AND tenant_id = ?",
...
def delete_source(source_id: str, user: dict[str, Any], *, reason: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        now = _now()
        conn.execute(
            "UPDATE kb_source_registry SET status = 'deleted', fetch_enabled = FALSE, deleted_at = ?, deleted_by = ?, deletion_reason = ?, updated_at = ?, updated_by = ?, lock_version = lock_version + 1 WHERE id = ? AND tenant_id = ?",
            (now, _actor(user), reason, now, _actor(user), source_id, tenant_id),
        )
        updated = _get_source_in_conn(conn, source_id, user, tenant_id=tenant_id)
...
def collect_source(
    ...
    *,
    ...
    metadata and never returns the body.  An internal ``content_sink`` may be
    supplied by a trusted continuation worker to parse/publish the bytes in
    the same request.  It is never serialized into the API result.
    """
    request = dict(payload or {})
    ...
    # ING-023's batch path opts into the operational policy gate.  The
    # historical single-source API keeps the default false for compatibility,
    # while every opted-in official collection records terms and robots facts
    # before the document request begins.
    robots_decision: dict[str, Any] = {
        "policy": "not_checked",
        "outcome": "not_checked",
        "allowed": True,
        "error_code": "",
    ...
    terms_metadata = source.get("metadata") if isinstance(source.get("metadata"), dict) else {}
    terms_status = str(terms_metadata.get("terms_review_status") or "").strip().lower()
    terms_review = {
        "required": bool(operational_policy and _is_official_source(source)),
        "status": terms_status if terms_status in {"approved", "waived", "not_required"} else "pending",
        "reviewer_present": bool(str(terms_metadata.get("reviewer") or "").strip()),
        "reviewed_at_present": bool(str(terms_metadata.get("reviewed_at") or "").strip()),
    }

    _audit_fetch(
        "fetch_started",
        details={"method": method, "policy_version": source.get("policy_version"), "operational_policy": bool(operational_policy)},
    )
    from services.safe_outbound_http import OutboundCollectionError, check_robots_policy, safe_fetch

    if operational_policy:
        _audit_fetch("terms_review_recorded", details=terms_review)
        if terms_review["required"] and terms_review["status"] not in {"approved", "waived", "not_required"}:
            _audit_fetch("fetch_failed", details={"code": "TERMS_REVIEW_REQUIRED"})
            raise SourceRegistryError(
                "TERMS_REVIEW_REQUIRED",
                "Official source terms review must be approved before collection",
                status_code=403,
            )
        robots_decision = check_robots_policy(
            source,
            transport=transport,
            resolver=resolver,
        ...
        _audit_fetch(
            "robots_checked",
            details={
                "policy": robots_decision.get("policy"),
                "outcome": robots_decision.get("outcome"),
                "allowed": bool(robots_decision.get("allowed")),
                "error_code": str(robots_decision.get("error_code") or "")[:128],
                "status_code": robots_decision.get("status_code"),
                "bytes_read": int(robots_decision.get("bytes_read") or 0),
                "review_required": bool(robots_decision.get("review_required")),
            },
        )
        if not robots_decision.get("allowed"):
            code = str(robots_decision.get("error_code") or "ROBOTS_POLICY_DENIED")[:128]
            _audit_fetch("fetch_failed", details={"code": code})
            raise SourceRegistryError(
                code,
                "Source collection was denied by the registered robots policy",
                status_code=403 if code == "ROBOTS_DISALLOWED" else 502,
                details={"robots_outcome": str(robots_decision.get("outcome") or "")[:64]},
            )
    ...
    try:
        result = safe_fetch(
            ...
            backoff_base_seconds=backoff_base_seconds,
            sleeper=sleeper or time.sleep,
            # A trusted publication continuation needs the response body in
            # memory for parsing/chunking.  Source versions intentionally keep
            # only a hash/reference, so a conditional 304 cannot satisfy the
            # continuation after a previous fetch.  Force a body fetch whenever
            # a sink is present; the legacy manifest-only path keeps normal
            # conditional requests and 304 replay semantics.
            conditional_headers={} if content_sink is not None else {
        ...
        if (
            ...
            return {
                ...
                "version": None,
                "policy": {"operational_enforced": bool(operational_policy), "terms_review": terms_review, "robots": robots_decision},
                "refresh": {
                    "outcome": "changed",
    ...
    if content_sink is not None:
        # Keep the body inside the trusted ingestion boundary.  The callback
        # is deliberately invoked only for a fresh GET response (304 and HEAD
        # paths above have no new bytes to publish).
...
def get_source_refresh_status(source_id: str, user: dict[str, Any]) -> dict[str, Any]:
    """Return a bounded operational read model for one Source Registry row.

    The response contains only status, hashes, counters and stable error codes;
    response bodies, headers, credentials and internal storage references are
    deliberately excluded.  Missing freshness migrations are represented as a
    safe ``not_configured`` state so V1 installations remain readable.
    """
    source_id = _bounded_refresh_text(source_id, field="source_id", maximum=128)
    conn = get_db_connection()
    try:
        ...
        if _database._USE_PG:
            ready = bool(conn.execute(
                "SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'kb_source_refresh_observation'"
            ).fetchone())
        else:
            ready = bool(conn.execute(
        ...
        latest_version_row = conn.execute(
            "SELECT id, content_hash, byte_size, fetched_at, status FROM kb_source_version "
            "WHERE tenant_id = ? AND source_registry_id = ? ORDER BY fetched_at DESC, id DESC LIMIT 1",
            (tenant_id, source_id),
        ).fetchone()
        latest_version = _row_dict(latest_version_row) or {}
        if not ready:
            return {
                "source_id": source_id,
                "tenant_id": tenant_id,
                "registry_status": str(source.get("status") or ""),
                "validation_status": str(source.get("validation_status") or "unvalidated"),
                "fetch_enabled": bool(source.get("fetch_enabled")),
                "freshness_status": "not_configured",
                "error_code": "SOURCE_FRESHNESS_SCHEMA_UNAVAILABLE",
                "observation_count": 0,
                "latest_version": {
                    "id": str(latest_version.get("id") or ""),
                    "content_hash": str(latest_version.get("content_hash") or ""),
                    "byte_size": int(latest_version.get("byte_size") or 0),
        ...
        rows = conn.execute(
            "SELECT checked_at, outcome, detection_type, error_code, content_hash, source_version_id, byte_size, http_status "
            "FROM kb_source_refresh_observation WHERE tenant_id = ? AND source_registry_id = ? "
            "ORDER BY checked_at DESC, id DESC LIMIT 200",
            (tenant_id, source_id),
        ).fetchall()
        observations = [_row_dict(row) or {} for row in rows]
        ...
        for item in observations:
            ...
            if outcome in counts:
                counts[outcome] += 1
            detection = str(item.get("detection_type") or "none")
            if detection in counts:
                counts[detection] += 1
        if not observations:
            freshness_status = "never_checked"
        ...
        return {
            ...
            "last_error_code": str(latest.get("error_code") or "")[:128],
            "last_content_hash": str(latest.get("content_hash") or "")[:64],
            "last_source_version_id": str(latest.get("source_version_id") or ""),
