    ...
    for metric in metrics:
        ...
        if lookups is not None:
            ...
            for lookup in lookups:
                if not isinstance(lookup, Mapping) or not _METRIC_NAME_RE.fullmatch(str(lookup.get("labelname") or "")):
...
def _fallback_exporter_modules() -> dict[str, dict[str, Any]]:
    ...
    return {
        GENERIC_IF_MIB_VARIANT["variant_key"]: {
            ...
            "metrics": [
                {"name": name, "oid": oid, "type": "DisplayString" if name in {"ifName", "ifAlias"} else ("counter" if "Octets" in name or "Errors" in name or "Discards" in name else "gauge"), "help": name, "indexes": [{"labelname": "ifIndex", "type": "gauge"}]}
                for name, oid in (
...
def render_exporter_modules(
    ...
    *,
    ...
    selected_keys = {str(_row(item).get("variant_key") or "") for item in assignments if _row(item).get("enabled", True) is not False}
    ...
    for device_value in devices:
        ...
        for entry_value in module_entries:
            ...
            assignment = Assignment(
                ...
                variant_key=str(variant.get("variant_key") or variant_key),
...
def render_server_targets(servers: Iterable[Mapping[str, Any]] = ()) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    linux_targets: list[dict[str, Any]] = []
    ...
    for server_val in servers:
        ...
        site_name = str(server.get("site_name") or server.get("site") or "").strip()
        ...
        if is_windows:
            windows_targets.append(entry)
        else:
            linux_targets.append(entry)

    linux_targets.sort(key=lambda x: x["labels"].get("hostname", ""))
    windows_targets.sort(key=lambda x: x["labels"].get("hostname", ""))
...
def render_vmagent_config(snapshot_path: str = "/etc/nexora-collector/runtime/vmagent/targets/snmp_targets.yml") -> dict[str, Any]:
    return {
        ...
        "scrape_configs": [
            ...
            },
            {
                ...
                "file_sd_configs": [
                    {"files": [
                        "/etc/nexora-collector/runtime/vmagent/targets/linux_targets.yml",
            ...
            },
            {
                ...
                "file_sd_configs": [
                    {"files": [
                        "/etc/nexora-collector/runtime/vmagent/targets/windows_targets.yml",
...
def compile_artifact(
    ...
    *,
    ...
    vmagent_yaml = yaml.safe_dump(render_vmagent_config(), allow_unicode=True, sort_keys=False)
