...
def list_racks(conn, datacenter: str = '') -> list[dict]:
    if datacenter:
        rows = conn.execute(
            "SELECT * FROM racks WHERE datacenter = ? ORDER BY name", (datacenter,)
        ).fetchall()
    else:
        rows = conn.execute("SELECT * FROM racks ORDER BY datacenter, name").fetchall()
...
def create_rack(conn, *, name: str, datacenter: str = '', room: str = '',
                row: str = '', total_u: int = 42, description: str = '',
                status: str = 'active', power_capacity_watts: int = 0,
                site_id: str = 'site-default', rack_code: str = '') -> dict:
    ...
    row_name = row
    remarks = description
    power_capacity_w = power_capacity_watts or 5000
    used_u = 0
    available_u = total_u
    
    now = _utc_now()
    conn.execute(
        """INSERT INTO racks (id, site_id, rack_code, rack_name, name, datacenter, room, row, room_name, row_name, total_u, used_u, available_u, power_capacity_w, power_capacity_watts, status, remarks, description, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (rack_id, site_id, rack_code, rack_name, name, datacenter, room, row, room_name, row_name, total_u, used_u, available_u, power_capacity_w, power_capacity_watts, status, remarks, description, now, now)
    )
    conn.commit()
    logger.info(f"[RackService] Created rack '{name}' in {datacenter or '(no DC)'}")
...
def update_rack(conn, rack_id: str, **fields) -> dict:
    ...
    supported_keys = (
        'name', 'datacenter', 'room', 'row', 'total_u', 'description', 'status', 'power_capacity_watts',
...
def _check_u_constraints(conn, rack_id: str, start_u: int, u_height: int,
    ...
    if start_u < 1:
        raise ValueError(f"start_u must be >= 1 (got {start_u})")
    if end_u > total_u:
        raise ValueError(
            f"Device exceeds rack capacity: U{start_u}-U{end_u} but rack only has {total_u}U"
        )
    ...
    for dev in existing:
        ...
        if start_u <= existing_end and end_u >= existing_start:
            raise ValueError(
                f"U-position conflict: U{start_u}-U{end_u} overlaps with "
                f"'{d['name']}' at U{existing_start}-U{existing_end}"
            )
...
def get_rack_stats(conn) -> dict:
    ...
    total_power_used = 0
    for r in racks:
        devices = list_rack_devices(conn, r['id'])
        total_devices += len(devices)
        occupied = set()
        for d in devices:
            occupied |= set(range(d['start_u'], d['start_u'] + d['u_height']))
            total_power_used += int(d.get('power_watts') or 0)
