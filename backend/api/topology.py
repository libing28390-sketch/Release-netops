...
def _authorized_topology_device_ids(conn, scope) -> list[str] | None:
    ...
    if scope is not None and scope.site_ids is not None and not scope.site_ids:
        return []
    clauses: list[str] = []
    params: list[str] = []
    if scope is not None and scope.site_ids is not None:
    ...
    if scope is not None and scope.tenant_id:
        clauses.append("COALESCE(s.tenant_id, d.tenant_id, 'tenant-default') = ?")
        params.append(scope.tenant_id)
    where = f"WHERE {' AND '.join(clauses)}" if clauses else ''
    rows = conn.execute(
        f'''SELECT d.id, d.platform, d.device_category, d.role
              FROM devices d
    ...
    result_ids = [
        ...
        if str(row['id'] or '').strip() and not is_server_device(dict(row))
    ]
    return result_ids
...
def get_links(
    ...
    if not managed_links:
        # Discovery writes normalized LLDP facts to the evidence graph before
        # the compatibility ``topology_links`` projection is rebuilt. During
        # that window the old code called get_current_links() a second time,
        # returning an empty rack even though verified physical edges existed.
        # Read the physical graph as a safe, read-only fallback so cascade
        # members remain available to the rack renderer.
        try:
            managed_links = get_physical_links(
                tenant_id=(scope.tenant_id if scope and scope.tenant_id else 'tenant-default'),
                site_id=site_id,
            ...
            if managed_links:
                graph_read_model = 'evidence_graph_physical_fallback'
        except Exception:
            managed_links = []
    unmanaged = (
        ...
        else get_unmanaged_neighbors(site_id=site_id, source_device_ids=device_ids, include_stale=include_stale)
    )
