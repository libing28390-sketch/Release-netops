...
from services.config_snapshot_storage import (
    CONFIG_CONTENT_TYPE,
    LEGACY_BACKUP_ROOT,
    delete_legacy_file,
...
from services.storage_service import StorageError, StorageNotFound, build_storage_service

logger = logging.getLogger(__name__)

router = APIRouter()

_BACKUP_NON_SERVER_ASSET_SQL = (
    "LOWER(COALESCE(NULLIF(TRIM(pa.asset_type), ''), 'network_device')) "
    "NOT IN ('server', 'host', 'physical_server')"
)
...
# Legacy root retained only for old rows and the compatibility export path.
# New snapshots use StorageService and live below STORAGE_LOCAL_ROOT when the
# local provider is selected.
BACKUP_ROOT = LEGACY_BACKUP_ROOT
...
def _write_config_file(vendor: str, hostname: str, ts: datetime, trigger: str, content: str, config_type: str = 'running') -> str:
    ...
    try:
        data = serialize_config_content(content)
...
def _read_config_file(file_path: str) -> str:
    return read_legacy_content(file_path)
...
def _snapshot_scope_filter(conn, user) -> tuple[str, list[str]]:
    """Build the asset tenant/site predicate for a snapshot content read."""
    ...
    # Legacy users with no tenant assignment can otherwise receive an
    # unbounded RackScope(None, None). Snapshot contents are sensitive, so
    # non-admin access without an explicit tenant must fail closed.
    ...
    clauses: list[str] = []
    params: list[str] = [scope.tenant_id]
    clauses.append('snapshot_scope_device.tenant_id = ?')
    if scope.site_ids is not None:
        placeholders = ','.join('?' for _ in scope.site_ids)
        clauses.append(f'snapshot_scope_site.id IN ({placeholders})')
        params.extend(scope.site_ids)
    clauses.append('snapshot_scope_site.tenant_id = ?')
    params.append(scope.tenant_id)

    return (
        '''EXISTS (
               SELECT 1
                 FROM devices snapshot_scope_device
                 LEFT JOIN physical_assets snapshot_scope_asset
                   ON snapshot_scope_asset.id = snapshot_scope_device.asset_id
                 LEFT JOIN sites snapshot_scope_site ON (
                       snapshot_scope_site.id = COALESCE(
                           NULLIF(snapshot_scope_asset.site_id, ''),
                           NULLIF(snapshot_scope_device.site_id, ''),
                           NULLIF(snapshot_scope_device.site, '')
                       )
                       OR (
                           COALESCE(snapshot_scope_asset.site_id, '') = ''
                           AND COALESCE(snapshot_scope_device.site_id, '') = ''
                           AND (
                               snapshot_scope_site.site_code = snapshot_scope_device.site
                               OR (
                                   snapshot_scope_site.site_name = snapshot_scope_device.site
                                   AND NOT EXISTS (
                                       SELECT 1
                                         FROM sites snapshot_scope_duplicate_site
                                        WHERE snapshot_scope_duplicate_site.site_name = snapshot_scope_device.site
                                          AND snapshot_scope_duplicate_site.id <> snapshot_scope_site.id
                                   )
                               )
                           )
                       )
                   )
                WHERE snapshot_scope_device.id = cs.device_id
                  AND ''' + ' AND '.join(clauses) + ')',
        params,
    )


def _load_snapshot_for_user(conn, snap_id: str, user):
    scope_filter, scope_params = _snapshot_scope_filter(conn, user)
    query = 'SELECT cs.* FROM config_snapshots cs WHERE cs.id = ?'
    params: list[str] = [snap_id]
    if scope_filter:
        query += f' AND ({scope_filter})'
        params.extend(scope_params)
    return conn.execute(query, tuple(params)).fetchone()


def _storage_columns(stored, *, status: str = 'READY', error: str = '') -> tuple:
    """Return additive storage metadata for snapshot INSERT statements."""
    return (
        stored.backend,
        ...
        int(stored.size or 0),
        stored.sha256 or '',
        stored.content_type or CONFIG_CONTENT_TYPE,
        status,
        error[:2000],
        _utc_now_iso(),
    )
...
def _storage_error_columns(
    ...
    *,
    ...
    spool = write_config_spool(snapshot_id, content, config_type)
    service = service or build_storage_service(purpose='config_backup')
    relative_key = object_key_for_snapshot(snapshot_id, ts, config_type)
    return (
        service.name,
        getattr(service, 'storage_config_id', None),
        service.bucket if service.name == 's3' else '',
        service.full_key(relative_key),
        '',
        spool.stat().st_size,
        '',
        CONFIG_CONTENT_TYPE,
...
def list_devices_with_backups(
    ...
    Scope:
      - Only devices with a verified vendor-first default or compatible published
        platform-profile backup action. Linux / server assets are excluded.
      - Includes devices with 0 snapshots so operators can trigger a first
        backup from the UI.
    """
    conn = get_db_connection()
    try:
        from services.config_backup_command_service import (
            CONFIG_BACKUP_DEFAULT_PLATFORM_TOKENS,
            CONFIG_BACKUP_DEFAULT_VENDOR_TOKENS,
            is_config_backup_candidate,
        )

        vendor_conditions = ["LOWER(COALESCE(d.vendor,'')) LIKE ?" for _ in CONFIG_BACKUP_DEFAULT_VENDOR_TOKENS]
        platform_conditions = ["LOWER(COALESCE(d.platform,'')) LIKE ?" for _ in CONFIG_BACKUP_DEFAULT_PLATFORM_TOKENS]
        default_identity_sql = (
            f"({' OR '.join(vendor_conditions)}) OR "
            "(LOWER(TRIM(COALESCE(d.vendor,''))) IN ('', 'other') AND "
            f"({' OR '.join(platform_conditions)}))"
        )
        identity_params = [f"{token}%" for token in CONFIG_BACKUP_DEFAULT_VENDOR_TOKENS]
        identity_params.extend(f"{token}%" for token in CONFIG_BACKUP_DEFAULT_PLATFORM_TOKENS)
        profile_action_sql = """
            EXISTS (
                SELECT 1
                FROM platform_profiles bp
                JOIN platform_releases br ON br.id = bp.current_release_id AND br.status = 'PUBLISHED'
                JOIN platform_release_actions ba ON ba.release_id = br.id
                    AND ba.action_code = 'get_running_config'
                WHERE bp.id = d.platform_profile_id
                    AND UPPER(COALESCE(bp.status, 'ACTIVE')) <> 'ARCHIVED'
            )
        """
        asset_type_sql = "LOWER(COALESCE(NULLIF(pa.asset_type, ''), 'network_device')) NOT IN ('server', 'host', 'physical_server')"
        where_sql = f"WHERE (({default_identity_sql}) OR ({profile_action_sql})) AND {asset_type_sql}"
        params: list = list(identity_params)
        if site_id:
        ...
        all_rows = conn.execute(f'''
            SELECT d.id, d.hostname, d.ip_address, d.platform, d.vendor, d.platform_profile_id, d.status,
                   bp.vendor AS backup_profile_vendor,
                   bp.platform_code AS backup_profile_platform_code,
                   bp.parser_platform AS backup_profile_parser_platform,
                   ({profile_action_sql}) AS has_published_backup_action,
            ...
            FROM devices d
            {joins_sql}
            LEFT JOIN platform_profiles bp ON bp.id = d.platform_profile_id
            LEFT JOIN config_snapshots cs ON cs.device_id = d.id
            {where_sql}
            GROUP BY d.id, d.hostname, d.ip_address, d.platform, d.vendor, d.platform_profile_id, d.status,
                     bp.vendor, bp.platform_code, bp.parser_platform,
        ...
        eligible_rows = [
            dict(row)
            for row in all_rows
            if is_config_backup_candidate(
                row["vendor"],
                row["platform"],
                row["asset_type"],
                has_published_profile_action=bool(row["has_published_backup_action"]),
                profile_vendor=row["backup_profile_vendor"],
                profile_platform_code=row["backup_profile_platform_code"],
                profile_parser_platform=row["backup_profile_parser_platform"],
            )
        ]
        ...
        public_rows = [
            {key: item[key] for key in (
                "id", "hostname", "ip_address", "platform", "status", "site_id", "site_name", "site_code",
                "asset_type", "device_category", "device_role", "backup_count", "latest_backup",
            )}
            for item in rows
        ]
        ...
        return {
            ...
            'page': page,
            'page_size': page_size,
            'tree': _build_device_scope_tree(eligible_rows),
        }
    finally:
        conn.close()
    ...
    try:
        ...
        data_sql = f'''
            SELECT d.id AS device_id, d.hostname, d.vendor, d.ip_address, d.status AS device_status,
                   ...
                   cs.storage_backend, cs.storage_bucket, cs.object_key,
                   cs.object_size, cs.object_sha256, cs.content_type, cs.storage_status
            FROM devices d
            LEFT JOIN (
                SELECT cs1.*
        ...
        for r in rows:
            items.append({
                ...
                'storage_backend': r['storage_backend'] or 'local',
                'storage_bucket': r['storage_bucket'] or '',
                'object_key': r['object_key'] or '',
                'object_size': r['object_size'] or 0,
                'object_sha256': r['object_sha256'] or '',
                'content_type': r['content_type'] or CONFIG_CONTENT_TYPE,
                'storage_status': r['storage_status'] or 'LEGACY',
            })
    ...
    try:
        ...
        if run_ids:
            ...
            count_rows = conn.execute(
                f'''SELECT rd.run_id,
                           ...
                           SUM(CASE WHEN {_BACKUP_NON_SERVER_ASSET_SQL} THEN 1 ELSE 0 END) AS total_devices,
                           SUM(CASE WHEN {_BACKUP_NON_SERVER_ASSET_SQL} AND rd.status = 'success' THEN 1 ELSE 0 END) AS success_count,
                           SUM(CASE WHEN {_BACKUP_NON_SERVER_ASSET_SQL} AND rd.status = 'failed' THEN 1 ELSE 0 END) AS failed_count,
                           SUM(CASE WHEN {_BACKUP_NON_SERVER_ASSET_SQL} AND rd.status = 'skipped' THEN 1 ELSE 0 END) AS skipped_count
            ...
            for count_row in count_rows:
                if int(count_row['ledger_total'] or 0) > 0:
                    run_counts_map[count_row['run_id']] = {
                        key: int(count_row[key] or 0)
                        for key in ('total_devices', 'success_count', 'failed_count', 'skipped_count')
                    }
            site_rows = conn.execute(
                f'''SELECT rd.run_id,
                    ...
                    WHERE rd.run_id IN ({placeholders}) AND {_BACKUP_NON_SERVER_ASSET_SQL}
        ...
        return {
            'items': [
                {
                    **dict(row),
                    **run_counts_map.get(row['id'], {}),
                    'site_summary': site_summary_map.get(row['id'], []),
                }
                for row in rows
            ],
            'total': total,
...
def get_backup_run(
    ...
    try:
        ...
        ledger_total = conn.execute(
            'SELECT COUNT(*) AS total FROM config_backup_run_devices WHERE run_id = ?',
            (run_id,),
        ).fetchone()['total']
        run_payload = dict(run)
        if int(ledger_total or 0) > 0:
            run_counts = conn.execute(
                f'''SELECT COUNT(*) AS total_devices,
                    ...
                    LEFT JOIN devices d ON d.id = rd.device_id
                    LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                    WHERE rd.run_id = ? AND {_BACKUP_NON_SERVER_ASSET_SQL}''',
                (run_id,),
            ).fetchone()
            for key in ('total_devices', 'success_count', 'failed_count', 'skipped_count'):
                run_payload[key] = int(run_counts[key] or 0)
        ...
        where = ['rd.run_id = ?', _BACKUP_NON_SERVER_ASSET_SQL]
        params: list = [run_id]
...
def create_snapshot(body: SnapshotCreate):
    """Save a config snapshot through the configured storage provider + DB."""
    ts = _utc_now()
    ...
    try:
        storage_service = build_storage_service(purpose='config_backup')
        stored = put_snapshot_content(
            snapshot_id=snap_id,
            timestamp=ts,
        ...
        logger.error("[Storage] Config snapshot %s upload failed: %s", snap_id, type(exc).__name__)
        # Preserve the already-validated encrypted payload as an explicit
        # spool so the scheduled retry job can publish it later.  This is not
        # a provider fallback: the row remains bound to the configured
        # provider and is visibly marked STORAGE_ERROR.
        try:
            spool = write_config_spool(snap_id, body.content, 'running')
            failed_service = storage_service or build_storage_service(purpose='config_backup')
            conn_error = get_db_connection()
            try:
                conn_error.execute(
                    '''INSERT INTO config_snapshots
                       (id, device_id, hostname, vendor, timestamp, trigger, author, tag, file_path, size,
                        ...
                        storage_backend, storage_config_id, storage_bucket, object_key, object_version_id, object_size,
                        object_sha256, content_type, storage_status, storage_error, storage_updated_at,
                        storage_spool_path)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                    (snap_id, body.device_id, body.hostname, body.vendor, ts.isoformat(), body.trigger,
                     body.author, body.tag or '', '', size, metadata['raw_hash'], metadata['normalized_hash'],
                     ...
                     failed_service.name, getattr(failed_service, 'storage_config_id', None),
                     failed_service.bucket if failed_service.name == 's3' else '',
                     failed_service.full_key(object_key_for_snapshot(snap_id, ts, 'running')), '', spool.stat().st_size, '',
                     CONFIG_CONTENT_TYPE, 'ERROR', 'storage upload failed', _utc_now_iso(), str(spool))
                )
                conn_error.commit()
            finally:
                conn_error.close()
        except Exception as spool_exc:
            logger.error("[Storage] Could not persist config snapshot spool %s: %s", snap_id, type(spool_exc).__name__)
        raise HTTPException(
            status_code=503,
            detail={
                'message': '配置内容已校验，但对象存储不可用，未创建有效版本',
                'code': 'CONFIG_SNAPSHOT_STORAGE_ERROR',
            },
        ) from exc
    ...
    try:
        conn.execute(
            ...
            (snap_id, body.device_id, body.hostname, body.vendor,
             ...
             '', size, metadata['raw_hash'], metadata['normalized_hash'],
             metadata['line_count'], metadata['section_count'], metadata['integrity_status'],
             metadata['lifecycle_status'], metadata['normalizer_version'], ts.isoformat(), '',
             *_storage_columns(stored), '')
        )
        conn.commit()
    finally:
        conn.close()
    ...
    return {
        ...
        'storage_config_id': getattr(stored, 'storage_config_id', '') or '',
        'storage_bucket': stored.bucket or '',
        'object_key': stored.object_key,
        'object_version_id': stored.version_id or '',
        'object_size': stored.size,
        'object_sha256': stored.sha256 or '',
        'content_type': stored.content_type or CONFIG_CONTENT_TYPE,
...
def get_snapshot_content(snap_id: str, user=require_role('Viewer')):
    """Return full config content for a snapshot."""
    conn = get_db_connection()
    try:
        row = _load_snapshot_for_user(conn, snap_id, user)
    finally:
        conn.close()
    ...
    try:
        content = read_snapshot_content(row, strict_storage_errors=True)
    ...
    return {
        ...
        'storage_config_id': row['storage_config_id'] or '',
        'storage_bucket': row['storage_bucket'] or '',
        'object_key': row['object_key'] or '',
        'storage_status': row['storage_status'] or 'LEGACY',
        'object_size': row['object_size'] or 0,
        'object_sha256': row['object_sha256'] or '',
        'content_type': row['content_type'] or CONFIG_CONTENT_TYPE,
    }


@router.get('/configs/snapshots/{snap_id}/download')
def download_snapshot(snap_id: str, user=require_role('Viewer')):
...
def delete_snapshot(snap_id: str):
    ...
    try:
        ...
        if not row:
            raise HTTPException(status_code=404, detail='Snapshot not found')
        try:
            delete_snapshot_object(row)
        except Exception as exc:
            logger.error("[Storage] Config snapshot %s delete failed: %s", snap_id, type(exc).__name__)
            raise HTTPException(status_code=503, detail='对象存储删除失败，未删除快照元数据') from exc
    ...
    try:
        ...
        today_total = conn.execute(
            f'''SELECT COUNT(*) AS c
                ...
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE cs.timestamp >= ? AND {_BACKUP_NON_SERVER_ASSET_SQL}''',
            (today_start,),
        ).fetchone()['c']
        today_scheduled = conn.execute(
            f'''SELECT COUNT(*) AS c
                ...
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE cs.timestamp >= ? AND cs.trigger = 'scheduled'
                  AND {_BACKUP_NON_SERVER_ASSET_SQL}''',
            (today_start,)
        ).fetchone()['c']
        today_manual = conn.execute(
            f'''SELECT COUNT(*) AS c
                ...
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE cs.timestamp >= ? AND cs.trigger = 'manual'
                  AND {_BACKUP_NON_SERVER_ASSET_SQL}''',
            (today_start,)
        ).fetchone()['c']
        ...
        today_run_counts = conn.execute(
            f'''SELECT
               ...
               LEFT JOIN devices d ON d.id = rd.device_id
               LEFT JOIN physical_assets pa ON pa.id = d.asset_id
               WHERE r.started_at >= ? AND {_BACKUP_NON_SERVER_ASSET_SQL}''',
            (today_start,),
        ).fetchone()
        ...
        daily_rows = conn.execute(
            f'''SELECT DATE(cs.timestamp) AS day, cs.trigger, COUNT(*) AS cnt
                FROM config_snapshots cs
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE cs.timestamp >= ? AND {_BACKUP_NON_SERVER_ASSET_SQL}
                GROUP BY DATE(cs.timestamp), cs.trigger ORDER BY day''',
            (seven_days_ago,)
        ).fetchall()
        daily_map: dict = {}
        for r in daily_rows:
        ...
        all_device_ids = conn.execute(
            f'''SELECT d.id
                FROM devices d
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE d.status = 'online' AND {_BACKUP_NON_SERVER_ASSET_SQL}'''
        ).fetchall()
        today_backed_ids = conn.execute(
            f'''SELECT DISTINCT cs.device_id
                FROM config_snapshots cs
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE cs.timestamp >= ? AND {_BACKUP_NON_SERVER_ASSET_SQL}''',
        ...
        total_snapshots = conn.execute(
            f'''SELECT COUNT(*) AS c
                ...
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE {_BACKUP_NON_SERVER_ASSET_SQL}'''
        ).fetchone()['c']
        total_devices_backed = conn.execute(
            f'''SELECT COUNT(DISTINCT cs.device_id) AS c
                FROM config_snapshots cs
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE {_BACKUP_NON_SERVER_ASSET_SQL}'''
        ...
        storage_row = conn.execute(
            f'''SELECT COALESCE(SUM(cs.size), 0) AS s
                FROM config_snapshots cs
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE {_BACKUP_NON_SERVER_ASSET_SQL}'''
        ...
        devices = conn.execute(
            f'''SELECT d.id, d.hostname, d.ip_address, d.platform, d.status
                FROM devices d
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE {_BACKUP_NON_SERVER_ASSET_SQL}'''
        ).fetchall()
        ...
        agg_rows = conn.execute(
            f'''SELECT cs.device_id, COUNT(*) AS cnt, MAX(cs.timestamp) AS latest
                FROM config_snapshots cs
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE {_BACKUP_NON_SERVER_ASSET_SQL}
        ...
        last_trigger_rows = conn.execute(
            f'''SELECT cs.device_id, cs.trigger
                FROM config_snapshots cs
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                INNER JOIN (
                    SELECT cs_latest.device_id, MAX(cs_latest.timestamp) AS max_ts
                    FROM config_snapshots cs_latest
                    LEFT JOIN devices d_latest ON d_latest.id = cs_latest.device_id
                    LEFT JOIN physical_assets pa_latest ON pa_latest.id = d_latest.asset_id
                    WHERE LOWER(COALESCE(NULLIF(TRIM(pa_latest.asset_type), ''), 'network_device'))
                          NOT IN ('server', 'host', 'physical_server')
                    GROUP BY cs_latest.device_id
        ...
        recent = conn.execute(
            f'''SELECT cs.id, cs.device_id, cs.hostname, cs.vendor, cs.timestamp,
                       cs.trigger, cs.author, cs.tag, cs.size
                FROM config_snapshots cs
                LEFT JOIN devices d ON d.id = cs.device_id
                LEFT JOIN physical_assets pa ON pa.id = d.asset_id
                WHERE {_BACKUP_NON_SERVER_ASSET_SQL}
                ORDER BY cs.timestamp DESC LIMIT 20'''
        ).fetchall()
...
async def trigger_backup_now(user=require_role('Administrator')):
...
def preview_schedule(n: int = Query(default=10, ge=1, le=50)):
    ...
    # 备份涉及的设备列表
    from services.config_backup_command_service import is_config_backup_candidate

    conn = get_db_connection()
    try:
        rows = conn.execute(
            """SELECT d.id, d.hostname, d.ip_address, d.platform, d.vendor, d.status,
                      d.platform_profile_id,
                      COALESCE(NULLIF(pa.asset_type, ''), 'network_device') AS asset_type,
                      p.vendor AS backup_profile_vendor,
                      p.platform_code AS backup_profile_platform_code,
                      p.parser_platform AS backup_profile_parser_platform,
                      EXISTS (
                          SELECT 1 FROM platform_profiles bp
                          JOIN platform_releases br ON br.id = bp.current_release_id AND br.status = 'PUBLISHED'
                          JOIN platform_release_actions ba ON ba.release_id = br.id
                              AND ba.action_code = 'get_running_config'
                          WHERE bp.id = d.platform_profile_id
                              AND UPPER(COALESCE(bp.status, 'ACTIVE')) <> 'ARCHIVED'
                      ) AS has_published_backup_action
               FROM devices d
               LEFT JOIN physical_assets pa ON pa.id = d.asset_id
        ...
        for row in rows:
            item = dict(row)
            if (item.get('ip_address') or '') in MOCK_IPS:
                continue
            if not is_config_backup_candidate(
                item.get('vendor'),
                item.get('platform'),
                item.get('asset_type'),
                has_published_profile_action=bool(item.get('has_published_backup_action')),
                profile_vendor=item.get('backup_profile_vendor'),
                profile_platform_code=item.get('backup_profile_platform_code'),
                profile_parser_platform=item.get('backup_profile_parser_platform'),
            ):
                continue
            devices.append({key: item[key] for key in ('id', 'hostname', 'ip_address', 'platform', 'status')})
    finally:
        conn.close()
...
def _cleanup_expired_snapshots(
    *,
    ...
    try:
        ...
        old = conn.execute(
            f'SELECT * FROM config_snapshots WHERE timestamp < ?{policy_clause}',
            (cutoff, policy_id) if policy_id else (cutoff,),
        ).fetchall()
        old_deleted_ids: list[str] = []
        for s in old:
            try:
                delete_snapshot_object(s)
                old_deleted_ids.append(str(s['id']))
            except Exception as exc:
                logger.warning("[Storage] Retention delete failed for snapshot %s: %s", s['id'], type(exc).__name__)
        if old_deleted_ids:
            conn.execute(
                f"DELETE FROM config_snapshots WHERE id IN ({','.join('?' * len(old_deleted_ids))})",
                old_deleted_ids,
            )
            deleted += len(old_deleted_ids)
        ...
        for d in devs:
            excess = conn.execute(
                f'''SELECT * FROM config_snapshots
            ...
            excess_deleted_ids: list[str] = []
            for s in excess:
                try:
                    delete_snapshot_object(s)
                    excess_deleted_ids.append(str(s['id']))
                except Exception as exc:
                    logger.warning("[Storage] Retention delete failed for snapshot %s: %s", s['id'], type(exc).__name__)
            if excess_deleted_ids:
                conn.execute(
                    f"DELETE FROM config_snapshots WHERE id IN ({','.join('?' * len(excess_deleted_ids))})",
                    excess_deleted_ids
                )
                deleted += len(excess_deleted_ids)
...
async def run_scheduled_backup(
    ...
    try:
        ...
        device_rows = conn.execute(
            "SELECT d.id, d.asset_id, d.hostname, d.ip_address, d.platform, d.vendor, d.role, d.site, d.site_id, d.status, "
            "d.platform_profile_id, COALESCE(NULLIF(pa.asset_type, ''), 'network_device') AS asset_type, "
            "p.vendor AS backup_profile_vendor, p.platform_code AS backup_profile_platform_code, "
            "p.parser_platform AS backup_profile_parser_platform, "
            "EXISTS (SELECT 1 FROM platform_profiles bp "
            "JOIN platform_releases br ON br.id = bp.current_release_id AND br.status = 'PUBLISHED' "
            "JOIN platform_release_actions ba ON ba.release_id = br.id AND ba.action_code = 'get_running_config' "
            "WHERE bp.id = d.platform_profile_id AND UPPER(COALESCE(bp.status, 'ACTIVE')) <> 'ARCHIVED') "
            "AS has_published_backup_action, "
            "d.config_backup_enabled, d.username, d.password, "
            "d.normal_username, d.normal_password, d.admin_username, d.admin_password, d.enable_password, "
            "d.credential_id, d.admin_credential_id, d.credential_source, d.vault_path, d.management_port "
            "FROM devices d LEFT JOIN physical_assets pa ON pa.id = d.asset_id "
            "LEFT JOIN platform_profiles p ON p.id = d.platform_profile_id WHERE d.status = 'online'"
        ).fetchall()
        devices = (
    ...
    # Keep only identifiable network equipment in the persisted run and its
    # denominator. Unsupported network devices remain visible as skipped; host
    # assets and unclassified non-network devices do not become skipped rows.
    network_devices = []
    eligible = []
    skipped_count = 0
    for device in devices:
        candidate = is_config_backup_candidate(
            device.get('vendor'),
            device.get('platform'),
            device.get('asset_type'),
            has_published_profile_action=bool(device.get('has_published_backup_action')),
            profile_vendor=device.get('backup_profile_vendor'),
            profile_platform_code=device.get('backup_profile_platform_code'),
            profile_parser_platform=device.get('backup_profile_parser_platform'),
        )
        is_linked_network_asset = (
            bool(device.get('asset_id'))
            and str(device.get('asset_type') or '').strip().lower() == 'network_device'
        )
        if not is_linked_network_asset and not is_network_device_for_config_backup(
            device.get('vendor'), device.get('platform'), device.get('asset_type')
        ) and not candidate:
            continue
        ...
        if not candidate:
            skipped_count += 1
            logger.info(f"[Scheduled Backup] [SKIP] {device['hostname']} (no verified default command or published backup action)")
            continue
    ...
    async def _backup_one(device) -> dict:
        ...
        default_commands = get_default_config_backup_commands(device.get('vendor'), _platform)
        use_profile_action = not default_commands and is_compatible_published_config_action(
            device.get('vendor'),
            device.get('platform'),
            device.get('backup_profile_vendor'),
            device.get('backup_profile_platform_code'),
            device.get('backup_profile_parser_platform'),
            has_published_profile_action=bool(device.get('has_published_backup_action')),
        )
        if default_commands:
            _platform = get_config_backup_platform(device.get('vendor'), _platform)
        if not default_commands and not use_profile_action:
            failed_count += 1
            return {
                'hostname': device['hostname'],
                'status': 'failed',
                'reason': '厂商未适配',
                'detail': '该厂商尚无默认配置备份命令，请先补充经过核实的命令映射。',
            }
        _running_cmd = (default_commands or {}).get('get_running_config', '')
        _startup_cmd = (default_commands or {}).get('get_startup_config', '')
            ...
            for attempt in range(run_retry_count + 1):
                try:
                    ...
                    if use_profile_action:
                        ...
                        commands = [str(registry_result.get('command') or _running_cmd)]
                        results = [{
                        ...
                        if collect_startup_config and results[0].get('success'):
                            ...
                            commands.append(str(st_reg_result.get('command') or _startup_cmd))
                            results.append({
        ...
        try:
            ...
            try:
                ...
                storage_fields = _storage_columns(stored) + ('',)
            except Exception as storage_exc:
                logger.error("[Scheduled Backup] Storage failed for %s: %s", device['hostname'], type(storage_exc).__name__)
                stored = None
                storage_fields = _storage_error_columns(snap_id, ts, output, 'running', service=storage_service)
            
            created_backup_items.append({
                'local_path': '',
                'remote_name': os.path.basename(stored.object_key if stored else storage_fields[3]),
                'archive_name': stored.object_key if stored else storage_fields[3],
                'hostname': device['hostname'],
            ...
            if collect_startup_config and startup_output and not _looks_like_error(startup_output) and not startup_output.startswith('% '):
                try:
                    ...
                    if st_metadata['integrity_status'] == 'verified':
                        ...
                        startup_storage_service = None
                        try:
                            startup_storage_service = build_storage_service(purpose='config_backup')
                            st_stored = put_snapshot_content(
                                snapshot_id=startup_snap_id,
                                timestamp=ts,
                                content=startup_output,
                                config_type='startup',
                                service=startup_storage_service,
                            )
                            startup_storage_fields = _storage_columns(st_stored) + ('',)
                        except Exception as storage_exc:
                            logger.error("[Scheduled Backup] Startup storage failed for %s: %s", device['hostname'], type(storage_exc).__name__)
                            st_stored = None
                            startup_storage_fields = _storage_error_columns(
                                startup_snap_id,
                                ts,
                                startup_output,
                                'startup',
                        ...
                        try:
                            conn_st.execute(
                                ...
                                (startup_snap_id, device['id'], device['hostname'], vendor,
                                 ts.isoformat(), run_trigger, run_author, '', '', st_size,
                                 ...
                                 has_unsaved, unsaved_summary, *startup_storage_fields)
                            )
                            conn_st.commit()
                        finally:
                            conn_st.close()
                        ...
                        created_backup_items.append({
                            'local_path': '',
                            'remote_name': os.path.basename(st_stored.object_key if st_stored else startup_storage_fields[3]),
                            'archive_name': st_stored.object_key if st_stored else startup_storage_fields[3],
                            'hostname': device['hostname'],
            ...
            try:
                conn2.execute(
                    ...
                    (snap_id, device['id'], device['hostname'], vendor,
                     ts.isoformat(), run_trigger, run_author, '', '', size,
                     ...
                     has_unsaved, unsaved_summary, *storage_fields)
                )
                conn2.commit()
            finally:
                conn2.close()
            ...
            return {
                ...
                'storage_status': storage_fields[8],
                'storage_error': storage_fields[9],
            }
        except Exception as e:
            failed_count += 1
...
async def sync_snapshot_to_tftp(payload: TFTPSyncSnapshotPayload):
    ...
    try:
        ...
        config_type = snap_dict.get('config_type') or 'running'
    finally:
        conn.close()
    ...
    object_name = str(snap_dict.get('object_key') or snap_dict.get('file_path') or snap_dict.get('id'))
    remote_name = os.path.basename(object_name)
    target_path = f"{options['path_prefix'].strip('/')}/{remote_name}" if options['path_prefix'].strip() else remote_name

    content = read_snapshot_content(snap_dict)
    if not content:
...
async def sync_batch_to_tftp(payload: TFTPSyncBatchPayload):
    ...
    try:
        ...
        snapshots = conn.execute("SELECT * FROM config_snapshots WHERE task_id = ?", (payload.run_id,)).fetchall()
    finally:
        conn.close()
    ...
    items = []
    for s in snapshots:
        snapshot_dict = dict(s)
        content = read_snapshot_content(snapshot_dict)
        object_name = str(snapshot_dict.get('object_key') or snapshot_dict.get('file_path') or snapshot_dict.get('id'))
        if content:
            items.append({
                'local_path': '',
                'remote_name': os.path.basename(object_name),
            ...
            logger.warning("[Archive Sync] Could not decode snapshot %s; skipping remote upload", s['id'])
