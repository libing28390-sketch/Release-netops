...
def _platform_registry_enabled() -> bool:
...
async def parse_device_output(execution_id: str, device_id: str, request: Request, payload: dict = Body(...), user=require_permission("playbook", "test")):
    ...
    template_path = _find_template(parser_platform, command)
    ...
    records = parse_with_textfsm(parser_platform, command, raw_output)
    if not records:
        return {
            'success': True,
...
async def preview_playbook(payload: dict = Body(...), user=require_permission("playbook", "test")):
    ...
    return {
        ...
        "registry_actions_enabled": _platform_registry_enabled(),
    }
...
async def execute_playbook(
    ...
    if approval_steps and not _platform_registry_enabled():
        raise HTTPException(status_code=503, detail={
            'code': 'PLATFORM_REGISTRY_DISABLED',
            'message': 'Approval steps require the controlled Playbook registry to be enabled',
        })
