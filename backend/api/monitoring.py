...
from services.metric_provider import extract_instant_metric_value, provider_from_environment
...
def _monitoring_tenant_scope(user: Any) -> tuple[str, tuple[Any, ...]]:
    ...
    historical unrestricted helper behavior; HTTP requests resolve a Viewer
    or Operator dependency and are filtered before any telemetry is read.
    """
...
def _monitoring_tenant_id(user: Any) -> str | None:
    if isinstance(user, dict) and str(user.get('role') or '') == 'Administrator':
        return None
    return str(user.get('tenant_id') or 'tenant-default') if isinstance(user, dict) else 'tenant-default'


def _assert_monitoring_device_visible(conn, device_id: str, user: Any) -> None:
    tenant_id = _monitoring_tenant_id(user)
    if tenant_id is None:
        return
    row = conn.execute(
        "SELECT 1 FROM devices WHERE id = ? AND COALESCE(tenant_id, 'tenant-default') = ?",
        (device_id, tenant_id),
    ).fetchone()
    if not row:
...
def _assert_monitoring_incident_visible(conn, incident_id: str, user: Any) -> None:
    tenant_id = _monitoring_tenant_id(user)
    if tenant_id is None:
        return
    row = conn.execute(
        ...
        SELECT i.id
         ...
         WHERE i.id = ?
           AND i.primary_device_id IS NOT NULL
           AND EXISTS (
               SELECT 1 FROM devices primary_device
                WHERE primary_device.id = i.primary_device_id
                  AND COALESCE(primary_device.tenant_id, 'tenant-default') = ?
           )
           AND NOT EXISTS (
               SELECT 1
                ...
                WHERE ia.incident_id = i.id
                  AND (a.device_id IS NULL OR COALESCE(d.tenant_id, 'tenant-default') <> ?)
           )
        """,
...
def monitoring_collection_status(
    ...
    if device_id:
        ...
        try:
            _assert_monitoring_device_visible(conn, device_id, user)
        finally:
            conn.close()
    items = list_collection_status(device_id=device_id, site_id=site_id, collector=collector, tenant_id=tenant_id)
    return {
        'items': items,
        'summary': collection_status_summary(site_id=site_id, tenant_id=tenant_id),
    }


@router.get('/monitoring/device/{device_id}/collection-status')
def monitoring_device_collection_status(device_id: str, user=require_role('Viewer')):
    conn = get_db_connection()
    try:
        _assert_monitoring_device_visible(conn, device_id, user)
    finally:
        conn.close()
    items = list_collection_status(device_id=device_id, tenant_id=_monitoring_tenant_id(user))
    return {
        'device_id': device_id,
        'items': items,
    }
...
async def monitoring_device_diagnostics(
    device_id: str,
    request: Request,
    incident_id: Optional[str] = Query(default=None),
    _user=require_role('Operator'),
):
    user = _user or _require_monitoring_session(request)
    conn = get_db_connection()
    try:
        if incident_id:
            _assert_monitoring_incident_visible(conn, incident_id, user)
        _assert_monitoring_device_visible(conn, device_id, user)
    finally:
        conn.close()
...
def search_online_devices(
    ...
    try:
        ...
        rows = conn.execute(
            ...
            LIMIT ?
            ''',
            (*tenant_params, pattern, pattern, limit),
        ).fetchall()
...
def list_monitoring_network_devices(
    ...
    tenant_id = (user.get('tenant_id') if isinstance(user, dict) and str(user.get('role') or '') != 'Administrator' else None)
    if isinstance(user, dict):
        where_parts.append("COALESCE(d.tenant_id, 'tenant-default') = ?")
        where_params.append(str(tenant_id or 'tenant-default'))
    if qv:
        pattern = f"%{qv}%"
...
@router.get('/monitoring/network-device-search')
def search_monitoring_network_devices(
    q: str = Query(..., min_length=1, max_length=120),
    limit: int = Query(default=100, ge=10, le=200),
    user=require_role('Viewer'),
):
    """Search managed network devices for a compact selector without page controls."""
    qv = (q or '').strip()
    if not qv:
        return {'items': [], 'query': qv, 'has_more': False}
    ...
    network_predicate = """
        ...
        AND LOWER(COALESCE(pa.asset_type, '')) NOT LIKE '%' || 'server' || '%'
    """
    where = [network_predicate]
    params: list[Any] = []
    if isinstance(user, dict):
        role = str(user.get('role') or '')
        tenant_id = user.get('tenant_id') if role != 'Administrator' else None
        where.append("COALESCE(d.tenant_id, 'tenant-default') = ?")
        params.append(str(tenant_id or 'tenant-default'))

    pattern = f"%{qv}%"
    where.append("(LOWER(COALESCE(d.hostname, '')) LIKE LOWER(?) OR LOWER(COALESCE(d.ip_address, '')) LIKE LOWER(?))")
    ...
    try:
        rows = conn.execute(
            f"""
            SELECT d.id, d.hostname, d.ip_address, d.platform, d.status, d.model, d.version
            FROM devices d
            LEFT JOIN physical_assets pa ON pa.id = d.asset_id
            WHERE {' AND '.join(where)}
            ORDER BY CASE WHEN LOWER(COALESCE(d.status, '')) = 'online' THEN 0 ELSE 1 END,
                     LOWER(COALESCE(d.hostname, '')), d.id
            LIMIT ?
            """,
...
def monitoring_overview(force_refresh: bool = Query(default=False), user=require_role('Viewer')):
    now_utc = datetime.now(timezone.utc).replace(microsecond=0)
    # HTTP dependencies are intentionally resolved through the route below;
    ...
    try:
        ...
        tenant_params: tuple[Any, ...] = () if tenant_id is None else (tenant_id,)
        ...
        device_counts = conn.execute(
            ...
            SELECT COUNT(*) AS total_devices,
                   ...
                   SUM(CASE WHEN d.status IS NULL OR d.status NOT IN ('online', 'offline') THEN 1 ELSE 0 END) AS unknown_devices
              FROM devices d
             WHERE 1 = 1
            """ + tenant_clause,
            tenant_params,
        ...
        devices_rows = conn.execute(
            ...
            "d.cpu_usage, d.memory_usage, d.temp, d.fan_status, d.psu_status, 'network_device' AS device_category "
            "FROM devices d LEFT JOIN sites s ON s.id = d.site_id WHERE d.status = 'online'" + tenant_clause,
            tenant_params,
        ).fetchall()
        devices = []
        ...
        open_alerts = conn.execute(
            "SELECT COUNT(*) AS c FROM alert_events a LEFT JOIN devices d ON d.id = a.device_id WHERE a.resolved_at IS NULL AND COALESCE(a.workflow_status, 'open') != 'suppressed'" + tenant_clause,
            tenant_params,
        ).fetchone()['c']
        ...
        alerts_24h = conn.execute(
            "SELECT COUNT(*) AS c FROM alert_events a LEFT JOIN devices d ON d.id = a.device_id WHERE a.created_at >= ?" + tenant_clause,
            (last_24h, *tenant_params),
        ).fetchone()['c']
        ...
        collection_summary = collection_status_summary(tenant_id=tenant_id)
        ...
        incident_page = list_incidents(conn, status='active', page=1, page_size=100, tenant_id=tenant_id)
        ...
        with _overview_cache_lock:
            _overview_cache[cache_key] = {
                'payload': payload,
                'expires_at': now_utc + timedelta(seconds=_OVERVIEW_CACHE_TTL_SECONDS),
            }
        return payload
    finally:
        conn.close()
...
def monitoring_device_realtime(
    ...
    try:
        ...
        device = conn.execute(
            ...
            "sys_name, cpu_usage, memory_usage, temp, fan_status, psu_status, asset_id FROM devices WHERE id = ?" + tenant_clause,
        ...
        # Resolve real-time hardware telemetry from provider if VM/Shadow mode
        provider_name = provider.strip().lower() if isinstance(provider, str) and provider.strip() else None
        selected_backend = (provider_name or os.environ.get("MONITORING_METRIC_BACKEND", "native")).strip().lower()
        if selected_backend in {"victoriametrics", "shadow"}:
            try:
                target_asset_id = str(device.get('asset_id') or device.get('id') or device_id)
                lookup_id = (
                    f"{device['asset_id']}|{device['id']}"
                    if device.get('asset_id') and device.get('id') and str(device['asset_id']) != str(device['id'])
                    else target_asset_id
                )
                provider_inst = provider_from_environment(mode=selected_backend)

                cpu_payload = provider_inst.instant(query="cpu_usage_percent", asset_id=lookup_id, metric_key="cpu_usage_percent")
                cpu_val = extract_instant_metric_value(cpu_payload.get("data"))
                if cpu_val is not None:
                    device['cpu_usage'] = cpu_val

                mem_payload = provider_inst.instant(query="memory_usage_percent", asset_id=lookup_id, metric_key="memory_usage_percent")
                mem_val = extract_instant_metric_value(mem_payload.get("data"))
                if mem_val is not None:
                    device['memory_usage'] = mem_val

                temp_payload = provider_inst.instant(query="temperature_celsius", asset_id=lookup_id, metric_key="temperature_celsius")
                temp_val = extract_instant_metric_value(temp_payload.get("data"))
                if temp_val is not None:
                    device['temp'] = temp_val
            except Exception as vm_err:
                logger.debug("Failed to query real-time hardware metrics from provider for %s: %s", device_id, vm_err)
...
def monitoring_device_trend(
    ...
    try:
        tenant_clause, tenant_params = _monitoring_tenant_scope(user)
        device = conn.execute(
            "SELECT id, hostname, ip_address, platform, status, device_category, asset_id FROM devices WHERE id = ?" + tenant_clause,
            (device_id, *tenant_params),
        ).fetchone()
        if not device:
            raise HTTPException(status_code=404, detail='Device not found')
        ...
        # V1 providers are opt-in per request so the existing Native response
        # remains backward compatible.  VM/Shadow results are normalized into
        # the same ``series`` envelope consumed by NetworkMonitoring.
        selected_provider = (provider or '').strip().lower()
        if selected_provider and selected_provider != 'native':
            if selected_provider not in {'shadow', 'victoriametrics'}:
                raise HTTPException(status_code=422, detail='Unsupported monitoring metric provider')
            metric_keys = ['cpu_usage_percent'] if is_server else [
                'interface_rx_bps', 'interface_tx_bps',
                'interface_in_errors_rate', 'interface_out_errors_rate',
                'interface_in_discards_rate', 'interface_out_discards_rate',
            ]
            provider_instance = provider_from_environment(mode=selected_provider)
            ...
            for metric_key in metric_keys:
                result = provider_instance.range(
                    query=metric_key,
                    metric_key=metric_key,
                    asset_id=lookup_id,
                    start=start_dt,
                    end=end_dt,
                    step_seconds=300 if range_hours > 24 else 60,
                )
                if str(result.get('status') or '').lower() == 'error':
                    provider_errors.append(str(result.get('error') or f'{metric_key} query failed'))
                if result.get('shadow'):
                    shadow_results.append(result['shadow'])
                for item in result.get('data') or []:
                    ...
                    if 'metric' in item:
                        ...
                        for timestamp, value in values:
                            ts_key = datetime.fromtimestamp(float(timestamp), tz=timezone.utc).isoformat()
                            point = points.setdefault(ts_key, {'ts_minute': ts_key})
                            point[metric_key] = _as_float(value)
                    else:
                        timestamp = item.get('timestamp') or item.get('ts_minute')
                        if not timestamp:
                            continue
                        point = points.setdefault(str(timestamp), {'ts_minute': str(timestamp)})
                        value = item.get('value')
                        point[metric_key] = _as_float(value)
            series = []
            for point in sorted(points.values(), key=lambda item: item.get('ts_minute', '')):
                if not is_server:
                    point['total_in_bps'] = point.get('interface_rx_bps')
                    point['total_out_bps'] = point.get('interface_tx_bps')
                    point['total_errors'] = sum(float(point.get(key) or 0) for key in ('interface_in_errors_rate', 'interface_out_errors_rate')) or None
                    point['total_drops'] = sum(float(point.get(key) or 0) for key in ('interface_in_discards_rate', 'interface_out_discards_rate')) or None
                series.append(point)
            return {
                'device': {'id': device['id'], 'hostname': device['hostname'], 'ip_address': device.get('ip_address'), 'platform': device.get('platform'), 'status': device['status']},
                'provider': selected_provider,
                'status': 'error' if provider_errors else 'success',
                'error': '; '.join(dict.fromkeys(provider_errors)) or None,
                'range_hours': range_hours,
                'resolution': '5m' if range_hours > 24 else '1m',
                'start_time': start_iso,
                'end_time': end_iso,
                'interface_name': interface_name.strip() if interface_name and interface_name.strip() else None,
                'series': series,
                'shadow': shadow_results or None,
            }
...
def monitoring_health_devices(
    ...
    try:
        tenant_id = _monitoring_tenant_id(user)
        evaluated = annotate_devices_with_health(conn, load_devices_for_health(conn, tenant_id=tenant_id))
        ...
        safe_items = [
            ...
            for device in items[offset:offset + limit]
        ]
        return {
            'items': safe_items,
            'total': len(items),
...
def monitoring_incidents(
    ...
    try:
        return list_incidents(
            ...
            page=page,
            page_size=page_size,
            tenant_id=_monitoring_tenant_id(_user),
        )
    finally:
        conn.close()
...
def monitoring_incident_detail(incident_id: str, _user=require_role('Viewer')):
    conn = get_db_connection()
    try:
        _assert_monitoring_incident_visible(conn, incident_id, _user)
...
def get_monitoring_outbound_status(
    ...
    # The status service is tenant-aware through its target/report queries;
    # expose only the scoped target list in this compatibility response.
...
def get_monitoring_outbound_targets(user=require_role("Viewer")):
    ...
    items = list_outbound_targets(tenant_id=_monitoring_tenant_id(user))
...
def upsert_monitoring_outbound_target(
    ...
    try:
        item = upsert_outbound_target(payload.model_dump(exclude_none=True), target_id=payload.id, tenant_id=_monitoring_tenant_id(user))
    except ValueError as exc:
...
def patch_monitoring_outbound_target(
    ...
    from services.outbound_probe_service import upsert_outbound_target
    try:
        item = upsert_outbound_target(payload.model_dump(exclude_none=True), target_id=target_id, tenant_id=_monitoring_tenant_id(user))
    except ValueError as exc:
        raise HTTPException(status_code=400, detail={"code": "INVALID_TARGET", "message": str(exc)}) from exc
    except Exception as exc:
        if "UNIQUE" in str(exc).upper():
...
def delete_monitoring_outbound_target(target_id: str, request: Request, user=require_role("Operator")):
    ...
    if not delete_outbound_target(target_id, tenant_id=_monitoring_tenant_id(user)):
        raise HTTPException(status_code=404, detail={"code": "TARGET_NOT_FOUND", "message": "Target not found"})
    _outbound_audit(
        request,
        user,
