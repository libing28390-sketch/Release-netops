    ...
    try:
        ...
        if search and search.strip():
            ...
            where_clauses.append('('
                ...
                'pa.datacenter LIKE ? OR pa.vendor LIKE ? OR pa.model LIKE ?'
                ')')
        ...
        asset_cols = (
            ', pa.asset_tag, pa.datacenter, pa.rack, pa.rack_unit, '
            'pa.department, pa.warranty_expiry, pa.purchase_date, '
            'pa.business_ip, pa.vlan, pa.uplink_switch, pa.uplink_port'
        )
        from_clause = 'FROM devices d LEFT JOIN physical_assets pa ON d.asset_id = pa.id'
        ...
        if str(mode).lower() == 'light':
            ...
            select_clause = (
                ...
                'd.fan_status, d.psu_status, d.sys_name, d.sys_location, d.sys_contact, d.asset_id, d.vendor, '
                'd.device_category'
                + asset_cols
            )
...
def update_device(device_id: str, device: dict = Body(...)):
    conn = get_db_connection()
    try:
        existing_row = conn.execute('SELECT credential_id, lifecycle_status, credential_source FROM devices WHERE id = ?', (device_id,)).fetchone()
        if not existing_row:
            raise HTTPException(status_code=404, detail='Device not found')
        ...
        if cred_source == 'vault' and vault_available() and vault_path:
            ...
            vault_write(vault_path, {
                ...
                'enable_password': submitted_enable or '',
                'priv_username': device.get('priv_username', ''),
                'normal_username': device.get('normal_username', ''),
                'normal_password': submitted_normal or '',
                'admin_username': device.get('admin_username', ''),
                'admin_password': submitted_admin or '',
            })
        else:
            username = device.get('username') or device.get('normal_username') or device.get('admin_username') or ''
...
def test_device_connection(payload: dict = Body(...)):
    ...
    logger.info(f"DEBUG CONNECT TARGET: hostname={hostname}, user={username}, pwd_len={len(password or '')}, pwd_val={password!r}")
    ...
    try:
        ...
        return _build_ssh_failure_response(hostname, ip_address, user_message, probe_output, probe_stages, status_code=500)
...
def collect_device_operational_data(device_id: str, payload: dict = Body(default={})): 
    ...
    device_info['normal_password'] = decrypt_credential(device_info.get('normal_password')) or ''
    device_info['admin_password'] = decrypt_credential(device_info.get('admin_password')) or ''
    device_info['enable_password'] = decrypt_credential(device_info.get('enable_password')) or ''

    categories = payload.get('categories') if isinstance(payload, dict) else None
    auth_role = payload.get('auth_role', 'normal') if isinstance(payload, dict) else 'normal'
