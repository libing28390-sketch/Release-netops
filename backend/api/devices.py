...
async def trigger_sync_librenms_mibs(
    ...
    """Sync LibreNMS MIBs for platform vendors plus standard MIBs."""
    ...
    def _sync_worker(target_vendors_list: list[str], retire_unlisted_vendors: bool):
        ...
        _librenms_sync_state["progress"] = "Fetching LibreNMS MIBs for supported platform vendors..."
        _librenms_sync_state["started_at"] = time.time()
        try:
            fetch_librenms_sources(TARGET_DIR)
            ...
            rule_result = sync_rules_from_repository(TARGET_DIR)
    ...
    return {
        "success": True,
        "message": "LibreNMS MIB sync started for registered platform vendors",
    ...
    if results['success']:
        ...
        async def _background_sync(prefetched=None):
            try:
                ...
                hostname_sync_result = None
                conn2 = get_db_connection()
                try:
                    if dev_info.get('sys_name'):
                        from services.hostname_sync_service import sync_device_hostname_metadata

                        hostname_sync_result = sync_device_hostname_metadata(conn2, device_id, dev_info['sys_name'])
                    if updates:
                    ...
                    conn2.close()
                if hostname_sync_result:
                    from services.hostname_sync_service import schedule_monitoring_target_sync_if_hostname_changed

                    schedule_monitoring_target_sync_if_hostname_changed(hostname_sync_result)
                record_collection_result(
                    device_id,
                    'snmp_interfaces',
...
def clean_system_hostname(raw_sys_name: str) -> str:
    """Clean standard RFC 1213 MIB-II sysName into a canonical management hostname.

    Works across all vendors without requiring any vendor-specific templates:
    strips trailing newlines, domain suffixes (e.g. '.CCIE', '.corp.local', '.eve.lab'),
    and extra whitespace.
    """
    from services.hostname_sync_service import clean_system_hostname as _clean_system_hostname
    return _clean_system_hostname(raw_sys_name)


@router.post("/devices/{device_id}/sync-hostname")
async def sync_device_hostname(device_id: str, user=require_role("Operator")):
    """Fetch actual sysName from device via RFC1213 standard SNMP MIB-II (1.3.6.1.2.1.1.5.0).

    Does not require any vendor template: works uniformly across all network vendors.
    Cleans domain suffixes, updates devices.hostname, devices.sys_name, and physical_assets.hostname,
    and updates topology observation matching.
    """
    conn = get_db_connection()
    try:
        device = conn.execute('SELECT * FROM devices WHERE id = ?', (device_id,)).fetchone()
    ...
    if not snmp_ip:
        raise HTTPException(status_code=400, detail="设备未配置管理 IP 地址")
    if not community:
        raise HTTPException(status_code=400, detail="设备未配置 SNMP 社区名/凭据，无法通过 SNMP 读取实际主机名")

    from services.snmp_service import _snmp_get
    try:
        # RFC 1213 sysName.0 is universal across all hardware vendors
        raw_sys_name = await _snmp_get(snmp_ip, community, '1.3.6.1.2.1.1.5.0', port, timeout=3)
    except Exception as exc:
        logger.warning("SNMP sysName read failed for %s: %s", snmp_ip, exc)
        raise HTTPException(status_code=502, detail=f"SNMP 连接失败: {exc}")

    if not raw_sys_name:
        raise HTTPException(status_code=502, detail="设备 SNMP 响应中 sysName.0 为空，请检查设备配置")

    clean_name = clean_system_hostname(raw_sys_name) or str(raw_sys_name).strip()
    previous_hostname = device_data.get('hostname') or ''
    ...
    try:
        ...
        hostname_sync_result = sync_device_hostname_metadata(conn2, device_id, raw_sys_name)
        conn2.commit()
    finally:
        conn2.close()
    if hostname_sync_result:
    ...
    return {
        ...
        "new_hostname": clean_name,
        "raw_sys_name": raw_sys_name,
        "message": f"主机名已成功从“{previous_hostname}”同步为“{clean_name}”"
    }


class ProbeHostnamePayload(BaseModel):
    ip: str = ""
    community: str = ""
    port: int = 161
...
@router.post("/devices/probe-hostname")
async def probe_device_hostname(payload: ProbeHostnamePayload, user=require_role("Viewer")):
    """Directly probe actual sysName from IP via RFC 1213 standard SNMP MIB-II (1.3.6.1.2.1.1.5.0).

    Does NOT require any vendor SNMP template. Works universally across all hardware vendors.
    Useful for interactive UI forms (such as AssetModal or device creation) to auto-fill hostname.
    """
    ip = str(payload.ip or "").strip()
    community = str(payload.community or "").strip()
    port = int(payload.port or 161)

    if payload.credential_id and not community:
        conn = get_db_connection()
        try:
            cred_row = conn.execute(
                "SELECT snmp_community FROM credentials WHERE id = ? OR credential_name = ?",
                (payload.credential_id, payload.credential_id)
            ).fetchone()
            if cred_row:
                from core.crypto import decrypt_credential
                community = decrypt_credential(cred_row["snmp_community"]) or str(cred_row["snmp_community"] or "").strip()
        finally:
            conn.close()

    if payload.device_id and (not ip or not community):
        conn = get_db_connection()
        try:
            device = conn.execute("SELECT * FROM devices WHERE id = ? OR asset_id = ?", (payload.device_id, payload.device_id)).fetchone()
            if device:
                device_data = dict(device)
                creds = resolve_collector_credentials(device_data)
                if not ip:
                    ip = creds["snmp"].get("server") or device_data.get("ip_address") or ""
                if not community:
                    community = creds["snmp"].get("community") or ""
                port = creds["snmp"].get("port") or port or 161
            else:
                asset = conn.execute("SELECT * FROM physical_assets WHERE id = ?", (payload.device_id,)).fetchone()
                if asset:
                    if not ip:
                        ip = asset["management_ip"] or ""
                    if not community and asset["snmp_credential_id"]:
                        cred_row = conn.execute("SELECT snmp_community FROM credentials WHERE id = ?", (asset["snmp_credential_id"],)).fetchone()
                        if cred_row:
                            from core.crypto import decrypt_credential
                            community = decrypt_credential(cred_row["snmp_community"]) or str(cred_row["snmp_community"] or "").strip()
                    if not community and asset["snmp_community"]:
                        from core.crypto import decrypt_credential
                        community = decrypt_credential(asset["snmp_community"]) or str(asset["snmp_community"] or "").strip()
        finally:
            conn.close()
    ...
    if not ip:
        raise HTTPException(status_code=400, detail="请提供有效的设备 IP 地址")

    from services.snmp_service import _snmp_get
    try:
        raw_sys_name = await _snmp_get(ip, community, "1.3.6.1.2.1.1.5.0", port, timeout=3)
    except Exception as exc:
        logger.warning("SNMP sysName probe failed for %s: %s", ip, exc)
        raise HTTPException(status_code=502, detail=f"SNMP 探测连接失败: {exc}")

    if not raw_sys_name:
        raise HTTPException(status_code=502, detail="设备未返回有效的 sysName.0")
...
class BatchSyncHostnamePayload(BaseModel):
    device_ids: list[str] | None = None
    max_concurrency: int = 10


@router.post("/devices/batch-sync-hostname")
async def batch_sync_hostname(
    payload: BatchSyncHostnamePayload = Body(default=BatchSyncHostnamePayload()),
    user=require_role("Operator"),
):
    """Batch probe and synchronize actual sysName for devices via universal RFC 1213 MIB-II (1.3.6.1.2.1.1.5.0).

    Does NOT require any vendor SNMP template. Works universally across all network equipment vendors.
    Safely executes with bounded concurrency (default 10) to prevent network congestion.
    """
    import asyncio
    ...
    try:
        if payload.device_ids:
            placeholders = ",".join(["?"] * len(payload.device_ids))
            devices = conn.execute(
                f"SELECT * FROM devices WHERE id IN ({placeholders})",
                tuple(payload.device_ids),
            ).fetchall()
        else:
            # Query all non-server network devices
            devices = conn.execute(
                """
                SELECT d.* FROM devices d
                WHERE LOWER(COALESCE(d.platform, '')) NOT LIKE '%linux%'
    ...
    if not device_list:
        return {
            ...
            "unchanged": 0,
            "failed": 0,
            "message": "没有找到需要同步主机名的网络设备",
            "details": [],
        }

    from services.snmp_service import _snmp_get

    sem = asyncio.Semaphore(max(1, min(20, payload.max_concurrency)))

    async def _sync_single(device_data: dict) -> dict:
        dev_id = device_data["id"]
        prev_name = device_data.get("hostname") or ""
        ip = device_data.get("ip_address")
        creds = resolve_collector_credentials(device_data)
        snmp_ip = creds["snmp"].get("server") or ip
        community = creds["snmp"].get("community") or ""
        port = creds["snmp"].get("port") or 161

        if not snmp_ip or not community:
            return {
                "device_id": dev_id,
                "ip": snmp_ip or ip,
                "previous_hostname": prev_name,
                "status": "failed",
                "error": "缺少管理 IP 或 SNMP Community 凭据",
            }
        ...
        if not raw_sys_name:
            return {
                ...
                "previous_hostname": prev_name,
                "status": "failed",
                "error": "设备响应中 sysName 为空",
            }

        clean_name = clean_system_hostname(raw_sys_name) or str(raw_sys_name).strip()

        # Update DB if name changed or sys_name changed
        conn_u = get_db_connection()
        try:
            from services.hostname_sync_service import sync_device_hostname_metadata

            hostname_sync_result = sync_device_hostname_metadata(conn_u, dev_id, raw_sys_name)
            conn_u.commit()
        finally:
            conn_u.close()

        is_updated = bool(
            hostname_sync_result
            and hostname_sync_result.get("previous_hostname") != hostname_sync_result.get("hostname")
        )
        return {
            "device_id": dev_id,
            "ip": snmp_ip,
            ...
            "new_hostname": clean_name,
            "raw_sys_name": raw_sys_name,
            "status": "updated" if is_updated else "unchanged",
        }

    results = await asyncio.gather(*[_sync_single(d) for d in device_list])

    updated_count = sum(1 for r in results if r.get("status") == "updated")
    unchanged_count = sum(1 for r in results if r.get("status") == "unchanged")
    failed_count = sum(1 for r in results if r.get("status") == "failed")
    if updated_count:
        from services.hostname_sync_service import schedule_monitoring_target_sync_if_hostname_changed

        first_changed_result = next((result for result in results if result.get("status") == "updated"), None)
        schedule_monitoring_target_sync_if_hostname_changed(first_changed_result)
    ...
    return {
        ...
        "failed": failed_count,
        "message": f"批量同步完成：已处理 {len(results)} 台设备，更新 {updated_count} 台，未变更 {unchanged_count} 台，失败 {failed_count} 台",
        "details": results,
    }
