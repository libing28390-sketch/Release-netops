...
def update_device(device_id: str, device: dict = Body(...)):
    ...
    try:
        ...
        if existing_row['asset_id']:
            conn.execute(
                'UPDATE physical_assets SET lifecycle_status = ?, updated_at = ? WHERE id = ?',
                (new_lifecycle, _utc_now(), existing_row['asset_id']),
            )
        conn.commit()
        ...
        async def _background_sync():
            try:
                ...
                dev_info, intf_data, metrics = await _aio.gather(
                    ...
                    collect_interface_data(snmp_ip, community, port),
                    collect_device_metrics(snmp_ip, platform, community, port),
                )
