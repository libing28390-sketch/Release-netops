...
GET    /api/tags/definitions            — 获取所有标签定义 (可选 ?category=environment)
...
def list_categories(current_user=require_role("Viewer")):
    conn = get_db_connection()
    try:
        return {"success": True, "data": tag_service.list_tag_categories(conn)}
    finally:
        conn.close()
...
def list_definitions(
    ...
    cache_key = f'definitions:{category or ""}'
    cache_hit, cached = read_cache.get('references', cache_key)
    if cache_hit:
        return cached
    conn = get_db_connection()
    try:
        rows = tag_service.list_tag_definitions(conn, category, include_inactive=True)
...
def create_definition(req: TagDefinitionCreate, current_user=require_role("Administrator")):
    conn = get_db_connection()
    try:
        ...
        log_audit_event(
            ...
            severity="info",
            status="success",
            summary=f"Created tag {req.category}/{req.code}",
            actor_username=current_user.get('username', ''),
            actor_role=current_user.get('role', ''),
            target_type="tag_definition",
            target_id=tag["id"],
            details={"category": req.category, "code": req.code},
        )
...
def update_definition(tag_id: str, req: TagDefinitionUpdate, current_user=require_role("Administrator")):
    conn = get_db_connection()
    try:
...
def set_device_tags(device_id: str, req: DeviceTagsUpdate, current_user=require_role("Administrator")):
    conn = get_db_connection()
    try:
...
def add_device_tag(device_id: str, tag_id: str, current_user=require_role("Administrator")):
    conn = get_db_connection()
    try:
...
def remove_device_tag(device_id: str, tag_id: str, current_user=require_role("Administrator")):
    conn = get_db_connection()
    try:
...
def batch_add_tags(req: BatchDeviceTagsUpdate, current_user=require_role("Administrator")):
    conn = get_db_connection()
    try:
...
def query_devices_by_tags(
    ...
    exclude_tag_ids: str = Query(default="", description="Comma-separated tag IDs to exclude"),
    current_user=require_role("Viewer"),
):
    conn = get_db_connection()
    try:
        ...
        excluded = [t.strip() for t in exclude_tag_ids.split(",") if t.strip()]
        groups = [[tag_id] for tag_id in ids] if match_all else [ids]
        device_ids = tag_service.resolve_device_ids_by_filter(conn, groups, excluded)
...
def resolve_tag_targets(req: TagTargetFilter, current_user=require_role("Viewer")):
    conn = get_db_connection()
    try:
        device_ids = (
            tag_service.resolve_device_ids_by_expression(conn, req.expression)
            if req.expression is not None
            else tag_service.resolve_device_ids_by_filter(conn, req.include_groups, req.exclude_tag_ids)
        )
        return {"success": True, "data": {"device_ids": device_ids, "count": len(device_ids)}}
    finally:
        conn.close()
