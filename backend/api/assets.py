...
from services.site_identity_service import ReservedSystemSiteError, resolve_canonical_site_id
...
_PUBLIC_ASSET_FIELDS = frozenset({
    ...
    'winrm_enabled', 'winrm_port', 'winrm_auth_mode', 'winrm_tls_mode',
    'winrm_cert_fingerprint',
...
def _management_port_for_storage(body) -> int:
    ...
    if method == 'winrm':
        return int(getattr(body, 'winrm_port', None) or value or 5986)
...
def _sanitize_asset_item(item: dict) -> dict:
    ...
    if 'snmp_community' in source or 'snmp_credential_id' in source:
        source['snmp_community_set'] = bool(source.get('snmp_community') or source.get('snmp_credential_id'))
    source['ssh_algorithm_profile'] = normalize_ssh_algorithm_profile(
        source.get('ssh_algorithm_profile')
    )
...
def _platform_from_vendor(vendor: str, asset_type: str = 'network_device') -> str:
    ...
    if 'maipu' in v or '迈普' in v:
        return 'maipu'
    if 'hillstone' in v or 'stoneos' in v or '山石' in v:
...
_VENDOR_PLATFORM_RULES: tuple[tuple[str, set[str], str], ...] = (
    ...
    ('hillstone', {'hillstone_stoneos'}, 'hillstone_stoneos'),
    ('stoneos', {'hillstone_stoneos'}, 'hillstone_stoneos'),
    ('山石', {'hillstone_stoneos'}, 'hillstone_stoneos'),
...
def _normalize_asset_platform(vendor: str, platform: str, asset_type: str = 'network_device') -> str:
    ...
    normalized = {
        ...
        'hillstone stoneos': 'hillstone_stoneos',
        '山石': 'hillstone_stoneos',
        '山石 stoneos': 'hillstone_stoneos',
    ...
    for vendor_marker, allowed, default_platform in _VENDOR_PLATFORM_RULES:
        if vendor_marker in vendor_normalized:
            if normalized in allowed:
                if default_platform == 'hillstone_stoneos':
...
def _safe_encrypt_snmp_community(community: str) -> str:
    """Persist a community or fail visibly instead of silently dropping it."""
    if not community:
        return community
    try:
        return encrypt_credential(community)
    except RuntimeError as exc:
        raise HTTPException(
            status_code=500,
            detail='SNMP Community 无法安全存储，请先配置 CREDENTIAL_ENCRYPTION_KEY。',
        ) from exc
...
def _resolve_snmp_credential_binding(conn, body) -> None:
    """Resolve the import-friendly SNMP credential name/id into its id.

    ``snmp_credential_id`` is the established API field and already accepts
    either a credential id or its unique name.  ``snmp_credential_name`` is a
    compatibility alias for spreadsheet/import clients that expose a name as
    a separate column.  Keep the established field authoritative when both
    are supplied, so existing callers cannot be silently redirected by the
    alias.
    """
    if not getattr(body, 'snmp_community', None) and getattr(body, 'community', None):
        body.snmp_community = str(getattr(body, 'community') or '').strip()
    requested_id = str(getattr(body, 'snmp_credential_id', '') or '').strip()
    requested_name = str(getattr(body, 'snmp_credential_name', '') or '').strip()
    requested = requested_id or requested_name
    if not requested:
        body.snmp_credential_id = ''
        return
    row = conn.execute(
        '''SELECT id, credential_name, credential_type, snmp_community
           FROM credentials WHERE id = ? OR credential_name = ?''',
        (requested, requested),
    ).fetchone()
    if not row:
    ...
    dec_community = decrypt_credential(row['snmp_community']) or str(row['snmp_community'] or '').strip()
    if not dec_community:
        raise HTTPException(status_code=422, detail='SNMP凭据未配置 Community')
    body.snmp_credential_id = str(row['id'])
    if hasattr(body, 'snmp_credential_name'):
        body.snmp_credential_name = str(row['credential_name'] or '')
    if not getattr(body, 'snmp_community', None):
        body.snmp_community = dec_community
    ...
    *,
    ...
    conn.execute(
        ...
        (
            ...
            _safe_encrypt_snmp_community(snmp_community) if snmp_community else '',
            now,
        ),
    )
...
def _create_linked_device(conn, asset_id: str, body, user: dict) -> str:
    ...
    existing_snmp_cred_id = getattr(body, 'snmp_credential_id', '') or ''
    if existing_snmp_cred_id and not community:
        snmp_row = conn.execute(
            'SELECT id, credential_name, credential_type, snmp_community FROM credentials WHERE id = ? OR credential_name = ?',
            (existing_snmp_cred_id, existing_snmp_cred_id)
        ).fetchone()
        if snmp_row:
            from core.crypto import decrypt_credential
            dec_snmp = decrypt_credential(snmp_row['snmp_community']) or str(snmp_row['snmp_community'] or '').strip()
            if dec_snmp:
                community = dec_snmp
    ...
    enc_snmp = _safe_encrypt_snmp_community(community) if community else ''
    ...
    ''', (
        ...
        _safe_encrypt_snmp_community(community), port,
    ...
    conn.execute(
        ...
        (normalize_ssh_algorithm_profile(getattr(body, 'ssh_algorithm_profile', None)), device_id),
    )
    ...
    conn.execute(
        '''UPDATE physical_assets
           SET credential_id = ?, admin_credential_id = ?, snmp_credential_id = ?,
               snmp_community = CASE WHEN snmp_community IS NULL OR snmp_community = '' THEN ? ELSE snmp_community END
           WHERE id = ?''',
        (credential_id or '', admin_credential_id or '', getattr(body, 'snmp_credential_id', '') or '', _safe_encrypt_snmp_community(community), asset_id),
    )
...
def _sync_device_from_asset(conn, asset_id: str, body) -> None:
            ...
            if cred_row:
                ...
                if input_id != credential_id:
                    ...
                    conn.execute('''
                        ...
                        WHERE id = ?
                    ''', (c_user, _safe_encrypt(dec_pwd), _safe_encrypt(dec_enable), _safe_encrypt_snmp_community(dec_snmp), device_id))
                    ...
                    conn.execute('''
                        ...
                        SET normal_username = ?, normal_password = ?,
                            enable_password = ?, snmp_community = ?
                        WHERE id = ?
                    ''', (c_user, _safe_encrypt(dec_pwd), _safe_encrypt(dec_enable), _safe_encrypt_snmp_community(dec_snmp), asset_id))
        else:
            # Clear binding
            if credential_id:
    ...
    # If a new snmp_credential_id is passed, update binding
    input_snmp_credential_id = getattr(body, 'snmp_credential_id', None)
    if input_snmp_credential_id is not None:
        if input_snmp_credential_id:
            cred_row = conn.execute(
                'SELECT id, credential_name, snmp_community FROM credentials WHERE id = ? OR credential_name = ?',
                (input_snmp_credential_id, input_snmp_credential_id),
            ).fetchone()
            if cred_row:
                input_id = cred_row['id']
                from core.crypto import decrypt_credential
                dec_snmp = decrypt_credential(cred_row['snmp_community']) or str(cred_row['snmp_community'] or '').strip()
                conn.execute('UPDATE devices SET snmp_credential_id = ?, snmp_community = ? WHERE id = ?',
                             (input_id, _safe_encrypt_snmp_community(dec_snmp), device_id))
                conn.execute('UPDATE physical_assets SET snmp_credential_id = ?, snmp_community = ? WHERE id = ?',
                             (input_id, _safe_encrypt_snmp_community(dec_snmp), asset_id))
        else:
            conn.execute('UPDATE devices SET snmp_credential_id = \'\' WHERE id = ?', (device_id,))
            conn.execute('UPDATE physical_assets SET snmp_credential_id = \'\' WHERE id = ?', (asset_id,))
    ...
    if credential_id:
        ...
        if cred_row:
            ...
            if is_device_specific:
                ...
                stored_snmp = _safe_encrypt_snmp_community(community) if community is not None and community != '' else cred_row['snmp_community']
                conn.execute('''
                    UPDATE credentials
    ...
    updates = {
        ...
        'ssh_algorithm_profile': (
            normalize_ssh_algorithm_profile(getattr(body, 'ssh_algorithm_profile'))
            if getattr(body, 'ssh_algorithm_profile', None) is not None
            else None
        ),
    ...
    if getattr(body, 'snmp_community', None) not in (None, ''):
        updates['snmp_community'] = _safe_encrypt_snmp_community(body.snmp_community)
...
def _delete_linked_device(conn, asset_id: str) -> None:
    ...
    # Server telemetry tables were introduced with explicit foreign keys but
    # without cascade rules on older PostgreSQL installations.  Remove both
    # raw and hourly samples in this transaction before deleting the device.
    conn.execute('DELETE FROM device_telemetry_samples WHERE device_id = ?', (device_id,))
    conn.execute('DELETE FROM device_telemetry_hourly WHERE device_id = ?', (device_id,))
...
class AssetCreate(BaseModel):
    ...
    ssh_algorithm_profile: str = 'auto'
    username: str = ''
    password: str = ''
    # PAM fields
    normal_username: str = ''
    ...
    snmp_community: str = ''
    # Compatibility alias used by older spreadsheets/API clients.
    community: Optional[str] = None
    snmp_port: int = 161
    management_port: int = 22
    winrm_enabled: bool = False
    winrm_port: int = 5986
    winrm_auth_mode: str = 'ntlm'
    winrm_tls_mode: str = 'pin_fingerprint'
    winrm_cert_fingerprint: str = ''
    device_category: str = ''
    function: str = ''
    zone: str = 'Unknown'
    power_watts: int = 0
    ...
    # Import clients may expose a dedicated name column.  The established
    # ``snmp_credential_id`` field remains the canonical output/binding field.
    snmp_credential_name: Optional[str] = ''
    tag_ids: list[str] = []
    tag_codes: str = ''
    ...
    def validate_connection_method(cls, v):
        ...
        if method not in {'ssh', 'netconf', 'winrm', 'web', 'none'}:
            raise ValueError('Connection method must be ssh, netconf, winrm, web or none')
        return method

    @field_validator('ssh_algorithm_profile', mode='before')
    @classmethod
    def validate_ssh_algorithm_profile(cls, v):
        return normalize_ssh_algorithm_profile(v)

    @field_validator('winrm_auth_mode', mode='before')
    @classmethod
    def validate_winrm_auth_mode(cls, v):
        value = str(v or 'ntlm').strip().lower()
        if value not in {'ntlm', 'basic'}:
            raise ValueError('WinRM authentication mode must be ntlm or basic')
        return value

    @field_validator('winrm_tls_mode', mode='before')
    @classmethod
    def validate_winrm_tls_mode(cls, v):
        value = str(v or 'pin_fingerprint').strip().lower()
        if value not in {'verify_ca', 'pin_fingerprint', 'insecure_lab'}:
            raise ValueError('WinRM TLS mode must be verify_ca, pin_fingerprint or insecure_lab')
        return value

    @field_validator('winrm_port', mode='before')
    @classmethod
    def validate_windows_ports(cls, v):
        value = int(v)
        if value < 1 or value > 65535:
            raise ValueError('Windows management port must be between 1 and 65535')
        return value
...
class AssetUpdate(BaseModel):
    ...
    ssh_algorithm_profile: Optional[str] = None
    ...
    winrm_enabled: Optional[bool] = None
    winrm_port: Optional[int] = None
    winrm_auth_mode: Optional[str] = None
    winrm_tls_mode: Optional[str] = None
    winrm_cert_fingerprint: Optional[str] = None
    ...
    snmp_credential_name: Optional[str] = None
    ...
    try:
        ...
        rows = conn.execute(
            f'''SELECT pa.*,
                ...
                sc.credential_name AS snmp_credential_name
            FROM physical_assets pa
            LEFT JOIN devices d ON d.asset_id = pa.id
            LEFT JOIN sites s ON s.id = pa.site_id
            LEFT JOIN credentials sc ON sc.id = COALESCE(NULLIF(pa.snmp_credential_id, ''), d.snmp_credential_id)
...
def _build_asset_import_template(snmpv2_credential_names: list[str]) -> bytes:
    """Build the distributable asset workbook with this vault's SNMPv2 names."""
    ...
    if not template_path.is_file():
        raise FileNotFoundError('Asset import template file is missing')

    workbook = openpyxl.load_workbook(template_path)
    network_sheet = workbook['网络设备']
    credential_headers = {'SNMPv2凭据名称（选填）', 'SNMP凭据（推荐）'}
    credential_column = next(
        (
            cell.column
            for cell in network_sheet[1]
            if str(cell.value or '').strip() in credential_headers
        ),
        None,
    )
    if credential_column is None:
        raise ValueError('SNMP credential column is missing from the asset template')

    names = sorted({str(name).strip() for name in snmpv2_credential_names if str(name or '').strip()}, key=str.casefold)
    helper_sheet_name = '_SNMPv2Credentials'
    if helper_sheet_name in workbook.sheetnames:
        del workbook[helper_sheet_name]
    if 'SNMPv2CredentialNames' in workbook.defined_names:
        del workbook.defined_names['SNMPv2CredentialNames']

    if names:
        helper_sheet = workbook.create_sheet(helper_sheet_name)
        helper_sheet['A1'] = 'SNMPv2 credential names'
        for row_number, credential_name in enumerate(names, start=2):
            cell = helper_sheet.cell(row=row_number, column=1, value=credential_name)
            # Credential names are user-managed text; force text so a name
            # beginning with '=' cannot become an Excel formula.
            cell.data_type = 's'
        helper_sheet.sheet_state = 'hidden'
        workbook.defined_names.add(DefinedName(
            'SNMPv2CredentialNames',
            attr_text=f"'{helper_sheet_name}'!$A$2:$A${len(names) + 1}",
        ))

        column_letter = openpyxl.utils.get_column_letter(credential_column)
        validation = DataValidation(
            type='list',
            formula1='=SNMPv2CredentialNames',
            allow_blank=True,
            errorStyle='stop',
        )
        validation.errorTitle = 'SNMP 凭据无效'
        validation.error = '请选择凭据中心中已存在的 SNMPv2 凭据名称。'
        validation.promptTitle = '选择 SNMPv2 凭据'
        validation.prompt = '只列出凭据中心中现有的 SNMPv2 凭据；不使用凭据中心时可留空。'
        validation.showErrorMessage = True
        validation.showInputMessage = True
        network_sheet.add_data_validation(validation)
...
@router.get('/assets/import-template', response_class=Response)
def download_asset_import_template(_user=require_role('Operator')):
    """Download the asset template with credential choices from the current vault."""
    conn = get_db_connection()
    try:
        rows = conn.execute(
            '''SELECT credential_name FROM credentials
               WHERE LOWER(COALESCE(credential_type, '')) = 'snmpv2'
                 AND credential_name IS NOT NULL
                 AND NULLIF(BTRIM(COALESCE(snmp_community, '')), '') IS NOT NULL
               ORDER BY credential_name'''
        ).fetchall()
        credential_names = [str(row['credential_name'] or '').strip() for row in rows]
    finally:
        conn.close()

    try:
        content = _build_asset_import_template(credential_names)
    except Exception as exc:  # noqa: BLE001
        logger.exception('Failed to build asset import template')
        raise HTTPException(status_code=500, detail='资产导入模板暂时不可用。') from exc
...
def create_asset(body: AssetCreate, _user=require_role("Operator")):
    ...
    try:
        ...
        _resolve_snmp_credential_binding(conn, body)
        now = _utc_now()
        ...
        ''', (
            ...
            _safe_encrypt_snmp_community(body.snmp_community), body.snmp_port, body.device_category,
        ...
        conn.execute(
            '''UPDATE physical_assets SET
                      ...
                      winrm_cert_fingerprint = ?
               WHERE id = ?''',
            (body.winrm_enabled, body.winrm_port, body.winrm_auth_mode, body.winrm_tls_mode,
             body.winrm_cert_fingerprint, asset_id),
        )
        conn.execute(
            'UPDATE physical_assets SET ssh_algorithm_profile = ? WHERE id = ?',
            (normalize_ssh_algorithm_profile(body.ssh_algorithm_profile), asset_id),
        )
...
def update_asset(asset_id: str, body: AssetUpdate, run_bg_rotate: bool = True, _user=require_role("Operator")):
    ...
    try:
        ...
        # Resolve the import/client compatibility alias only after the target
        # asset has passed the resource-scope check.  This prevents callers
        # without access to the asset from probing credential names through
        # validation errors.
        if (
            getattr(body, 'snmp_credential_name', None) is not None
            or getattr(body, 'snmp_credential_id', None) is not None
        ):
            _resolve_snmp_credential_binding(conn, body)
        ...
        updatable = [
            ...
            'ssh_algorithm_profile',
            'winrm_enabled', 'winrm_port', 'winrm_auth_mode', 'winrm_tls_mode', 'winrm_cert_fingerprint'
        ]
        for field in updatable:
            ...
            if val is not None:
                ...
                if field in ('password', 'normal_password', 'admin_password', 'enable_password', 'snmp_community'):
                    ...
                    updates.append(f'{field} = ?')
                    params.append(_safe_encrypt_snmp_community(val) if field == 'snmp_community' else val)
...
def _friendly_import_site_value(value: object) -> str:
    """Return a user-facing site label without leaking internal fallback IDs."""
    raw = str(value or '').strip()
    if raw.casefold() in {'site-default', 'default_site', 'default site'}:
        return '系统默认站点'
    return raw or '（空）'


def _asset_import_site_error(
    body: AssetCreate,
    row_number: int,
    requested_site: object,
    *,
    reason: str,
) -> dict:
    """Build a safe, actionable site validation error for import callers."""
    asset_label = str(body.hostname or body.asset_tag or '').strip()
    if not asset_label:
        asset_label = f'第 {row_number} 条资产'
    site_label = _friendly_import_site_value(requested_site)

    if reason == 'reserved_system_site':
        message = (
            f'第 {row_number} 条资产“{asset_label}”的站点“{site_label}”是系统保留站点，'
            '不能作为业务资产归属。请在 CMDB 站点管理中选择实际业务站点；'
            '如果资产暂时没有站点，请清空“站点/数据中心”列。'
        )
    elif reason == 'unknown_site':
        message = (
            f'第 {row_number} 条资产“{asset_label}”的站点“{site_label}”未在 CMDB 站点中找到。'
            '请填写已存在的站点 ID、编码或名称；如果资产暂时没有站点，请清空“站点/数据中心”列。'
        )
    else:
        message = (
            f'第 {row_number} 条资产“{asset_label}”的站点“{site_label}”无效，'
            '请在 CMDB 站点管理中选择实际业务站点，或清空“站点/数据中心”列。'
        )
...
def import_assets(
    ...
    try:
        ...
        pending_hostname_sync: list[str] = []
        ...
        def _safe_import_error(exc: Exception) -> dict:
            ...
            if isinstance(detail, dict):
                ...
                # Keep only non-sensitive context that helps the UI identify
                # the failed import item.  Never pass through arbitrary detail
                # fields from an exception.
                for key in ('reason', 'row', 'field', 'asset_label', 'site_label'):
        ...
        def _import_one(body: AssetCreate, row_number: int) -> dict:
            ...
            _resolve_snmp_credential_binding(conn, body)
            requested_tag_ids = _resolve_asset_tag_ids(conn, body)
            requested_site = body.site_id
            try:
                body.site_id = resolve_canonical_site_id(conn, body.site_id)
            except ReservedSystemSiteError as exc:
                raise HTTPException(
                    status_code=422,
                    detail=_asset_import_site_error(
                        body,
                        row_number,
                        requested_site,
                ...
                reason = 'unknown_site' if str(exc).startswith('Unknown CMDB site:') else 'invalid_site'
                raise HTTPException(
                    status_code=422,
                    detail=_asset_import_site_error(
                        body,
            ...
            legacy_production = asset_origin == 'legacy' and body.lifecycle_status == 'production'
            initial_lifecycle = 'production' if legacy_production else 'staging'
            was_hostname_empty = not (body.hostname or '').strip()
            if was_hostname_empty:
                if (body.management_ip or '').strip():
                    body.hostname = body.management_ip.strip()
                elif (body.asset_tag or '').strip():
                    body.hostname = body.asset_tag.strip()
                else:
                    raise HTTPException(status_code=422, detail={
                        'code': 'ASSET_IDENTITY_REQUIRED',
                        'message': '导入资产必须至少填写主机名、资产编号或管理IP',
                    })
            if legacy_production:
                reason = (body.takeover_exempt_reason or '').strip()
                normal_username = (body.normal_username or body.username or '').strip()
            ...
            ''', (
                ...
                _safe_encrypt(getattr(body, 'enable_password', '') or ''),
                getattr(body, 'auth_model', '') or 'single',
                _safe_encrypt_snmp_community(body.snmp_community), body.snmp_port,
            ...
            if body.asset_type in ('network_device', 'server'):
                device_id = _create_linked_device(conn, asset_id, body, _user)
                _sync_asset_tags(conn, asset_id, requested_tag_ids)
                if was_hostname_empty and device_id:
                    pending_hostname_sync.append(device_id)
                if legacy_production:
                    conn.execute(
        ...
        if created > 0:
            try:
                ...
                pass
        if pending_hostname_sync:
            import asyncio as _aio
            async def _bg_auto_sync(dids: list[str]):
                for did in dids:
                    try:
                        from api.devices import sync_device_hostname
                        await sync_device_hostname(did)
                    except Exception as e:
                        logger.debug("Auto hostname sync skipped for %s: %s", did, e)
            try:
                loop = _aio.get_running_loop()
                loop.create_task(_bg_auto_sync(pending_hostname_sync))
            except RuntimeError:
                pass
        response = {
            'created': created,
...
def verify_asset_connectivity(asset_id: str, _user=require_role("Operator")):
    ...
    try:
        ...
        ssh_profile = normalize_ssh_algorithm_profile(asset.get('ssh_algorithm_profile'))
        ...
        if not username or not password:
            ...
            if dev:
                ...
                ssh_profile = normalize_ssh_algorithm_profile(
                    device.get('ssh_algorithm_profile') or ssh_profile
                )
        ...
        if username and password:
            try:
                ...
                # Keep the onboarding probe on the same cross-vendor SSH
                # algorithm profile as the normal drivers.
