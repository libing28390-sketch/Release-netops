...
def _create_linked_device(conn, asset_id: str, body, user: dict) -> str:
    ...
    conn.execute(
        '''UPDATE physical_assets
           ...
           WHERE id = ?''',
        (credential_id or '', admin_credential_id or '', getattr(body, 'snmp_credential_id', '') or '', _safe_encrypt(community), asset_id),
    )
