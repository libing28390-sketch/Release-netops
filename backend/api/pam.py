...
from services.pam_recording_storage import (
...
from services.storage_service import StorageError, StorageNotFound, build_storage_service, iter_stream
...
def _enforce_requester_scope(requester_username: str, user: dict) -> None:
    """Keep personal history queries bound to the authenticated requester.

    Administrators retain the existing ability to inspect another user's
    sessions for controlled audit work. Other roles may only pass their own
    username when using the optional requester filter.
    """
    requested = str(requester_username or '').strip()
    if not requested:
        return
    current = str(user.get('username') or '').strip() if isinstance(user, dict) else ''
    role = str(user.get('role') or '').strip() if isinstance(user, dict) else ''
    role_profile = str(user.get('role_profile') or '').strip() if isinstance(user, dict) else ''
    is_admin = role == 'Administrator' or role_profile == 'System Administrator'
    if not is_admin and requested != current:
        raise HTTPException(
            status_code=403,
            detail={
                'code': 'PAM_REQUESTER_SCOPE_DENIED',
                'message': 'Only administrators can query another requester',
            },
        )


def _pam_session_columns(conn) -> set[str]:
    """Return the available PAM session columns from PostgreSQL metadata.

    The production schema is migrated to the complete PAM shape, but audit
    readers may run while a deployment upgrade is still converging. Discovering
    columns lets the list/export path substitute safe defaults in that window.
    """
    try:
        rows = conn.execute(
...
def get_asyncssh_compatible_algorithms(profile: object = None) -> dict[str, list[str]]:
    """Return comprehensive algorithm suites supported by AsyncSSH.

    Modern algorithms remain first.  The default ``auto`` profile appends the
    safe legacy subset needed by older multi-vendor hardware; high-risk names
    such as group1/ssh-dss/MD5 are only returned for the explicit
    ``legacy_break_glass`` profile.
    """
    try:
        ...
        return {
            'encryption_algs': filter_ssh_algorithm_names(
                asyncssh.encryption.get_encryption_algs(), 'encryption', profile
            ),
            'kex_algs': filter_ssh_algorithm_names(
                asyncssh.kex.get_kex_algs(), 'kex', profile
            ),
            'mac_algs': filter_ssh_algorithm_names(
                asyncssh.mac.get_mac_algs(), 'mac', profile
            ),
            'server_host_key_algs': filter_ssh_algorithm_names(
                asyncssh.public_key.get_public_key_algs(), 'server_host_key', profile
            ),
        }
    except Exception:
        return {}
...
def recover_stale_sessions_on_startup():
    ...
    try:
        ...
        orphaned = conn.execute(
            "SELECT * FROM pam_sessions WHERE status IN ('connecting', 'active')"
        ).fetchall()
        stale_spools: list[tuple[str, str, str]] = []
        for orphan in orphaned:
            ...
            spool = str(orphan['recording_spool_path'] or '') if 'recording_spool_path' in orphan.keys() else ''
            if spool:
                content_type = str(orphan['content_type'] or '') if 'content_type' in orphan.keys() else ''
                stale_spools.append((oid, spool, content_type))
        if orphaned:
            conn.commit()
            logger.info(f"[PAM] Recovered {len(orphaned)} stale session(s) from previous server instance")
            for session_id, spool, content_type in stale_spools:
                threading.Thread(
                    target=finalize_pam_recording,
                    args=(session_id, spool),
                    kwargs={'content_type': content_type or None},
                    daemon=True,
                    name=f'pam-recording-finalize-{session_id[:12]}',
                ).start()
        ...
        log_audit_event(
            ...
            details={
                ...
                'ssh_algorithm_profile': normalize_ssh_algorithm_profile(
                    device.get('ssh_algorithm_profile') or asset.get('ssh_algorithm_profile')
                ),
...
def list_sessions(
    ...
    _enforce_requester_scope(requester_username, user)
    conn = get_db_connection()
    try:
        ...
        session_columns = _pam_session_columns(conn)
        conditions = []
        params = []
        tenant_id = str(user.get('tenant_id') or 'tenant-default') if isinstance(user, dict) else None
        if tenant_id and 'tenant_id' in session_columns:
        ...
        # Keep the public response shape stable for databases that predate
        # PAM web-session/audit migrations.  The normal schema selects the
        # real columns; a legacy/fixture table receives conservative defaults
        # for fields that were added later.
        column_defaults = {
            'session_kind': "'ssh_terminal'",
            'web_profile_id': 'NULL',
            'target_scheme': "''",
            'target_path': "''",
            'recording_status': "''",
        ...
        for name in (
            ...
            'recording_spool_path', 'storage_backend', 'storage_bucket', 'object_key',
            'object_version_id', 'object_size', 'object_sha256', 'content_type',
            'storage_status', 'storage_error', 'storage_updated_at',
            'last_heartbeat_at', 'risk_level', 'risk_summary',
            'target_hostname', 'archived', 'created_at', 'updated_at',
        ):
            if name in session_columns:
                selected_columns.append(name)
            else:
                selected_columns.append(f"{column_defaults.get(name, 'NULL')} AS {name}")
        rows = conn.execute(
            f'''
        ...
        # asset_id = NULL after the PostgreSQL foreign-key migration.
        #
        ...
        for item in rows:
            ...
            if item['id'] in _active_sessions:
                item['status'] = 'active'
            item['recording_available'] = bool(
                item.get('recording_path')
                or item.get('recording_spool_path')
                or (item.get('object_key') and item.get('storage_status') == 'READY')
            )
            items.append(item)
...
def export_sessions_csv(
    ...
    for item in items:
        writer.writerow([
            ...
            'yes' if item.get('recording_available') else 'no',
        ])
...
def get_session_recording(session_id: str, user=require_permission('pam', 'view')):
    """Retrieve a recording from the selected provider or legacy disk path."""
    conn = get_db_connection()
    try:
        row = conn.execute('SELECT * FROM pam_sessions WHERE id = ?', (session_id,)).fetchone()
        if not row:
        ...
        if isinstance(user, dict):
            ...
            if session_row:
                ...
                if user_tenant != 'tenant-default' and row_tenant not in {user_tenant, 'tenant-default'}:
                    raise HTTPException(status_code=404, detail='Recording not found')
        row_dict = dict(row)
        object_key = str(row_dict.get('object_key') or '').strip()
        storage_status = str(row_dict.get('storage_status') or 'LEGACY').upper()
        spool_path = str(row_dict.get('recording_spool_path') or '').strip()
        legacy_path = str(row_dict.get('recording_path') or '').strip()
        suffix = Path(object_key or spool_path or legacy_path).suffix.lower()
        media_types = {
        ...
        if object_key and storage_status == 'READY':
            try:
                provider_id = str(row_dict.get('storage_config_id') or '').strip()
                service = build_storage_service(
                    storage_config_id=provider_id or None,
                    backend_name=(str(row_dict.get('storage_backend') or '').strip().lower() or None)
                    if not provider_id else None,
                    bucket=str(row_dict.get('storage_bucket') or '').strip() or None,
                )
                stream = service.get_stream(object_key)
            except StorageNotFound as exc:
                raise HTTPException(status_code=404, detail='Recording not found') from exc
            except StorageError as exc:
                raise HTTPException(status_code=503, detail='Recording storage unavailable') from exc
            return StreamingResponse(
                iter_stream(stream),
                media_type=media_types.get(suffix, row_dict.get('content_type') or 'application/octet-stream'),
                headers={'Content-Disposition': f'attachment; filename="{filename}"'},
            )

        path = spool_path or legacy_path
        if not path or not os.path.exists(path):
            raise HTTPException(status_code=404, detail='Recording not found')
        return FileResponse(path, media_type=media_types.get(suffix, 'application/octet-stream'), filename=filename)
    finally:
        conn.close()
...
async def pam_terminal_ws(websocket: WebSocket, session_token: str):
    ...
    try:
        ...
        ssh_algs = get_asyncssh_compatible_algorithms(
            device.get('ssh_algorithm_profile') or asset.get('ssh_algorithm_profile')
        )
        ssh_conn = await asyncssh.connect(
            ...
            encryption_algs=ssh_algs.get('encryption_algs'),
            kex_algs=ssh_algs.get('kex_algs'),
            mac_algs=ssh_algs.get('mac_algs'),
            server_host_key_algs=ssh_algs.get('server_host_key_algs'),
        )
    ...
    recording_path = str(recording_spool_path(session_id, '.cast'))
    ...
    _update_session_status(session_id, 'active', recording_spool_path=recording_path)
...
def _scan_and_finalize_recording(session_id: str, spool_path: str, content_type: str | None = None) -> None:
    """Finish risk scanning before deleting the local recording spool."""
    _scan_session_for_risk(session_id, spool_path)
    finalize_pam_recording(session_id, spool_path, content_type=content_type)
...
def get_session_commands(session_id: str, user=require_permission('pam', 'view')):
    ...
    try:
        row = conn.execute(
            "SELECT * FROM pam_sessions WHERE id = ?",
            (session_id,),
        ).fetchone()
        if not row:
        ...
        recording_path = str(row['recording_spool_path'] or row['recording_path'] or '') if 'recording_spool_path' in row.keys() else str(row['recording_path'] or '')
        if not recording_path or not os.path.exists(recording_path):
            return {"commands": [], "risk_level": 0, "risk_summary": ""}
...
def _cleanup_old_sessions():
    ...
    try:
            ...
            """SELECT * FROM pam_sessions
               WHERE created_at < ?
                 AND (COALESCE(recording_path, '') <> ''
                      OR COALESCE(recording_spool_path, '') <> ''
                      OR COALESCE(object_key, '') <> '')""",
            (cutoff,)
        ).fetchall()
        ...
        for s in old_sessions:
            sid = s['id']
            try:
                delete_pam_recording(s)
                conn.execute(
                    """UPDATE pam_sessions
                       SET recording_path = '', recording_spool_path = '', object_key = '',
                           storage_status = 'DELETED', storage_error = '', storage_updated_at = ?
                       WHERE id = ?""",
                    (datetime.now(timezone.utc).isoformat(), sid),
                )
                logger.info(f"[PAM PURGE] Deleted recording for session: {sid}")
            except Exception as e:
                logger.error(f"[PAM PURGE] Failed to delete recording for {sid}: {e}")
        conn.commit()
    except Exception as e:
...
def _update_session_status(session_id: str, status: str, close_reason: str = None, recording_spool_path: str = None):
    """Update business status and queue completed recordings for storage."""
    conn = get_db_connection()
    try:
        now = _utc_now()
        if status in ('closed', 'error', 'timeout'):
            ...
            row = conn.execute('SELECT * FROM pam_sessions WHERE id = ?', (session_id,)).fetchone()
            duration = 0
            spool = recording_spool_path or (str(row['recording_spool_path'] or '') if row and 'recording_spool_path' in row.keys() else '')
            legacy_path = str(row['recording_path'] or '') if row and 'recording_path' in row.keys() else ''
            ...
            conn.execute(
                """UPDATE pam_sessions
                   SET status=?, closed_at=?, close_reason=?, duration_seconds=?,
                       recording_spool_path=?,
                       storage_status = CASE WHEN ? <> '' AND COALESCE(object_key, '') = '' THEN 'PENDING' ELSE storage_status END,
                       updated_at=? WHERE id=?""",
                (status, now, close_reason, duration, spool, spool, now, session_id),
            )
            conn.commit()

            if spool:
                threading.Thread(
                    target=_scan_and_finalize_recording,
                    args=(session_id, spool),
                    daemon=True,
                    name=f'pam-recording-finalize-{session_id[:12]}',
                ).start()
            elif legacy_path:
                threading.Thread(target=_scan_session_for_risk, args=(session_id, legacy_path), daemon=True).start()
            
            threading.Thread(target=_cleanup_old_sessions, daemon=True).start()
        else:
            # Active sessions write only to the explicit local spool.  The
            # final object is published after the session closes.
            if recording_spool_path:
                conn.execute(
                    """UPDATE pam_sessions
                       SET status=?, recording_spool_path=?, storage_status='PENDING',
                           storage_error='', connected_at=?, updated_at=? WHERE id=?""",
                    (status, recording_spool_path, now, now, session_id),
                )
            else:
                conn.execute(
...
def create_access_request(payload: AccessRequestIn, user=require_permission('pam', 'request_approval')):
    level = (payload.access_level or 'normal').strip().lower()
    if level not in ('normal', 'admin'):
        raise HTTPException(status_code=400, detail='Invalid access_level')
    ...
    try:
        ...
        ''', (
            request_id, payload.asset_id, user.get('user_id') or user.get('id') or '',
            user['username'], level, payload.reason,
...
def list_access_requests(status: str = '', requester_username: str = '', limit: int = 50, user=require_permission('pam', 'view')):
    _enforce_requester_scope(requester_username, user)
    if user.get('role') != 'Administrator':
...
def approve_access_request(request_id: str, payload: AccessReviewIn, user=require_permission('pam', 'approve')):
    conn = get_db_connection()
    try:
        row = conn.execute('SELECT * FROM pam_access_requests WHERE id = ? FOR UPDATE', (request_id,)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail='Request not found')
        actor_id = str(user.get('user_id') or user.get('id') or '')
        if (actor_id and str(row['requester_user_id'] or '') == actor_id) or row['requester_username'] == user['username']:
            raise HTTPException(status_code=403, detail='Cannot approve your own access request')
        ...
        conn.execute(
            ...
            (user['username'], payload.comment, expires_at, now, request_id),
        )
        conn.commit()
...
def reject_access_request(request_id: str, payload: AccessReviewIn, user=require_permission('pam', 'approve')):
    conn = get_db_connection()
    try:
        row = conn.execute('SELECT * FROM pam_access_requests WHERE id = ? FOR UPDATE', (request_id,)).fetchone()
        if not row:
        ...
        conn.execute(
            ...
            (user['username'], payload.comment, now, request_id),
        )
        conn.commit()
