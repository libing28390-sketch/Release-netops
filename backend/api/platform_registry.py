...
from services.platform_identification_service import (
    bind_device,
    bind_devices_batch,
    unbind_devices_batch,
    identify_device,
    identify_device_from_inventory,
    identify_device_from_snmp,
    identify_device_live,
    list_identification_conflicts,
)
from services.platform_registry_health_service import get_profile_health
...
class PlatformProfileUpdateRequest(_PlatformRequestModel):
    platform_code: str | None = Field(default=None, min_length=3, max_length=64, pattern=r"^[a-z][a-z0-9_]{2,63}$")
    name_zh: str | None = Field(default=None, max_length=128)
    name_en: str | None = Field(default=None, max_length=128)
    vendor: str | None = Field(default=None, max_length=128)
    connection_driver: str | None = Field(default=None, min_length=1, max_length=64)
    parser_platform: str | None = Field(default=None, min_length=1, max_length=64)
...
class PlatformBatchUnbindingRequest(BaseModel):
    device_ids: list[str] = Field(..., min_length=1, max_length=200)
    force: bool = False
...
def api_registry_capabilities(user=require_permission("platform", "read")):
    return {
        "success": True,
        "data": {
            # Kept as a response compatibility field for older clients. Write
            # access is now controlled only by RBAC and lifecycle checks; no
            # environment feature gate can disable the registry.
            "write_enabled": True,
...
def api_update_profile(
    ...
    user=require_permission("platform", "update"),
):
    return {
        "success": True,
        "data": _call(update_profile, profile_id, payload.model_dump(exclude_unset=True), user),
...
def api_delete_profile(
    profile_id: str = Path(..., min_length=1, max_length=128),
    user=require_permission("platform", "delete"),
):
    return {"success": True, "data": _call(delete_profile, profile_id, user), "message": "Platform profile archived"}
...
def api_bind_device(device_id: str, payload: PlatformBindingRequest, user=require_permission("platform", "bind_device")):
    return {
        ...
        "data": _call(
            ...
            lock=payload.lock,
            force=payload.force,
            allow_version_mismatch=payload.allow_version_mismatch,
        ),
...
def api_device_platform_detect(
    ...
    if request.observations:
        ...
        # SNMP/sysDescr-derived version is already inventory data. Prefer it
        # before the gated CLI probe so auto-detection never opens SSH merely
        # to rediscover a version that SNMP has already supplied.
        data = _call(identify_device_from_inventory, device_id, user)
        if data is None:
            # The periodic collector may not have persisted its latest
            # sysDescr yet. Probe SNMP once before falling back to CLI so a
            # currently reachable SNMP agent is not needlessly bypassed.
            data = _call(identify_device_from_snmp, device_id, user)
        if data is None:
            data = _call(identify_device_live, device_id, user)
    return {
        # Keep the compatibility endpoint's HTTP/API envelope stable; the
        # identification outcome (MATCHED/NO_MATCH/CONFLICT) remains in data.
        "success": True,
        "data": data,
        "message": "SNMP inventory version used; CLI probe skipped" if data.get("source") == "snmp_inventory" else "",
    }
...
@device_registry_router.post("/devices/platform-binding/batch/unbind")
def api_device_platform_binding_batch_unbind(
    payload: PlatformBatchUnbindingRequest,
    user=require_permission("platform", "bind_device"),
):
    return {
        "success": True,
        ...
        "message": "Batch device platform binding removed",
    }
