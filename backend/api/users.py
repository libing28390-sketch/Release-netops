...
from core.rbac import RECOMMENDED_ROLE_PROFILES, require_role
...
def _build_user_channels(user_channels: dict, shared_rows: list[dict]) -> dict:
    ...
    for s_row in shared_rows:
        ...
        if not isinstance(user_pref, dict):
            user_pref = {}
        personal_url = str(user_pref.get('webhook_url') or '').strip()
        if '***' in personal_url or '****' in personal_url:
            personal_url = ''
        effective_url = personal_url or url
        masked_url = _mask_webhook_url(effective_url)
        personal_secret = str(user_pref.get('secret') or '').strip()
        if '***' in personal_secret or '****' in personal_secret:
            personal_secret = ''
        effective_secret = personal_secret or str(s_row.get('secret') or '').strip()
        is_enabled = (
        ...
        final_channels[platform] = {
            ...
            'enabled': is_enabled,
            'secret': '***' if effective_secret else '',
            'creator_username': s_row.get('creator_username'),
        }
        if platform == 'email':
            final_channels[platform]['active_enabled'] = user_pref.get('active_enabled') is not False
            final_channels[platform]['recovery_enabled'] = user_pref.get('recovery_enabled') is not False
    # Ensure all standard platforms are present even if not configured.
    for p in ('feishu', 'dingtalk', 'wechat', 'email'):
        if p not in final_channels:
            user_pref = user_channels.get(p, {})
            if not isinstance(user_pref, dict):
                user_pref = {}
            personal_url = str(user_pref.get('webhook_url') or '').strip()
            if '***' in personal_url or '****' in personal_url:
            ...
            if '***' in personal_secret or '****' in personal_secret:
                personal_secret = ''
            final_channels[p] = {
                'webhook_url': _mask_webhook_url(personal_url),
                'enabled': bool(user_pref.get('enabled')) if 'enabled' in user_pref else bool(personal_url),
                'secret': '***' if personal_secret else '',
                'creator_username': '',
            }
            if p == 'email':
                final_channels[p]['active_enabled'] = user_pref.get('active_enabled') is not False
                final_channels[p]['recovery_enabled'] = user_pref.get('recovery_enabled') is not False
...
def _get_session(token: str) -> dict | None:
    ...
    try:
        row = conn.execute(
            ...
            "COALESCE(u.role_profile, '') AS role_profile, "
        ...
        if not row:
            logger.debug("[Auth Session] Session unavailable or user inactive")
            return None
        ...
        if elapsed > _SESSION_TTL:
            logger.debug(
                "[Auth Session] Session expired. elapsed=%.2fs TTL=%ss",
                elapsed, _SESSION_TTL,
            )
...
GRAFANA_SESSION_COOKIE = 'nexora_grafana_session'
...
def _set_grafana_session_cookie(response: Response, request: Request, token: str) -> None:
    response.set_cookie(
...
def _clear_grafana_session_cookie(response: Response, request: Request) -> None:
...
async def create_user(request: Request, user: dict = Body(...)):
    ...
    try:
        ...
        conn.execute(
            'INSERT INTO users (id, username, password, role, role_profile, status, last_login, '
            'avatar_url, group_name, change_groups_json, display_name, phone, email, tenant_id) '
            'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            (
                user_id,
                username,
...
def switch_user(request: Request, response: Response, payload: dict = Body(...)):
    """
    ...
    try:
        ...
        _set_grafana_session_cookie(response, request, new_token)
...
async def login(request: Request, response: Response, payload: dict = Body(...)):
    ...
    try:
        ...
        if user and await _verify_password(password, user['password']):
            ...
            _set_grafana_session_cookie(response, request, token)
            return {
                "success": True,
                "token": token,
                "user": profile,
...
async def update_user(user_id: str, request: Request, user: dict = Body(...)):
    ...
    try:
        ...
        username = user.get('username', existing['username']) if is_admin else existing['username']
        ...
        group_name = str((user.get('group_name', existing['group_name']) if is_admin else existing['group_name']) or '').strip()
        try:
        ...
        change_groups = _normalize_change_groups(user.get('change_groups', existing_change_groups) if is_admin else existing_change_groups)
        ...
        if notification_channels_raw is not None and isinstance(notification_channels_raw, dict):
            for platform, cfg in notification_channels_raw.items():
                if not isinstance(cfg, dict):
                    continue
                if str(platform).strip().lower() == 'email':
                    continue
                
                existing_global = conn.execute(
                    'SELECT webhook_url, secret, creator_username, enabled FROM global_notification_channels WHERE platform = ?',
                    (platform,)
                ).fetchone()
                ...
                if not existing_global:
                    ...
                    if auth_username == creator or is_admin:
        ...
        if password:
            conn.execute('DELETE FROM sessions WHERE user_id = ? AND token != ?', (user_id, session_token))
        conn.commit()
        result = conn.execute(
...
def create_grafana_session(
    ...
    """Bridge an existing Nexora bearer session to a Grafana-only cookie."""
    auth_header = request.headers.get('Authorization', '')
    token = auth_header.replace('Bearer ', '', 1) if auth_header.startswith('Bearer ') else ''
    if not token:
...
def authenticate_grafana_proxy(
    response: Response,
    user=require_role("Viewer"),
):
    """Validate the Nginx auth-proxy bearer and return a read-only Grafana identity."""
    username = str(user.get('username') or '').strip()
    if not re.fullmatch(r'[A-Za-z][A-Za-z0-9._-]{2,31}', username):
        user_id = re.sub(r'[^A-Za-z0-9_-]', '', str(user.get('user_id') or ''))
        if not user_id:
            raise HTTPException(status_code=401, detail="Grafana identity is unavailable")
        username = f'nexora-{user_id}'

    response.headers['X-Grafana-User'] = username
    response.headers['X-Grafana-Role'] = (
        'Admin'
        if user.get('role') == 'Administrator' or user.get('role_profile') == 'System Administrator'
        else 'Viewer'
    )
...
def logout(
    ...
    """Revoke the current Nexora session and clear its Grafana bridge cookie."""
    auth_header = request.headers.get('Authorization', '')
    token = auth_header.replace('Bearer ', '', 1) if auth_header.startswith('Bearer ') else ''
    if token:
        _delete_session(token)
    _clear_grafana_session_cookie(response, request)
    return {"success": True}
...
def mfa_setup(request: Request):
    ...
    conn = get_db_connection()
    try:
        user = conn.execute('SELECT username, mfa_secret, mfa_enabled FROM users WHERE id = ?', (sess['user_id'],)).fetchone()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        if user['mfa_enabled']:
            raise HTTPException(status_code=409, detail="MFA is already enabled; disable it before enrolling again")
...
def mfa_enable(request: Request, payload: dict = Body(...)):
    ...
    try:
        updated = conn.execute(
            'UPDATE users SET mfa_secret = ?, mfa_enabled = 1 WHERE id = ? AND COALESCE(mfa_enabled, 0) = 0',
            (secret, sess['user_id']),
        )
        if updated.rowcount != 1:
            raise HTTPException(status_code=409, detail="MFA is already enabled or user is unavailable")
        conn.commit()
...
async def mfa_verify(request: Request, payload: dict = Body(...), response: Response = None):
    ...
    # Bound guesses for each password-verified ticket, independent of IP.
    token_info['attempts'] = token_info.get('attempts', 0) + 1
    if token_info['attempts'] >= 5:
        _temp_mfa_tokens.pop(temp_token, None)

    username = token_info["username"]
    conn = get_db_connection()
    try:
        user = conn.execute('SELECT * FROM users WHERE username = ?', (username,)).fetchone()
        if not user or user['status'] != 'active' or not user['mfa_enabled'] or not user['mfa_secret']:
