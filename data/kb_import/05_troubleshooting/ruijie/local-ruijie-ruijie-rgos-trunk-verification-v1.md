---
schema_version: '2.0'
document_id: local.ruijie.ruijie_rgos.trunk.verification.v1
title: Ruijie RGOS Trunk 端口 核验与排障（本地官网摘要）
vendor: Ruijie
...
feature: trunk
subfeature: trunk_verification_and_troubleshooting
risk_level: low
verification_level: official_reference_source_checked
rag_priority: 85
source_url: https://www.ruijie.com.cn/fw/wt/37263
source_urls:
- https://www.ruijie.com.cn/fw/wt/37263
...
# Ruijie RGOS Trunk 端口 核验与排障（本地官网摘要）
...
重点核对两端允许 VLAN 集合、Native/PVID 语义和生成树状态；两端命令名称不同但排障维度相同。 常见异常：允许列表不对、Native/PVID 不一致、生成树阻塞或两端协商参数不兼容。 来源页面用于候选证据定位，精确命令和输出字段仍需按型号与软件版本复核。
...
- 重点核对两端允许 VLAN 集合、Native/PVID 语义和生成树状态；两端命令名称不同但排障维度相同。
- 常见异常：允许列表不对、Native/PVID 不一致、生成树阻塞或两端协商参数不兼容。
